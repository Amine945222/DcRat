...
#define HAVE_LOCALTIME_S
         ...
         */
        private static int getDigits(string zDate, int N0, int min0, int max0, char nextC0, ref int pVal0, int N1,
            int min1, int max1, char nextC1, ref int pVal1)
        {
            var c0 = getDigits(zDate + '\0', N0, min0, max0, nextC0, ref pVal0);
            return c0 == 0
                ? 0
        ...
        private static int getDigits(string zDate, int N0, int min0, int max0, char nextC0, ref int pVal0, int N1,
            int min1, int max1, char nextC1, ref int pVal1, int N2, int min2, int max2, char nextC2, ref int pVal2)
        {
            var c0 = getDigits(zDate + '\0', N0, min0, max0, nextC0, ref pVal0);
            if (c0 == 0) return 0;
            var zDate1 = zDate.Substring(zDate.IndexOf(nextC0) + 1);
            var c1 = getDigits(zDate1 + '\0', N1, min1, max1, nextC1, ref pVal1);
            if (c1 == 0) return c0;
        ...
        private static int getDigits(string zDate, int N, int min, int max, char nextC, ref int pVal)
        {
            ...
            while (N-- != 0)
            {
                if (!sqlite3Isdigit(zDate[zIndex])) goto end_getDigits;
...
//} while ( nextC != 0 && zIndex < zDate.Length );
        ...
        private static int parseTimezone(string zDate, DateTime p)
        {
            ...
            //zDate++;
            if (getDigits(zDate.Substring(1), 2, 0, 14, ':', ref nHr, 2, 0, 59, '\0', ref nMn) != 2) return 1;
            //zDate += 5;
            p.tz = sgn * (nMn + nHr * 60);
        ...
        private static int parseHhMmSs(string zDate, DateTime p)
        {
            ...
            var s = 0;
            var ms = 0.0;
            if (getDigits(zDate, 2, 0, 24, ':', ref h, 2, 0, 59, '\0', ref m) != 2) return 1;
            var zIndex = 5; // zDate += 5;
            if (zIndex < zDate.Length && zDate[zIndex] == ':')
            {
                zIndex++; // zDate++;
                if (getDigits(zDate.Substring(zIndex), 2, 0, 59, '\0', ref s) != 1) return 1;
                zIndex += 2; // zDate += 2;
        ...
        private static int parseYyyyMmDd(string zDate, DateTime p)
        {
            ...
            while (zIndex < zDate.Length &&
                   (sqlite3Isspace(zDate[zIndex]) || 'T' == zDate[zIndex])) zIndex++; //zDate++; }
        ...
        {
            var isRealNum = 0; /* Return from sqlite3IsNumber().  Not used */
            if (parseYyyyMmDd(zDate, p) == 0) return 0;

            if (parseHhMmSs(zDate, p) == 0) return 0;

            if (sqlite3StrICmp(zDate, "now") == 0)
            {
                setDateTimeToCurrent(context, p);
                return 0;
            }

            if (sqlite3IsNumber(zDate, ref isRealNum, SQLITE_UTF8) != 0)
            {
                double r = 0;
         ...
         */
        private static int localtimeOffset(DateTime p)
        {
            DateTime x;
            var y = new DateTime();
            time_t t;
            ...
            x.validJD = 0;
            computeJD(x);
            t = x.iJD / 1000 - 210866760000L; //  t = x.iJD/1000 - 21086676*(i64)10000;
#if HAVE_LOCALTIME_R
{
struct tm sLocal;
...
y.s = sLocal.tm_sec;
         ...
         */
        private static int parseModifier(string zMod, DateTime p)
        {
            var rc = 1;
            int n;
            double r = 0;
            var z = new StringBuilder(zMod.ToLower());
            string zBuf; //[30];
            //z = zBuf;
...
#if !SQLITE_OMIT_LOCALTIME
            {
                ...
                case 'w':
                {
                     ...
                     */
                    if (z.ToString().StartsWith("weekday ") && sqlite3AtoF(z.ToString().Substring(8), ref r) !=
                                                            0 //getValue( z[8], ref r ) > 0
                                                            && (n = (int)r) == r && n >= 0 && r < 7)
                    {
                        sqlite3_int64 Z;
                        computeYMD_HMS(p);
                ...
                case '9':
                {
                    ...
                    else if (n == 5 && z.ToString() == "month")
                    {
                        ...
                        computeJD(p);
                        y = (int)r;
                        if (y != r) p.iJD += (long)((r - y) * 30.0 * 86400000.0 + rRounder);
                    }
                    else if (n == 4 && z.ToString() == "year")
                    {
                        ...
                        p.validJD = 0;
                        computeJD(p);
                        if (y != r) p.iJD += (sqlite3_int64)((r - y) * 365.0 * 86400000.0 + rRounder);
                    }
                    else
                    {
                        rc = 1;
        ...
        )
        {
            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(z) || parseDateOrTime(context, z, ref p) != 0) return 1;
            }
        ...
        {
            {
                ...
                StringBuilder z;
                sqlite3 db;
                var zFmt = sqlite3_value_text(argv[0]);
                var zBuf = new StringBuilder(100);
                var argv1 = new sqlite3_value[argc - 1];
                for (i = 0; i < argc - 1; i++)
                {
                ...
                for (i = j = 0; i < zFmt.Length; i++)
                    ...
                    else
                    {
                        ...
                        {
                            case 'd':
                                sqlite3_snprintf(3, ref zTemp, "%02d", x.D);
                                z.Append(zTemp);
                                j += 2;
                                break;
                            case 'f':
                            ...
                            case 'H':
                                sqlite3_snprintf(3, ref zTemp, "%02d", x.h);
                                z.Append(zTemp);
                                j += 2;
                                break;
                            case 'W': /* Fall thru */
                            case 'j':
                            {
                                ...
                                if (zFmt[i] == 'W')
                                {
                                    ...
                                    sqlite3_snprintf(3, ref zTemp, "%02d", (nDay + 7 - wd) / 7);
                                    z.Append(zTemp);
                                    j += 2;
                                }
                                else
                                {
                                    sqlite3_snprintf(4, ref zTemp, "%03d", nDay + 1);
                                    z.Append(zTemp);
                                    j += 3;
                                }
                            ...
                            case 'm':
                                sqlite3_snprintf(3, ref zTemp, "%02d", x.M);
                                z.Append(zTemp);
                                j += 2;
                                break;
                            case 'M':
                            ...
                            case 's':
                            {
                                sqlite3_snprintf(30, ref zTemp, "%lld",
                                    x.iJD / 1000 - 21086676 * (i64)10000);
                                z.Append(zTemp);
                                j = sqlite3Strlen30(z);
                                break;
                            }
                            case 'S':
                                sqlite3_snprintf(3, ref zTemp, "%02d", (int)x.s);
                                z.Append(zTemp);
                                j += 2;
                                break;
                            case 'w':
...
FuncDef[] aFunc = (FuncDef)GLOBAL( FuncDef, aDateTimeFuncs );
            ...
            var aFunc = aDateTimeFuncs;
#endif
            for (i = 0; i < ArraySize(aDateTimeFuncs); i++) sqlite3FuncDefInsert(pHash, aFunc[i]);
        }
    }
}