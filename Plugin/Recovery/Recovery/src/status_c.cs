...
//# define wsdStat x[0]
        ...
        private static readonly sqlite3StatType wsdStat = sqlite3Stat;
#endif
        ...
        private static void sqlite3StatusAdd(int op, int N)
        {
            ...
            if (wsdStat.nowValue[op] > wsdStat.mxValue[op]) wsdStat.mxValue[op] = wsdStat.nowValue[op];
        }
        ...
        {
            wsdStatInit();
            if (op < 0 || op >= ArraySize(wsdStat.nowValue)) return SQLITE_MISUSE;
            pCurrent = wsdStat.nowValue[op];
            pHighwater = wsdStat.mxValue[op];
            if (resetFlag != 0) wsdStat.mxValue[op] = wsdStat.nowValue[op];
            return SQLITE_OK;
        }
        ...
        )
        {
            switch (op)
            {
                case SQLITE_DBSTATUS_LOOKASIDE_USED:
                {
                    ...
                    if (resetFlag != 0) db.lookaside.mxOut = db.lookaside.nOut;
                    break;
                }
                default:
                {
