    ...
    public partial class CSSQLite
    {
        ...
        )
        {
            ...
            if (pOrig.op != TK_COLUMN && (zType.Length == 0 || zType[0] != 'G'))
            {
                ...
                if (pDup == null) return;
                if (pEList.a[iCol].iAlias == 0) pEList.a[iCol].iAlias = (u16)(++pParse.nAlias);
        ...
        )
        {
            ...
            var cnt = 0; /* Number of matching column names */
            var cntTab = 0; /* Number of matching table names */
            ...
            var pTopNC = pNC; /* First namecontext in the list */
            ...
            while (pNC != null && cnt == 0)
            {
                ...
                if (pSrcList != null)
                    for (i = 0; i < pSrcList.nSrc; i++) //, pItem++ )
                    {
                        ...
                        if (zTab != null)
                        {
                            ...
                            {
                                var zTabName = pItem.zAlias;
                                if (sqlite3StrICmp(zTabName, zTab) != 0) continue;
                            }
                            else
                            {
                                var zTabName = pTab.zName;
                                if (NEVER(zTabName == null) || sqlite3StrICmp(zTabName, zTab) != 0) continue;
                                if (zDb != null && sqlite3StrICmp(db.aDb[iDb].zName, zDb) != 0) continue;
                            }
                        }
                ...
                {
                    var pTriggerStack = pParse.trigStack;
                    Table pTab = null;
                    u32 piColMask = 0;
                    var bNew = false;
                    var bOld = false;
...
break;
        ...
        */
                ...
                */
                ...
                case TK_FUNCTION:
                {
                    var pList = pExpr.x.pList; /* The argument list */
                    var n = pList != null ? pList.nExpr : 0; /* Number of arguments */
                    var no_such_func = false; /* True if no such function exists */
                    var wrong_num_args = false; /* True if wrong number of arguments */
                    var is_agg = false; /* True if is an aggregate function */
                    int auth; /* Authorization to use the function */
                    int nId; /* Number of characters in function name */
                    string zId; /* The function name. */
                    FuncDef pDef; /* Information about the function */
                    var enc = pParse.db.aDbStatic[0].pSchema.enc; // ENC( pParse.db );   /* The database encoding */
...
return WRC_Prune;
                ...
                case TK_IN:
                {
                    ...
                    if (ExprHasProperty(pExpr, EP_xIsSelect))
                    {
                        ...
                        if (pNC.isCheck != 0) sqlite3ErrorMsg(pParse, "subqueries prohibited in CHECK constraints");
#endif
                ...
                case TK_VARIABLE:
                {
                    if (pNC.isCheck != 0) sqlite3ErrorMsg(pParse, "parameters prohibited in CHECK constraints");
                    break;
                }
#endif
            }
        ...
        )
        {
            ...
            if (pE.op == TK_ID)
            {
                var zCol = pE.u.zToken;

                for (i = 0; i < pEList.nExpr; i++)
                {
                    var zAs = pEList.a[i].zName;
                    if (zAs != null && sqlite3StrICmp(zAs, zCol) == 0) return i + 1;
                }
            }
...
return 1;
            ...
            while (pSelect != null && moreToDo != 0)
            {
                ...
                for (i = 0; i < pOrderBy.nExpr; i++) //, pItem++)
                {
                    ...
                    {
                        var pColl = pE.pColl;
                        var flags = pE.flags & EP_ExpCollate;
...
return 1;
        ...
        )
        {
            ...
            for (i = 0; i < pOrderBy.nExpr; i++) //, pItem++ )
            {
                ...
                pItem.iCol = 0;
                if (sqlite3ResolveExprNames(pNC, ref pE) != 0) return 1;
            }
        ...
        private static int resolveSelectStep(Walker pWalker, Select p)
        {
            ...
            Debug.Assert(p != null);
            if ((p.selFlags & SF_Resolved) != 0) return WRC_Prune;
            ...
            while (p != null)
            {
                ...
                for (i = 0; i < pEList.nExpr; i++)
                {
                    var pX = pEList.a[i].pExpr;
                    if (sqlite3ResolveExprNames(sNC, ref pX) != 0) return WRC_Abort;
                }
                ...
                */
                ...
                {
                    var pItem = p.pSrc.a[i];
                    if (pItem.pSelect != null)
                    {
                        var zSavedContext = pParse.zAuthContext;
                ...
                */
                 ...
                 */
                if (!isCompound && resolveOrderGroupBy(sNC, p, p.pOrderBy, "ORDER") != 0) return WRC_Abort;
             ...
             ** the compound have been resolved.
             */
            if (isCompound && resolveCompoundOrderBy(pParse, pLeftmost) != 0) return WRC_Abort;
...
#endif
            if (pNC.nErr > 0 || w.pParse.nErr > 0) ExprSetProperty(pExpr, EP_Error);
            if (pNC.hasAgg != 0)
                ExprSetProperty(pExpr, EP_Agg);
            else if (savedHasAgg != 0) pNC.hasAgg = 1;
