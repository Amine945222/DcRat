...
namespace CS_SQLite3
{
 ...
 SQLITE_CORRUPT_BKPT();
        ...
        private static int sqlite3InitOne(sqlite3 db, int iDb, ref string pzErrMsg)
        {
            ...
            var zMasterName = SCHEMA_TABLE(iDb);
            var openedTransaction = 0;
...
//#define temp_master_schema 0
            ...
            */
            ...
            if (pDb.pBt == null)
            {
                if (OMIT_TEMPDB == 0 && ALWAYS(iDb == 1)) DbSetProperty(db, 1, DB_SchemaLoaded);
                return SQLITE_OK;
            }
             ...
             */
            for (i = 0; i < ArraySize(meta); i++) sqlite3BtreeGetMeta(pDb.pBt, i + 1, ref meta[i]);
             ...
             */
            if (iDb == 0 && meta[BTREE_FILE_FORMAT - 1] >= 4) db.flags &= ~SQLITE_LegacyFileFmt;
...
//        db.mallocFailed = 1;
        ...
        {
            int i, rc;
            var commit_internal = !((db.flags & SQLITE_InternChanges) != 0);
            ...
            for (i = 0; rc == SQLITE_OK && i < db.nDb; i++)
            {
                ...
                if (rc != 0) sqlite3ResetInternalSchema(db, i);
            }
                                ...
                                && !DbHasProperty(db, 1, DB_SchemaLoaded))
            {
                ...
                if (rc != 0) sqlite3ResetInternalSchema(db, 1);
            }
#endif

            db.init.busy = 0;
            if (rc == SQLITE_OK && commit_internal) sqlite3CommitInternalChanges(db);
        ...
        private static int sqlite3ReadSchema(Parse pParse)
        {
            ...
            var db = pParse.db;
            Debug.Assert(sqlite3_mutex_held(db.mutex));
            if (0 == db.init.busy) rc = sqlite3Init(db, ref pParse.zErrMsg);
            if (rc != SQLITE_OK)
            {
                pParse.rc = rc;
                pParse.nErr++;
        ...
        private static void schemaIsValid(Parse pParse)
        {
            ...
            {
                var openedTransaction = 0; /* True if a transaction is opened */
                var pBt = db.aDb[iDb].pBt; /* Btree database to read cookie from */
                ...
                if (cookie != db.aDb[iDb].pSchema.schema_cookie) pParse.rc = SQLITE_SCHEMA;

                /* Close the transaction, if one was opened. */
                if (openedTransaction != 0) sqlite3BtreeCommit(pBt);
            }
        }
        ...
        )
        {
            ...
            {
                string zSqlCopy;
                var mxLen = db.aLimit[SQLITE_LIMIT_SQL_LENGTH];
                ...
                if (zSqlCopy != null) sqlite3RunParser(pParse, zSqlCopy, ref zErrMsg);
            ...
            if (pParse.checkSchema != 0) schemaIsValid(pParse);
            if (pParse.rc == SQLITE_SCHEMA) sqlite3ResetInternalSchema(db, 0);
            //if ( db.mallocFailed != 0 )
            //{
            //  pParse.rc = SQLITE_NOMEM;
            //}
            ...
            if (db.init.busy == 0)
            {
                var pVdbe = pParse.pVdbe;
                sqlite3VdbeSetSql(pVdbe, zSql, zSql.Length - (pParse.zTail == null ? 0 : pParse.zTail.Length),
                    saveSqlFlag);
            }
