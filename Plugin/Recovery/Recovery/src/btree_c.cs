...
namespace CS_SQLite3
{
    ...
    public partial class CSSQLite
    {
         ...
         ** SQLite database.
         */
        private static readonly string zMagicHeader = SQLITE_FILE_HEADER;
...
p.eState = CURSOR_INVALID;
        ...
        private static int btreeSetHasContent(BtShared pBt, Pgno pgno)
        {
            ...
            if (null == pBt.pHasContent)
            {
                ...
                if (null == pBt.pHasContent) rc = SQLITE_NOMEM;
            }
        ...
        private static int saveCursorPosition(BtCursor pCur)
        {
            ...
            {
                var pKey = new byte[pCur.nKey]; //void pKey = sqlite3Malloc( (int)pCur.nKey );
                //if( pKey !=null){
                rc = sqlite3BtreeKey(pCur, 0, (u32)pCur.nKey, pKey);
                if (rc == SQLITE_OK) pCur.pKey = pKey;
                //else{
        ...
        )
        {
            ...
            if (pKey != null) sqlite3VdbeDeleteUnpackedRecord(pIdxKey);
            return rc;
        }
...
//#define ptrmapPutOvflPtr(x, y, rc)
        ...
        )
        {
            ...
            if (likely(nPayload <= pPage.maxLocal))
            {
                ...
                pInfo.iOverflow = 0;
                if ((nSize & ~3) == 0) nSize = 4; /* Minimum cell size is 4 */
        ...
        private static u16 cellSizePtr(MemPage pPage, int iCell)
        {
            ...
            var
                pCell = new byte[13]; // Minimum Size = (2 bytes of Header  or (4) Child Pointer) + (maximum of) 9 bytes data
        ...
        {
            var info = new CellInfo();
            var pTemp = new byte[pCell.Length];
...
** this function verifies that this invariant is not violated. */
      ...
      btreeParseCellPtr(pPage, pCell, ref debuginfo);
            ...
            if (nSize > pPage.maxLocal)
            {
                ...
                testcase(nSize == pPage.maxLocal);
                testcase(nSize == pPage.maxLocal + 1);
                if (nSize > pPage.maxLocal) nSize = (u32)minLocal;
                nSize += 4;
            }
    ...
    static u16 cellSize(MemPage pPage, int iCell)
    {
      return cellSizePtr(pPage, findCell(pPage, iCell));
...
#else
        private static int cellSize(MemPage pPage, int iCell)
        {
            return -1;
        }
#endif
        ...
        private static void ptrmapPutOvflPtr(MemPage pPage, int pCell, ref int pRC)
        {
            ...
            if (info.iOverflow != 0)
            {
                var ovfl = sqlite3Get4byte(pPage.aData, pCell, info.iOverflow);
        ...
        private static void ptrmapPutOvflPtr(MemPage pPage, u8[] pCell, ref int pRC)
        {
            ...
            if (info.iOverflow != 0)
            {
                var ovfl = sqlite3Get4byte(pCell, info.iOverflow);
                ptrmapPut(pPage.pBt, ovfl, PTRMAP_OVERFLOW1, pPage.pgno, ref pRC);
            }
        }
#endif
...
return SQLITE_CORRUPT_BKPT;
        ...
        private static int allocateSpace(MemPage pPage, int nByte, ref int pIdx)
        {
            ...
            var data = pPage.aData; /* Local cache of pPage.aData */
        ...
        return SQLITE_CORRUPT_BKPT();
            ...
            else if (gap + 2 <= top)
            {
                ...
                {
                    var size = get2byte(data, pc + 2); /* Size of free slot */
                    if (size >= nByte)
                    {
                        var x = size - nByte;
                        testcase(x == 4);
...
memset(data[start], 0, size);
        ...
        {
            var pPage = sqlite3PagerGetExtra(pDbPage);
        ...
        private static MemPage btreePageLookup(BtShared pBt, Pgno pgno)
        {
            ...
            if (pDbPage) return btreePageFromDbPage(pDbPage, pgno, pBt);
            return null;
        }
         ...
         */
        private static Pgno pagerPagecount(BtShared pBt)
        {
            var nPage = -1;
            int rc;
      ...
      Pgno iLastPg = pagerPagecount(pBt);//  TESTONLY( Pgno iLastPg = pagerPagecount(pBt); )
            ...
            if (rc == SQLITE_OK)
            {
                ...
                if (rc != SQLITE_OK) releasePage(ppPage);
            }
        ...
        )
        {
            ...
            var rc = SQLITE_OK; /* Result code from this function */
...
p.sharable = 1;
            ...
            if (pBt == null)
            {
                ...
                if (rc == SQLITE_OK) rc = sqlite3PagerReadFileheader(pBt.pPager, zDbHeader.Length, zDbHeader);
...
break;
            ...
            if (rc != SQLITE_OK)
            {
                if (pBt != null && pBt.pPager != null) sqlite3PagerClose(pBt.pPager);
...
return removed;
        ...
        {
            if (null == pBt.pTmpSpace) pBt.pTmpSpace = new byte[pBt.pageSize]; //sqlite3PageMalloc( pBt.pageSize );
        }
        ...
        private static int sqlite3BtreeClose(ref Btree p)
        {
            ...
            while (pCur != null)
            {
                var pTmp = pCur;
                pCur = pCur.pNext;
                if (pTmp.pBtree == p) sqlite3BtreeCloseCursor(pTmp);
            }
            ...
            if (!p.sharable || removeFromSharingList(pBt))
            {
                ...
                if (pBt.xFreeSchema != null && pBt.pSchema != null) pBt.xFreeSchema(pBt.pSchema);
...
return BTREE_AUTOVACUUM_NONE;
        ...
        private static int lockBtree(BtShared pBt)
        {
            ...
            if (nPage > 0)
            {
                ...
                rc = SQLITE_NOTADB;
                if (memcmp(page1, zMagicHeader, 16) != 0) goto page1_init_failed;
                if (page1[18] > 1) pBt.readOnly = true;
        ...
        private static int newDatabase(BtShared pBt)
        {
            ...
            rc = sqlite3PagerPagecount(pBt.pPager, ref nPage);
            if (NEVER(rc != SQLITE_OK) || nPage > 0) return rc;
            pP1 = pBt.pPage1;
            Debug.Assert(pP1 != null);
            data = pP1.aData;
            rc = sqlite3PagerWrite(pP1.pDbPage);
            if (rc != 0) return rc;
            Buffer.BlockCopy(Encoding.UTF8.GetBytes(zMagicHeader), 0, data, 0,
                16); // memcpy(data, zMagicHeader, sizeof(zMagicHeader));
        ...
        private static int sqlite3BtreeBeginTrans(Btree p, int wrflag)
        {
             ...
             */
            if (p.inTrans == TRANS_WRITE || (p.inTrans == TRANS_READ && 0 == wrflag)) goto trans_begun;
            ...
            do
            {
                ...
                if (rc == SQLITE_OK && wrflag != 0)
                {
                    ...
                    else
                    {
                        ...
                        if (rc == SQLITE_OK) rc = newDatabase(pBt);
                    }
                }

                if (rc != SQLITE_OK) unlockBtreeIfUnused(pBt);
...
pBt.isExclusive = (u8)(wrflag>1);
        ...
        private static int setChildPtrmaps(MemPage pPage)
        {
            ...
            for (i = 0; i < nCell; i++)
            {
                ...
                if (0 == pPage.leaf)
                {
                    var childPgno = sqlite3Get4byte(pPage.aData, pCell);
            ...
            if (0 == pPage.leaf)
            {
                var childPgno = sqlite3Get4byte(pPage.aData, pPage.hdrOffset + 8);
                ptrmapPut(pBt, childPgno, PTRMAP_BTREE, pgno, ref rc);
            }
        ...
        )
        {
            ...
            {
                var nextOvfl = sqlite3Get4byte(pDbPage.aData);
                if (nextOvfl != 0)
                {
            ...
            if (eType != PTRMAP_ROOTPAGE)
            {
                ...
                releasePage(pPtrPage);
                if (rc == SQLITE_OK) ptrmapPut(pBt, iFreePage, eType, iPtrPage, ref rc);
            }
        ...
        private static int incrVacuumStep(BtShared pBt, Pgno nFin, Pgno iLastPg)
        {
            ...
            if (!PTRMAP_ISPAGE(pBt, iLastPg) && iLastPg != PENDING_BYTE_PAGE(pBt))
            {
                ...
                if (nFreeList == 0) return SQLITE_DONE;
          ...
          return SQLITE_CORRUPT_BKPT();
                ...
                else
                {
                    ...
                    if (rc == SQLITE_OK) rc = relocatePage(pBt, pLastPg, eType, iPtrPage, iFreePg, nFin != 0 ? 1 : 0);
                    releasePage(pLastPg);
                    if (rc != SQLITE_OK) return rc;
                }
            }
      ...
      int nRef = sqlite3PagerRefcount(pPager);
          ...
          return SQLITE_CORRUPT_BKPT();
                ...
                for (iFree = nOrig; iFree > nFin && rc == SQLITE_OK; iFree--) rc = incrVacuumStep(pBt, nFin, iFree);
                ...
                if (rc != SQLITE_OK) sqlite3PagerRollback(pPager);
            }
...
//# define setChildPtrmaps(x) SQLITE_OK
        ...
        private static int sqlite3BtreeCommit(Btree p)
        {
            ...
            if (rc == SQLITE_OK) rc = sqlite3BtreeCommitPhaseTwo(p);
            sqlite3BtreeLeave(p);
            return rc;
        }
...
sqlite3BtreeTripAllCursors(p, rc);
            ...
            if (p.inTrans == TRANS_WRITE)
            {
                ...
                if (btreeGetPage(pBt, 1, ref pPage1, 0) == SQLITE_OK) releasePage(pPage1);
        ...
        )
        {
            ...
            if (NEVER(wrFlag != 0 && pBt.readOnly)) return SQLITE_READONLY;
            if (iTable == 1 && pagerPagecount(pBt) == 0) return SQLITE_EMPTY;
            ...
            if (pCur.pNext != null) pCur.pNext.pPrev = pCur;
            pBt.pCursor = pCur;
            pCur.eState = CURSOR_INVALID;
            pCur.cachedRowid = 0;
            return SQLITE_OK;
        ...
        private static int sqlite3BtreeCloseCursor(BtCursor pCur)
        {
            ...
            if (pBtree != null)
            {
                ...
                for (i = 0; i <= pCur.iPage; i++) releasePage(pCur.apPage[i]);
                unlockBtreeIfUnused(pBt);
                invalidateOverflowCache(pCur);
      ...
      return pCur != null && pCur.eState == CURSOR_VALID;
        ...
        )
        {
            ...
            if (rc == SQLITE_OK)
            {
                ...
                if (rc == SQLITE_OK) next = sqlite3Get4byte(pPage.aData);
            }
        ...
        )
        {
            if (eOp != 0)
            {
                ...
                var rc = sqlite3PagerWrite(pDbPage);
                if (rc != SQLITE_OK) return rc;
        ...
        return SQLITE_CORRUPT_BKPT();
            ...
            {
                var a = (int)amt;
                if (a + offset > pCur.info.nLocal) a = (int)(pCur.info.nLocal - offset);
            ...
            {
                var ovflSize = (u32)(pBt.usableSize - 4); /* Bytes content per ovfl page */
                Pgno nextPage;
...
nextPage = pCur.aOverflow[iIdx+1];
                    ...
                    else
                    {
                        ...
                        if (rc == SQLITE_OK)
                        {
                            ...
                            if (a + offset > ovflSize) a = (int)(ovflSize - offset);
...
return SQLITE_ABORT;
        ...
        private static byte[] sqlite3BtreeKeyFetch(BtCursor pCur, ref int pAmt, ref int outOffset)
        {
            ...
            Debug.Assert(cursorHoldsMutex(pCur));
            if (ALWAYS(pCur.eState == CURSOR_VALID)) p = fetchPayload(pCur, ref pAmt, ref outOffset, false);
            return p;
        }
        ...
        private static byte[] sqlite3BtreeDataFetch(BtCursor pCur, ref int pAmt, ref int outOffset)
        {
            ...
            Debug.Assert(sqlite3_mutex_held(pCur.pBtree.db.mutex));
            Debug.Assert(cursorHoldsMutex(pCur));
            if (ALWAYS(pCur.eState == CURSOR_VALID)) p = fetchPayload(pCur, ref pAmt, ref outOffset, true);
            return p;
        }
...
//#  define assertParentIndex(x,y,z)
        ...
        private static int moveToRoot(BtCursor pCur)
        {
            ...
            {
                int i;
                for (i = 1; i <= pCur.iPage; i++) releasePage(pCur.apPage[i]);
                pCur.iPage = 0;
            }
            else
            {
...
** to the last entry in the b-tree. */
        ...
        private static int sqlite3BtreeNext(BtCursor pCur, ref int pRes)
        {
            ...
            pRes = 0;
            if (pPage.leaf != 0) return SQLITE_OK;
            rc = moveToLeftmost(pCur);
            return rc;
        }
        ...
        return SQLITE_CORRUPT_BKPT();
            ...
            rc = SQLITE_CORRUPT_BKPT();
                    ...
                    if (k == 0 && 0 == searchList)
                    {
                        ...
                        if (rc != 0) goto end_allocate_page;
                        pPgno = iTrunk;
            ...
            rc = SQLITE_CORRUPT_BKPT();
                    ...
                    else if (searchList != 0 && nearby == iTrunk)
                    {
                        ...
                        else
                        {
                            ...
                            var iNewTrunk = sqlite3Get4byte(pTrunk.aData, 8);
                            if (iNewTrunk > mxPage)
                            {
#if SQLITE_DEBUG || DEBUG
                rc = SQLITE_CORRUPT_BKPT();
                            ...
                            if (rc != SQLITE_OK) goto end_allocate_page;
                            ...
                            sqlite3Put4byte(pNewTrunk.aData, (u32)4, k - 1);
                    ...
                    else if (k > 0)
                    {
                        ...
                        if (nearby > 0)
                        {
                            ...
                            for (i = 1; i < k; i++)
                            {
                                var d2 = (int)(sqlite3Get4byte(aData, 8 + i * 4) - nearby);
                                if (d2 < 0) d2 = -d2;
                                if (d2 < dist)
                                {
                                    closest = i;
...
if (false)
            ...
            if (nFree != 0)
            {
                ...
                iTrunk = sqlite3Get4byte(pPage1.aData, 32);
                rc = btreeGetPage(pBt, iTrunk, ref pTrunk, 0);
                if (rc != SQLITE_OK) goto freepage_out;
          ...
          rc = SQLITE_CORRUPT_BKPT();
...
#if !SQLITE_SECURE_DELETE
                        if (pPage != null) sqlite3PagerDontWrite(pPage.pDbPage);
#endif
             ...
             */
            if (pPage == null && SQLITE_OK != (rc = btreeGetPage(pBt, iPage, ref pPage, 0))) goto freepage_out;
            rc = sqlite3PagerWrite(pPage.pDbPage);
            if (rc != SQLITE_OK) goto freepage_out;
        ...
        {
            if (pRC == SQLITE_OK) pRC = freePage2(pPage.pBt, pPage, pPage.pgno);
        }
        ...
        private static int clearCell(MemPage pPage, int pCell)
        {
            ...
            Debug.Assert(sqlite3_mutex_held(pPage.pBt.mutex));
            btreeParseCellPtr(pPage, pCell, ref info);
            if (info.iOverflow == 0) return SQLITE_OK; /* No overflow pages. Return without doing anything */
          ...
          return SQLITE_CORRUPT_BKPT();
                ...
                if (pOvfl != null) sqlite3PagerUnref(pOvfl.pDbPage);
                if (rc != 0) return rc;
                ovflPgno = iNext;
            }
        ...
        )
        {
            ...
            /* Fill in the header. */
            nHeader = 0;
            if (0 == pPage.leaf) nHeader += 4;
            if (pPage.hasData != 0)
                nHeader += putVarint(pCell, nHeader, nData + nZero); //putVarint( pCell[nHeader], nData + nZero );
            else
                nData = nZero = 0;
          ...
          return SQLITE_CORRUPT_BKPT();
            ...
            while (nPayload > 0)
            {
                if (spaceLeft == 0)
                {
                    ...
                    {
                        var eType = (u8)(pgnoPtrmap != 0 ? PTRMAP_OVERFLOW2 : PTRMAP_OVERFLOW1);
                        ptrmapPut(pBt, pgnoOvfl, eType, pgnoPtrmap, ref rc);
                        if (rc != 0) releasePage(pOvfl);
                    }
#endif
                    if (rc != 0)
                    {
                ...
                {
                    var pZeroBlob = new byte[n]; // memset(pPayload, 0, n);
        ...
        {
            var idx = 0; /* Where to write new cell content in data[] */
            int j; /* Loop counter */
            ...
            if (pPage.nOverflow != 0 || sz + 2 > pPage.nFree)
            {
                ...
                if (iChild != 0) sqlite3Put4byte(pCell, iChild);
                j = pPage.nOverflow++;
            ...
            {
                var rc = sqlite3PagerWrite(pPage.pDbPage);
                if (rc != SQLITE_OK)
                {
                    pRC = rc;
                    return;
                ...
                if (iChild != 0) sqlite3Put4byte(data, idx, iChild);
        ...
        )
        {
            ...
            for (i = nCell - 1; i >= 0; i--)
            {
                ...
                Buffer.BlockCopy(apCell, 0, data, cellbody,
                    aSize[i]); //          memcpy(data[cellbody], apCell[i], aSize[i]);
            }
...
if (false)
#endif
                {
                    ...
                    if (szCell[0] > pNew.minLocal) ptrmapPutOvflPtr(pNew, pCell, ref rc);
                }
                 ...
                 */
                var iCell = findCell(pPage, pPage.nCell - 1); //pCell = findCell( pPage, pPage.nCell - 1 );
                pCell = pPage.aData;
                var _pCell = iCell;
...
return 1;
        ...
        int rc;//    TESTONLY(int rc;)
                ...
                Buffer.BlockCopy(aFrom, iData, aTo, iData,
                    pBt.usableSize - iData); //memcpy(aTo[iData], ref aFrom[iData], pBt.usableSize-iData);
...
if (false)
        ...
        )
        {
            ...
            var nCell = 0; /* Number of cells in apCell[] */
            var nMaxCells = 0; /* Allocated size of apCell, szCell, aFrom. */
            var nNew = 0; /* Number of pages in apNew[] */
            int nOld; /* Number of pages in apOld[] */
            int i, j, k; /* Loop counters */
            int nxDiv; /* Next divider slot in pParent.aCell[] */
            var rc = SQLITE_OK; /* The return code */
            ...
            var iOvflSpace = 0; /* First unused byte of aOvflSpace[] */
...
apDiv[i] = &aOvflSpace[apDiv[i]-pParent.aData];
            ...
            for (i = 0; i < nOld; i++)
            {
                ...
                {
                    var sz = (u16)szNew[i];
                    var pTemp = new byte[sz + leafCorrection];
                    Debug.Assert(nCell < nMaxCells);
                    szCell[nCell] = sz;
            ...
            */
            ...
            for (i = 0; i < nNew; i++)
            {
                ...
                if (j < nCell)
                {
                    ...
                    {
                        //------------ pCell -= 4;
                        var _pCell_4 = new byte[pCell.Length + 4];
...
#endif
            {
                ...
                for (i = 0; i < nCell; i++)
                {
                    ...
                    if (isDivider != 0 || pOld.pgno != pNew.pgno)
                    {
                        ...
                        if (szCell[i] > pNew.minLocal) ptrmapPutOvflPtr(pNew, apCell[i], ref rc);
                    }
                }
                ...
                if (0 == leafCorrection)
                    for (i = 0; i < nNew; i++)
                    {
                        var key = sqlite3Get4byte(apNew[i].aData, 8);
...
if (false)
        ...
        {
            var rc = SQLITE_OK;
            var nMin = pCur.pBt.usableSize * 2 / 3;
            var aBalanceQuickSpace = new u8[13];
            u8[] pFree = null;
      ...
      int balance_deeper_called = 0;//TESTONLY( int balance_deeper_called = 0 );
                ...
                else
                {
                    ...
                    if (rc == SQLITE_OK)
                    {
...
#endif
                        {
                             ...
                             */
                            var pSpace =
                                new u8[pCur.pBt.pageSize]; // u8 pSpace = sqlite3PageMalloc( pCur.pBt.pageSize );
        ...
        )
        {
            ...
            if (pCur.pKeyInfo == null) invalidateIncrblobCursors(p, nKey, 0);
        ...
        private static int sqlite3BtreeDelete(BtCursor pCur)
        {
            ...
            if (pCur.pKeyInfo == null) invalidateIncrblobCursors(p, pCur.info.nKey, 0);
            ...
            {
                while (pCur.iPage > iCellDepth) releasePage(pCur.apPage[pCur.iPage--]);
                rc = balance(pCur);
            }

            if (rc == SQLITE_OK) moveToRoot(pCur);
            return rc;
        }
...
return rc;
        ...
        return SQLITE_CORRUPT_BKPT();
            ...
            else if ((rc = sqlite3PagerWrite(pPage.pDbPage)) == 0) zeroPage(pPage, pPage.aData[0] | PTF_LEAF);
        ...
        private static int sqlite3BtreeClearTable(Btree p, int iTable, ref int pnChange)
        {
            ...
            if (SQLITE_OK == rc) rc = clearDatabasePage(pBt, (Pgno)iTable, 0, ref pnChange);
            sqlite3BtreeLeave(p);
            return rc;
        }
...
if( idx==BTREE_LARGEST_ROOT_PAGE && pMeta>0 ) pBt.readOnly = 1;
        ...
        private static int sqlite3BtreeCount(BtCursor pCur, ref i64 pnEntry)
        {
            ...
            while (rc == SQLITE_OK)
            {
                 ...
                 */
                pPage = pCur.apPage[pCur.iPage];
                if (pPage.leaf != 0 || 0 == pPage.intKey) nEntry += pPage.nCell;
        ...
        )
        {
            ...
            pCheck.nErr++;
            va_start(ap, zFormat);
            if (pCheck.errMsg.nChar != 0) sqlite3StrAccumAppend(pCheck.errMsg, "\n", 1);
            if (!string.IsNullOrEmpty(zMsg1)) sqlite3StrAccumAppend(pCheck.errMsg, zMsg1, -1);
        ...
        )
        {
            ...
            while (N-- > 0 && pCheck.mxErr != 0)
            {
                ...
                if (isFreeList != 0)
                {
                    var n = (int)sqlite3Get4byte(pOvflData, 4);
#if !SQLITE_OMIT_AUTOVACUUM
                    if (pCheck.pBt.autoVacuum) checkPtrmap(pCheck, (u32)iPage, PTRMAP_FREEPAGE, 0, zContext);
#endif
                    ...
                    else
                    {
                        for (i = 0; i < n; i++)
                        {
                            var iFreePage = sqlite3Get4byte(pOvflData, 8 + i * 4);
#if !SQLITE_OMIT_AUTOVACUUM
        ...
        )
        {
            ...
            */
            ...
            for (i = 0; i < pPage.nCell && pCheck.mxErr != 0; i++)
            {
                ...
                */
                ...
                var iCell = findCell(pPage, i); //pCell = findCell( pPage, i );
                pCell = pPage.aData;
                ...
                {
                    var nPage = (int)(sz - info.nLocal + usableSize - 5) / (usableSize - 4);
                    var pgnoOvfl = sqlite3Get4byte(pCell, iCell, info.iOverflow);
#if !SQLITE_OMIT_AUTOVACUUM
                    if (pBt.autoVacuum) checkPtrmap(pCheck, pgnoOvfl, PTRMAP_OVERFLOW1, (u32)iPage, zContext);
#endif
...
#if !SQLITE_OMIT_AUTOVACUUM
                    if (pBt.autoVacuum) checkPtrmap(pCheck, (u32)pgno, PTRMAP_BTREE, (u32)iPage, zContext);
#endif
...
#if !SQLITE_OMIT_AUTOVACUUM
                if (pBt.autoVacuum) checkPtrmap(pCheck, (u32)pgno, PTRMAP_BTREE, (u32)iPage, "");
#endif
            ...
            */
            ...
            //}else
            {
                var contentOffset = (u16)get2byte(data, hdr + 5);
                Debug.Assert(contentOffset <= usableSize); /* Enforced by btreeInitPage() */
                //memset(hit+contentOffset, 0, usableSize-contentOffset);
                //memset(hit, 1, contentOffset);
                for (var iLoop = contentOffset - 1; iLoop >= 0; iLoop--) hit[iLoop] = 1;
                nCell = get2byte(data, hdr + 3);
                cellStart = hdr + 12 - 4 * pPage.leaf;
                for (i = 0; i < nCell; i++)
                {
                    var pc = get2byte(data, cellStart + i * 2);
                    u16 size = 1024;
                    int j;
                    if (pc <= usableSize - 4) size = cellSizePtr(pPage, data, pc);
...
#if !SQLITE_OMIT_AUTOVACUUM
                if (pBt.autoVacuum && aRoot[i] > 1) checkPtrmap(sCheck, (u32)aRoot[i], PTRMAP_ROOTPAGE, 0, "");
#endif
