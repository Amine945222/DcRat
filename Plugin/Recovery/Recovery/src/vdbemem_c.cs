...
return rc;
        ...
        private static int sqlite3VdbeMemGrow(Mem pMem, int n, int preserve)
        {
            ...
            //}
            if ((pMem.flags & MEM_Dyn) != 0 && pMem.xDel != null) pMem.xDel(ref pMem.z);
...
return SQLITE_OK;
        ...
        {
            Debug.Assert(pMem.db == null || sqlite3_mutex_held(pMem.db.mutex));
            if ((pMem.flags & MEM_Term) != 0 || (pMem.flags & MEM_Str) == 0) return SQLITE_OK; /* Nothing to do */
        ...
        private static int sqlite3VdbeMemStringify(Mem pMem, int enc)
        {
            ...
            if (sqlite3VdbeMemGrow(pMem, nByte, 0) != 0) return SQLITE_NOMEM;
        ...
        private static i64 sqlite3VdbeIntValue(Mem pMem)
        {
            ...
            // assert( EIGHT_BYTE_ALIGNMENT(pMem) );
            flags = pMem.flags;
            if ((flags & MEM_Int) != 0) return pMem.u.i;

            if ((flags & MEM_Real) != 0) return doubleToInt64(pMem.r);
        ...
        private static double sqlite3VdbeRealValue(Mem pMem)
        {
            Debug.Assert(pMem.db == null || sqlite3_mutex_held(pMem.db.mutex));
            //assert( EIGHT_BYTE_ALIGNMENT(pMem) );
            if ((pMem.flags & MEM_Real) != 0) return pMem.r;

            if ((pMem.flags & MEM_Int) != 0) return pMem.u.i;
        ...
        private static void sqlite3VdbeIntegerAffinity(Mem pMem)
        {
             ...
             */
            if (pMem.r == pMem.u.i && pMem.u.i - 1 < pMem.u.i + 1) pMem.flags |= MEM_Int;
        }
        ...
        private static bool sqlite3VdbeMemTooBig(Mem p)
        {
            ...
            {
                var n = p.n;
                if ((p.flags & MEM_Zero) != 0) n += p.u.nZero;
        ...
        {
            var nByte = n; /* New value for pMem->n */
            ...
            {
                var nAlloc = (u32)nByte;
                if ((flags & MEM_Term) != 0) nAlloc += (u32)(enc == SQLITE_UTF8 ? 1 : 2);
                if (nByte > iLimit) return SQLITE_TOOBIG;
                if (sqlite3VdbeMemGrow(pMem, (int)nAlloc, 0) != 0) return SQLITE_NOMEM;
                //if ( nAlloc < z.Length )
                ...
                if (enc == 0)
                {
                    pMem.z = null;
                    pMem.zBLOB = new byte[n];
                    for (var i = 0; i < n && i < z.Length; i++) pMem.zBLOB[i] = (byte)z[i];
                }
                else
                {
...
return SQLITE_NOMEM;
        ...
        private static int sqlite3MemCompare(Mem pMem1, Mem pMem2, CollSeq pColl)
        {
             ...
             ** are NULL, return 0.
             */
            if ((combined_flags & MEM_Null) != 0) return (f2 & MEM_Null) - (f1 & MEM_Null);
            ...
            {
                if ((f1 & (MEM_Int | MEM_Real)) == 0) return 1;
                if ((f2 & (MEM_Int | MEM_Real)) == 0) return -1;
                if ((f1 & f2 & MEM_Int) == 0)
                {
                    double r1, r2;
                    if ((f1 & MEM_Real) == 0)
        ...
        )
        {
            ...
            var available = 0; /* Number of bytes available on the local btree page */
            ...
            else if (SQLITE_OK == (rc = sqlite3VdbeMemGrow(pMem, amt + 2, 0)))
            {
                ...
                //pMem.z[amt] = 0;
                //pMem.z[amt+1] = 0;
                if (rc != SQLITE_OK) sqlite3VdbeMemRelease(pMem);
            }
        ...
        private static string sqlite3ValueText(sqlite3_value pVal, int enc)
        {
            ...
            if ((pVal.flags & MEM_Null) != 0) return null;
            ...
            if ((pVal.flags & MEM_Str) != 0)
            {
                ...
                if ((enc & SQLITE_UTF16_ALIGNED) != 0 && 1 == (1 & pVal.z[0])) //1==(1&SQLITE_PTR_TO_INT(pVal.z))
                {
                    ...
                    if (sqlite3VdbeMemMakeWriteable(pVal) != SQLITE_OK) return null;
                }
        ...
        {
            var p = new Mem(); //sqlite3DbMallocZero(db, sizeof(*p));
            if (p != null)
            {
                p.flags = MEM_Null;
                p.type = SQLITE_NULL;
        ...
        )
        {
            ...
            if (op == TK_STRING || op == TK_FLOAT || op == TK_INTEGER)
            {
                ...
                {
                    sqlite3VdbeMemSetInt64(pVal, pExpr.u.iValue);
                }
                else
                {
                ...
                if (enc != SQLITE_UTF8) sqlite3VdbeChangeEncoding(pVal, enc);
            }
