...
//#define PAGERTRACE(X)
         ...
         */
        private const int PAGER_UNLOCK = 0;
        private const int PAGER_SHARED = 1; /* same as SHARED_LOCK */
        private const int PAGER_RESERVED = 2; /* same as RESERVED_LOCK */
        private const int PAGER_EXCLUSIVE = 4; /* same as EXCLUSIVE_LOCK */
        private const int PAGER_SYNCED = 5;
...
if( (O = (char*)(P->xCodec(P->pCodec,D,N,X)))==0 ){ E; }
        ...
        private static void CODEC2(Pager P, byte[] D, uint N, int X, int E, ref byte[] O)
        {
            O = D;
        }
#endif
...
//# define PAGER_INCR(v)
         ...
         */
        private static readonly byte[] aJournalMagic =
        {
      ...
      return true;
        ...
        private static bool subjRequiresPage(PgHdr pPg)
        {
            ...
            for (i = 0; i < pPager.nSavepoint; i++)
            {
                ...
                if (p.nOrig >= pgno && 0 == sqlite3BitvecTest(p.pInSavepoint, pgno)) return true;
            }
        ...
        private static int read32bits(sqlite3_file fd, int offset, ref u32 pRes)
        {
            ...
            if (rc == SQLITE_OK) pRes = sqlite3Get4byte(ac);
            return rc;
        }
         ...
         */
        private static int osUnlock(sqlite3_file pFd, int eLock)
        {
            if (pFd.pMethods == null) return SQLITE_OK;
...
pPg.pageHash==pager_pagehash(pPg) );
        ...
        {
            int rc; /* Return code */
            var len = 0; /* Length in bytes of master journal name */
            var szJ = 0; /* Total size in bytes of journal file pJrnl */
            var cksum = 0; /* MJ checksum value read from journal */
            ...
            for (u = 0; u < len; u++) cksum -= zMaster[u];
            if (cksum != 0)
        ...
        private static i64 journalHdrOffset(Pager pPager)
        {
            ...
            if (c != 0)
                offset = (int)(((c - 1) / pPager.sectorSize + 1) *
                               pPager.sectorSize); //offset = ((c-1)/JOURNAL_HDR_SZ(pPager) + 1) * JOURNAL_HDR_SZ(pPager);
        ...
        private static int zeroJournalHdr(Pager pPager, int doTruncate)
        {
            ...
            if (pPager.journalOff != 0)
            {
                ...
                if (rc == SQLITE_OK && iLimit > 0)
                {
                    ...
                    if (rc == SQLITE_OK && sz > iLimit) rc = sqlite3OsTruncate(pPager.jfd, (int)iLimit);
                }
            }
        ...
        private static int writeJournalHdr(Pager pPager)
        {
            ...
            var nHeader = (u32)pPager.pageSize; /* Size of buffer pointed to by zHeader */
        ...
        )
        {
             ...
             */
            pPager.journalOff = journalHdrOffset(pPager);
            if (pPager.journalOff + JOURNAL_HDR_SZ(pPager) > journalSize) return SQLITE_DONE;
            ...
            if (isHot != 0 || iHdrOff != pPager.journalHdr)
            {
                ...
                if (rc != 0) return rc;
                if (memcmp(aMagic, aJournalMagic, aMagic.Length) != 0) return SQLITE_DONE;
            }
        ...
        private static int writeMasterJournal(Pager pPager, string zMaster)
        {
            ...
            var jrnlSize = 0; /* Size of journal file on disk */
            ...
            for (nMaster = 0; nMaster < zMaster.Length && zMaster[nMaster] != 0; nMaster++) cksum += zMaster[nMaster];

            /* If in full-sync mode, advance to the next disk sector before writing
             ** the master journal name. This is in case the previous page written to
             ** the journal has already been synced.
             */
            if (pPager.fullSync) pPager.journalOff = journalHdrOffset(pPager);
            iHdrOff = pPager.journalOff;
        ...
        private static void releaseAllSavepoints(Pager pPager)
        {
            ...
            for (ii = 0; ii < pPager.nSavepoint; ii++) sqlite3BitvecDestroy(ref pPager.aSavepoint[ii].pInSavepoint);
            if (!pPager.exclusiveMode || sqlite3IsMemJournal(pPager.sjfd)) sqlite3OsClose(pPager.sjfd);
...
0==MEMDB
        ...
        {
            var rc = SQLITE_OK; /* Error code from journal finalization operation */
            var rc2 = SQLITE_OK; /* Error code from db file unlock operation */
            if (pPager.state < PAGER_RESERVED) return SQLITE_OK;
            releaseAllSavepoints(pPager);
            ...
            if (isOpen(pPager.jfd))
            {
                ...
                else
                {
                    ...
                    sqlite3OsClose(pPager.jfd);
                    if (!pPager.tempFile) rc = sqlite3OsDelete(pPager.pVfs, pPager.zJournal, 0);
                }
#if SQLITE_CHECK_PAGES
...
0==MEMDB
        ...
        {
            var cksum = pPager.cksumInit; /* Checksum value to return */
            var i = pPager.pageSize - 200; /* Loop counter */
            while (i > 0)
            {
                cksum += aData[i];
                i -= 200;
        ...
        )
        {
            ...
            if (pgno > pPager.dbSize || sqlite3BitvecTest(pDone, pgno) != 0) return SQLITE_OK;
            if (isMainJrnl != 0)
            {
                rc = read32bits(jfd, pOffset - 4, ref cksum);
                if (rc != 0) return rc;
                if (0 == isSavepnt && pager_cksum(pPager, aData) != cksum) return SQLITE_DONE;
            }

            if (pDone != null && (rc = sqlite3BitvecSet(pDone, pgno)) != SQLITE_OK) return rc;
...
0==MEMDB
            ...
            {
                var ofst = (pgno - 1) * pPager.pageSize;
                rc = sqlite3OsWrite(pPager.fd, aData, pPager.pageSize, ofst);
                if (pgno > pPager.dbFileSize) pPager.dbFileSize = pgno;
...
CODEC1( pPager, aData, pgno, 0, rc = SQLITE_NOMEM );
            ...
            else if (0 == isMainJrnl && pPg == null)
            {
                ...
                if ((rc = sqlite3PagerAcquire(pPager, pgno, ref pPg, 1)) != SQLITE_OK) return rc;
...
CODEC1(pPager, pData, pPg.pgno, 3, rc = SQLITE_NOMEM);
        ...
        private static int pager_truncate(Pager pPager, u32 nPage)
        {
            ...
            if (pPager.state >= PAGER_EXCLUSIVE && isOpen(pPager.fd))
            {
                ...
                if (rc == SQLITE_OK && currentSize != newSize)
                {
                    ...
                    if (rc == SQLITE_OK) pPager.dbSize = nPage;
                }
            }
        ...
        private static void setSectorSize(Pager pPager)
        {
            ...
            if (pPager.sectorSize > MAX_SECTOR_SIZE) pPager.sectorSize = MAX_SECTOR_SIZE;
        }
        ...
        {
            var pVfs = pPager.pVfs;
            var szJ = 0; /* Size of the journal file in bytes */
            ...
            if (rc != SQLITE_OK || szJ == 0) goto end_playback;
            ...
            zMaster = null;
            if (rc != SQLITE_OK || res == 0) goto end_playback;
            pPager.journalOff = 0;
        ...
        private static int pagerPlaybackSavepoint(Pager pPager, PagerSavepoint pSavepoint)
        {
            ...
            if (pSavepoint != null)
            {
                ...
                if (null == pDone) return SQLITE_NOMEM;
            }
            ...
            {
                u32 ii; /* Loop counter */
                var offset = pSavepoint.iSubRec * (4 + pPager.pageSize);
            ...
            if (rc == SQLITE_OK) pPager.journalOff = (int)szJ;
            return rc;
        }
        ...
        private static void sqlite3PagerSetSafetyLevel(Pager pPager, int level, bool bFullFsync)
        {
...
(int)pPager->nReserve);
         ...
         */
        private static int sqlite3PagerSetPagesize(Pager pPager, ref u16 pPageSize, int nReserve)
        {
         ...
         */
        private static long sqlite3PagerMaxPageCount(Pager pPager, int mxPage)
        {
            if (mxPage > 0) pPager.mxPgno = (Pgno)mxPage;
            var idummy = 0;
...
//# define enable_simulated_io_errors()
         ...
         */
        private static int sqlite3PagerPagecount(Pager pPager, ref int pnPage)
        {
            ...
            else
            {
                ...
                var n = 0; /* File size in bytes returned by OsFileSize() */
             ...
             */
            if (nPage > pPager.mxPgno) pPager.mxPgno = (Pgno)nPage;
...
*/
    static Pgno sqlite3PagerPagenumber( DbPage pPg )
    {
      return pPg.pgno;
         ...
         */
        private static int syncJournal(Pager pPager)
        {
            if (pPager.needSync)
            {
         ...
         */
        private static int pager_write_pagelist(PgHdr pList)
        {
            Pager pPager; /* Pager object */
            int rc; /* Return code */
            ...
            while (rc == SQLITE_OK && pList)
            {
                ...
                {
                    var offset = (pList.pgno - 1) * pPager.pageSize; /* Offset to write */
                    byte[] pData = null; /* Data to write */

                    /* Encode the database */
                    CODEC2(pPager, pList.pData, pgno, 6, SQLITE_NOMEM,
                        ref pData); //     CODEC2(pPager, pList->pData, pgno, 6, return SQLITE_NOMEM, pData);
...
pList.pageHash = pager_pagehash(pList);
        ...
        private static int subjournalPage(PgHdr pPg)
        {
            ...
            {
                var pData = pPg.pData;
                var offset = pPager.nSubRec * (4 + pPager.pageSize);
                byte[] pData2 = null;

                CODEC2(pPager, pData, pPg.pgno, 7, SQLITE_NOMEM,
                    ref pData2); //CODEC2(pPager, pData, pPg.pgno, 7, return SQLITE_NOMEM, pData2);
                PAGERTRACE("STMT-JOURNAL %d page %d\n", PAGERID(pPager), pPg.pgno);
                Debug.Assert(pageInJournal(pPg) || pPg.pgno > pPager.dbOrigSize);
                rc = write32bits(pPager.sjfd, offset, pPg.pgno);
                if (rc == SQLITE_OK) rc = sqlite3OsWrite(pPager.sjfd, pData2, pPager.pageSize, offset + 4);
            }
        ...
        )
        {
            ...
            var nPathname = 0; /* Number of bytes in zPathname */
            var useJournal = (flags & PAGER_OMIT_JOURNAL) == 0; /* False to omit journal */
            var noReadlock = (flags & PAGER_NO_READLOCK) != 0; /* True to omit read-lock */
            var pcacheSize = sqlite3PcacheSize(); /* Bytes to allocate for PCache */
...
szPageDflt = ii;
            ...
            if (!useJournal)
                pPager.journalMode = PAGER_JOURNALMODE_OFF;
            else if (memDb != 0) pPager.journalMode = PAGER_JOURNALMODE_MEMORY;
        ...
        private static int hasHotJournal(Pager pPager, ref int pExists)
        {
            var pVfs = pPager.pVfs;
            int rc; /* Return code */
            var exists = 0; /* True if a journal file is present */
                ...
                if (rc == SQLITE_OK && locked == 0)
                {
                    ...
                    if (rc == SQLITE_OK)
                    {
                        ...
                        else
                        {
                             ...
                             ** it can be ignored.
                             */
                            var f = SQLITE_OPEN_READONLY | SQLITE_OPEN_MAIN_JOURNAL;
...
Debug.Assert(  pPager.state>=PAGER_SHARED && 0==MEMDB );
            ...
            iOffset = (pgno - 1) * pPager.pageSize;
...
CODEC1(pPager, pPg.pData, pPg.pgno, 3, rc = SQLITE_NOMEM);
      ...
      PAGER_INCR( ref pPager.nRead );
        ...
        {
            var rc = SQLITE_OK; /* Return code */
            var isErrorReset = false; /* True if recovering from error state */

            if (pPager.errCode != 0)
            {
                if (isOpen(pPager.jfd) || !string.IsNullOrEmpty(pPager.zJournal)) isErrorReset = true;
                pPager.errCode = SQLITE_OK;
                pager_reset(pPager);
            }
...
0==MEMDB
                ...
                if (pPager.pBackup != null || sqlite3PcachePagecount(pPager.pPCache) > 0)
                {
                    ...
                    for (var i = 0;
                         i < dbFileVers.Length;
                         i++) //if (memcmp(pPager.dbFileVers, dbFileVers, dbFileVers).Length != 0)
...
1==MEMDB
          ...
          rc =
 sqlite3BitvecSet( pPager.pInJournal, pgno );          //TESTONLY( rc = ) sqlite3BitvecSet(pPager.pInJournal, pgno);
#else
...
pPg.pageHash = pager_pagehash(pPg);
            ...
            pager_acquire_err:
            Debug.Assert(rc != SQLITE_OK);
            if (pPg != null) sqlite3PcacheDrop(pPg);
        ...
        {
            var rc = SQLITE_OK; /* Return code */
            var pVfs = pPager.pVfs; /* Local cache of vfs pointer */
            ...
            if (pPager.pInJournal == null) return SQLITE_NOMEM;
            ...
            if (!isOpen(pPager.jfd))
            {
                ...
                {
                    var flags = /* VFS flags to open journal file */
...
pVfs, pPager.zJournal, pPager.jfd, flags, jrnlBufferSize(pPager)
            ...
            if (rc == SQLITE_OK && pPager.nSavepoint != 0) rc = openSubJournal(pPager);
...
Debug.Assert( 0==MEMDB && !pPager.tempFile );
                 ...
                 */
                if (rc == SQLITE_OK && pPager.journalMode != PAGER_JOURNALMODE_OFF) rc = pager_open_journal(pPager);
            }
...
CHECK_PAGE(pPg);
            ...
            else
            {
                ...
                if (NEVER(rc != SQLITE_OK)) return rc;
            ...
            sqlite3_pager_writej_count.iValue = iValue;
                 ...
                 */
                if (subjRequiresPage(pPg)) rc = subjournalPage(pPg);
            }
        ...
        private static int sqlite3PagerWrite(DbPage pDbPage)
        {
            ...
            var pPg = pDbPage;
            var pPager = pPg.pPager;
            var nPagePerSector = (u32)(pPager.sectorSize / pPager.pageSize);

            if (nPagePerSector > 1)
            {
                var nPageCount = 0; /* Total number of pages in database file */
                u32 pg1; /* First page of the sector pPg is located on. */
                u32 nPage; /* Number of pages starting at pg1 to journal */
                int ii; /* Loop counter */
                var needSync = false; /* True if any page has PGHDR_NEED_SYNC */
...
0==MEMDB
                ...
                for (ii = 0; ii < nPage && rc == SQLITE_OK; ii++)
                {
                    ...
                    {
                        if ((pPage.flags & PGHDR_NEED_SYNC) != 0) needSync = true;
                        sqlite3PagerUnref(pPage);
                    }
                }
...
0==MEMDB
                    ...
                    {
                        var pPage = pager_lookup(pPager, (u32)(pg1 + ii));
                        if (pPage != null)
                        {
...
int DIRECT_MODE = isDirectMode;
            ...
            if (!pPager.changeCountDone && ALWAYS(pPager.dbSize > 0))
            {
                 ...
                 */
                if (!DIRECT_MODE && ALWAYS(rc == SQLITE_OK)) rc = sqlite3PagerWrite(pPgHdr);
                ...
                if (rc == SQLITE_OK)
                {
                    ...
                    if (DIRECT_MODE)
                    {
                        var zBuf = pPgHdr.pData;
                        Debug.Assert(pPager.dbFileSize > 0);
                        rc = sqlite3OsWrite(pPager.fd, zBuf, pPager.pageSize, 0);

                        if (rc == SQLITE_OK) pPager.changeCountDone = true;
                    }
                    else
                    {
...
rc = pager_incr_changecounter(pPager, 0);
                   ...
                   )
                {
                    ...
                    var iSkip = PAGER_MJ_PGNO(pPager); /* Pending lock page */
                    var dbSize = pPager.dbSize; /* Database image size */
                ...
                if (!pPager.noSync && !noSync) rc = sqlite3OsSync(pPager.fd, pPager.sync_flags);
                IOTRACE("DBSYNC %p\n", pPager);
      ...
      return MEMDB != 0;
        ...
        private static int sqlite3PagerOpenSavepoint(Pager pPager, int nSavepoint)
        {
            ...
            if (nSavepoint > nCurrent && pPager.useJournal != 0)
            {
                ...
                for (ii = nCurrent; ii < nSavepoint; ii++)
                {
                    ...
                    if (null == aNew[ii].pInSavepoint) return SQLITE_NOMEM;
                }
        ...
        private static int sqlite3PagerSavepoint(Pager pPager, int op, int iSavepoint)
        {
            ...
            if (iSavepoint < pPager.nSavepoint)
            {
                ...
                {
                    var pSavepoint = nNew == 0 ? null : pPager.aSavepoint[nNew - 1];
...
MEMDB != 0
        ...
        private static bool sqlite3PagerLockingMode(Pager pPager, int eMode)
        {
            ...
            if (eMode >= 0 && !pPager.tempFile) pPager.exclusiveMode = eMode != 0;
         ...
         */
        private static int sqlite3PagerJournalMode(Pager pPager, int eMode)
        {
...
0==MEMDB
            ...
            {
                if (isOpen(pPager.jfd)) sqlite3OsClose(pPager.jfd);
        ...
        {
            if (iLimit >= -1) pPager.journalSizeLimit = iLimit;
