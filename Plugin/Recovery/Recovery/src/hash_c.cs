...
namespace CS_SQLite3
{
    public partial class CSSQLite
    {
        ...
        private static u32 strHash(string z, int nKey)
        {
            ...
            while (nKey > 0)
            {
                h = (h << 3) ^ h ^ (_z < z.Length ? sqlite3UpperToLower[(byte)z[_z++]] : 0);
                nKey--;
            }
...
if( new_size==pH->htsize ) return false;
            ...
            for (var i = 0; i < new_size; i++) new_ht[i] = new _ht();
            ...
            {
                var h = strHash(elem.pKey, elem.nKey) % new_size;
                next_elem = elem.next;
        ...
        )
        {
            ...
            {
                if (elem.nKey == nKey && sqlite3StrNICmp(elem.pKey, pKey, nKey) == 0) return elem;
                elem = elem.next;
            }
         ...
         */
        private static object sqlite3HashFind(Hash pH, string pKey, int nKey)
        {
         ...
         */
        private static object sqlite3HashInsert(ref Hash pH, string pKey, int nKey, object data)
        {
