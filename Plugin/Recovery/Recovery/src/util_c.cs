...
rc = isnan(x);
         ...
         */
        private static void sqlite3ErrorClear(Parse pParse)
        {
        ...
        private static int[] UpperToLower;
        ...
        private static int sqlite3StrICmp(string zLeft, string zRight)
        {
        ...
        private static int sqlite3_strnicmp(string zLeft, int offsetLeft, string zRight, int N)
        {
        ...
        private static int sqlite3StrNICmp(string zLeft, int offsetLeft, string zRight, int N)
        {
            ...
            //b = (unsigned char *)zRight;
            int a = 0, b = 0;
            while (N-- > 0 && zLeft[a + offsetLeft] != 0 &&
        ...
        private static int sqlite3StrNICmp(string zLeft, string zRight, int N)
        {
            ...
            //b = (unsigned char *)zRight;
            int a = 0, b = 0;
            while (N-- > 0 && (zLeft[a] == zRight[b] || (zLeft[a] != 0 && zLeft[a] < 256 && zRight[b] < 256 &&
            ...
            if (N < 0) return 0;
            if (zLeft[a] < 256 && zRight[b] < 256) return UpperToLower[zLeft[a]] - UpperToLower[zRight[b]];
            return zLeft[a] - zRight[b];
        }
         ...
         */
        private static int sqlite3IsNumber(string z, ref int realnum, int enc)
        {
            if (string.IsNullOrEmpty(z)) return 0;
            var incr = enc == SQLITE_UTF8 ? 1 : 2;
            var zIndex = 0;
         ...
         */
        private static int sqlite3AtoF(string z, ref double pResult)
        {
...
return sqlite3Atoi64(z, pResult);
         ...
         */
        private static bool sqlite3Atoi64(string zNum, ref i64 pNum)
        {
            zNum = zNum.Trim() + " ";
            int i;
            for (i = 1; i < zNum.Length; i++)
                if (!sqlite3Isdigit(zNum[i]))
                    break;
            return long.TryParse(zNum.Substring(0, i), out pNum
            );
            //i64 v = 0;
            //int neg;
            //int i, c;
         ...
         */
        private static bool sqlite3FitsIn64Bits(string zNum, bool negFlag)
        {
            long pNum;
            Debug.Assert(zNum[0] >= '0' && zNum[0] <= '9'); /* zNum is an unsigned number */
            var result = negFlag ? long.TryParse("-" + zNum, out pNum) : long.TryParse(zNum, out pNum);
         ...
         */
        private static int getVarint(byte[] p, ref u32 v)
        {
            v = p[0];
            if (v <= 0x7F) return 1;
            u64 u64_v = 0;
        ...
        private static int getVarint(byte[] p, int offset, ref u32 v)
        {
            v = p[offset + 0];
            if (v <= 0x7F) return 1;
            u64 u64_v = 0;
        ...
        private static int getVarint(byte[] p, int offset, ref int v)
        {
            v = p[offset + 0];
            if (v <= 0x7F) return 1;
            u64 u64_v = 0;
        ...
        private static int getVarint(byte[] p, int offset, ref i64 v)
        {
            v = p[offset + 0];
            if (v <= 0x7F) return 1;
            u64 u64_v = 0;
        ...
        private static int getVarint(byte[] p, int offset, ref u64 v)
        {
            v = p[offset + 0];
            if (v <= 0x7F) return 1;
        ...
        private static int getVarint32(byte[] p, ref u32 v)
        {
        ...
        private static int getVarint32(string s, u32 offset, ref int v)
        {
            //(*B=*(A))<=0x7f?1:sqlite3GetVarint32(A,B))
            v = s[(int)offset];
            if (v <= 0x7F) return 1;
        ...
        private static int getVarint32(string s, u32 offset, ref u32 v)
        {
            //(*B=*(A))<=0x7f?1:sqlite3GetVarint32(A,B))
            v = s[(int)offset];
            if (v <= 0x7F) return 1;
        ...
        private static int getVarint32(byte[] p, u32 offset, ref u32 v)
        {
            //(*B=*(A))<=0x7f?1:sqlite3GetVarint32(A,B))
            v = p[offset];
            if (v <= 0x7F) return 1;
        ...
        private static int getVarint32(byte[] p, int offset, ref u32 v)
        {
            //(*B=*(A))<=0x7f?1:sqlite3GetVarint32(A,B))
            v = p[offset];
            if (v <= 0x7F) return 1;
        ...
        private static int getVarint32(byte[] p, int offset, ref int v)
        {
            //(*B=*(A))<=0x7f?1:sqlite3GetVarint32(A,B))
            v = p[offset + 0];
            if (v <= 0x7F) return 1;
        ...
        private static int sqlite3PutVarint(byte[] p, int offset, u64 v)
        {
            ...
            buf[0] &= 0x7f;
            Debug.Assert(n <= 9);
            for (i = 0, j = n - 1; j >= 0; j--, i++) p[offset + i] = buf[j];
            return n;
        }
         ...
         */
        private static u8 sqlite3GetVarint(byte[] p, int offset, ref u64 v)
        {
            u32 a, b, s;
         ...
         */
        private static u8 sqlite3GetVarint32(byte[] p, ref int v)
        {
            u32 u32_v = 0;
            var result = sqlite3GetVarint32(p, 0, ref u32_v);
            v = (int)u32_v;
            return result;
        }

        private static u8 sqlite3GetVarint32(byte[] p, int offset, ref int v)
        {
            u32 u32_v = 0;
            var result = sqlite3GetVarint32(p, offset, ref u32_v);
            v = (int)u32_v;
            return result;
        }

        private static u8 sqlite3GetVarint32(byte[] p, ref u32 v)
        {
            return sqlite3GetVarint32(p, 0, ref v);
        }

        private static u8 sqlite3GetVarint32(byte[] p, int offset, ref u32 v)
        {
            u32 a, b;
...
#else
        private static bool sqlite3SafetyOn(sqlite3 db)
        {
            return false;
        }
#endif
...
#else
        private static bool sqlite3SafetyOff(sqlite3 db)
        {
            return false;
        }
#endif
