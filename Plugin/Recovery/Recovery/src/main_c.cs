...
return rc;
            ...
            if (rc == SQLITE_OK)
            {
                ...
                if (sqlite3GlobalConfig.pInitMutex == null)
                {
                    ...
                    if (sqlite3GlobalConfig.bCoreMutex && sqlite3GlobalConfig.pInitMutex == null) rc = SQLITE_NOMEM;
                }
            }

            if (rc == SQLITE_OK) sqlite3GlobalConfig.nRefInitMutex++;
...
memset( pHash, 0, sizeof( sqlite3GlobalFunctions ) );
                ...
                var pHash = sqlite3GlobalFunctions;
#endif
      ...
      if ( rc == SQLITE_OK )
      {
         ...
         */
        private static int sqlite3_shutdown()
        {
        ...
        private static int sqlite3_config(int op, sqlite3_pcache_methods ap)
        {
            //  va_list ap;
            var rc = SQLITE_OK;
            switch (op)
        ...
        private static int sqlite3_config(int op, ref sqlite3_pcache_methods ap)
        {
            //  va_list ap;
            var rc = SQLITE_OK;
            switch (op)
            {
                ...
                {
                    if (sqlite3GlobalConfig.pcache.xInit == null) sqlite3PCacheSetDefault();
        ...
        private static int sqlite3_config(int op, sqlite3_mem_methods ap)
        {
            //  va_list ap;
            var rc = SQLITE_OK;
            switch (op)
        ...
        private static int sqlite3_config(int op, ref sqlite3_mem_methods ap)
        {
            //  va_list ap;
            var rc = SQLITE_OK;
            switch (op)
...
break;
        ...
        private static int sqlite3_db_config(sqlite3 db, int op, params object[] ap)
        {
            ...
            switch (op)
            {
                ...
                {
                    var pBuf = (byte[])va_arg(ap, "byte[]");
                    var sz = (int)va_arg(ap, "int");
                    var cnt = (int)va_arg(ap, "int");
        ...
        {
            var n = nKey1 < nKey2 ? nKey1 : nKey2;
            var r = sqlite3StrNICmp(pKey1, pKey2, nKey1 < nKey2 ? nKey1 : nKey2);
        ...
        public static int sqlite3_close(sqlite3 db)
        {
            ...
            for (i = db.aCollSeq.first; i != null; i = i.next)
            {
                ...
                var pColl = (CollSeq[])i.data; // sqliteHashData(i);
...
sqlite3HashClear(&db.aModule);
            ...
            if (db.pErr != null) sqlite3ValueFree(ref db.pErr);
        ...
        private static void sqlite3RollbackAll(sqlite3 db)
        {
            ...
            for (i = 0; i < db.nDb; i++)
                ...
                {
                    if (sqlite3BtreeIsInTrans(db.aDb[i].pBt)) inTrans = 1;
...
/* SQLITE_NOTADB      */ "file is encrypted or is not a database"
        ...
        )
        {
            ...
            u8[] delays = { 1, 2, 5, 10, 15, 20, 25, 25, 25, 50, 50, 100 };
            u8[] totals = { 0, 1, 3, 8, 18, 33, 53, 78, 103, 128, 178, 228 };
            //# define NDELAY (delays.Length/sizeof(delays[0]))
            var NDELAY = delays.Length;
            var db = (sqlite3)ptr;
            var timeout = db.busyTimeout;
            int delay, prior;
...
return rc;
        ...
        )
        {
            ...
            if (omitJournal) btFlags |= BTREE_OMIT_JOURNAL;
            if ((db.flags & SQLITE_NoReadlock) != 0) btFlags |= BTREE_NO_READLOCK;
#if !SQLITE_OMIT_MEMORYDB
            if (string.IsNullOrEmpty(zFilename) && sqlite3TempInMemory(db)) zFilename = ":memory:";
        ...
        {
            string z;
            if (db == null) return sqlite3ErrStr(SQLITE_NOMEM);
            if (!sqlite3SafetyCheckSickOrOk(db)) return sqlite3ErrStr(SQLITE_MISUSE);
            sqlite3_mutex_enter(db.mutex);
            ...
            //else
            {
                ...
                //Debug.Assert( 0 == db.mallocFailed );
                if (string.IsNullOrEmpty(z)) z = sqlite3ErrStr(db.errCode);
            }
            sqlite3_mutex_leave(db.mutex);
            return z;
        }
...
return z;
        ...
        {
            if (db != null && !sqlite3SafetyCheckSickOrOk(db)) return SQLITE_MISUSE;
            if (null == db /*|| db.mallocFailed != 0 */) return SQLITE_NOMEM;
        ...
        )
        {
            ...
            if (enc2 == SQLITE_UTF16 || enc2 == SQLITE_UTF16_ALIGNED) enc2 = SQLITE_UTF16NATIVE;
            if (enc2 < SQLITE_UTF8 || enc2 > SQLITE_UTF16BE) return SQLITE_MISUSE;
            ...
            if (pColl != null && pColl.xCmp != null)
            {
                ...
                {
                    var aColl = (CollSeq[])sqlite3HashFind(db.aCollSeq, zName, nName);
                    int j;
                    for (j = 0; j < 3; j++)
                    {
                        var p = aColl[j];
                        if (p.enc == pColl.enc)
                        {
                            if (p.xDel != null) p.xDel(ref p.pUser);
                            p.xCmp = null;
                        }
                    }
                }
         ...
         */
        private static readonly int[] aHardLimit =
        {
            SQLITE_MAX_LENGTH,
        ...
        private static int sqlite3_limit(sqlite3 db, int limitId, int newLimit)
        {
            ...
            if (newLimit >= 0)
            {
                if (newLimit > aHardLimit[limitId]) newLimit = aHardLimit[limitId];
...
sqlite3HashInit( ref db.aModule );
             ...
             */
            createCollation(db, "BINARY", SQLITE_UTF8, 0, binCollFunc, null);
            createCollation(db, "BINARY", SQLITE_UTF16BE, 0, binCollFunc, null);
            createCollation(db, "BINARY", SQLITE_UTF16LE, 0, binCollFunc, null);
            createCollation(db, "RTRIM", SQLITE_UTF8, 1, binCollFunc, null);
            //if ( db.mallocFailed != 0 )
            //{
            //  goto opendb_out;
            //}
            ...
            if (pColl != null) pColl.type = SQLITE_COLL_NOCASE;
            ...
            if (rc != SQLITE_OK)
            {
                if (rc == SQLITE_IOERR_NOMEM) rc = SQLITE_NOMEM;
            ...
            if (rc != SQLITE_OK) goto opendb_out;
        ...
        private static int sqlite3_file_control(sqlite3 db, string zDbName, int op, ref int pArg)
        {
            ...
            if (iDb < db.nDb)
            {
                ...
                if (pBtree != null)
                {
                    ...
                    if (fd.pMethods != null) rc = sqlite3OsFileControl(fd, (u32)op, ref pArg);
        ...
        private static int sqlite3_test_control(int op, params object[] ap)
        {
            ...
            switch (op)
            {
              ...
              TCLsqlite3PendingByte.iValue = sqlite3PendingByte;
                ...
                {
                    var db = (sqlite3)va_arg(ap, "sqlite3");
                    var x = (int)va_arg(ap, "int");
                    sqlite3_mutex_enter(db.mutex);
