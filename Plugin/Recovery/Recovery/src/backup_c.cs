...
namespace CS_SQLite3
{
    ...
    public partial class CSSQLite
    {
        ...
        {
            var i = sqlite3FindDbName(pDb, zDb);
        ...
        private static int backupOnePage(sqlite3_backup p, Pgno iSrcPg, byte[] zSrcData)
        {
             ...
             */
            if (nSrcPgsz != nDestPgsz && sqlite3PagerIsMemdb(sqlite3BtreePager(p.pDest))) rc = SQLITE_READONLY;

            /* This loop runs once for each destination page spanned by the source
             ** page. For each iteration, variable iOff is set to the byte offset
             ** of the destination page.
             */
            for (iOff = iEnd - nSrcPgsz; rc == SQLITE_OK && iOff < iEnd; iOff += nDestPgsz)
            {
                DbPage pDestPg = null;
                var iDest = (u32)(iOff / nDestPgsz) + 1;
        ...
        {
            var iCurrent = 0;
            var rc = sqlite3OsFileSize(pFile, ref iCurrent);
            if (rc == SQLITE_OK && iCurrent > iSize) rc = sqlite3OsTruncate(pFile, iSize);
            return rc;
        }
        ...
        public static int sqlite3_backup_step(sqlite3_backup p, int nPage)
        {
            ...
            if (p.pDestDb != null) sqlite3_mutex_enter(p.pDestDb.mutex);
            ...
            if (!isFatalError(rc))
            {
                ...
                var nSrcPage = -1; /* Size of source db in pages */
                 ...
                 */
                if (rc == SQLITE_OK) rc = sqlite3PagerPagecount(pSrcPager, ref nSrcPage);
                ...
                if (rc == SQLITE_OK)
                {
                    ...
                    if (p.iNext > (Pgno)nSrcPage)
                        rc = SQLITE_DONE;
                    else if (0 == p.isAttached) attachBackupObject(p);
                }
                   ...
                   )
                {
                    ...
                    int nDestTruncate;
                    if (p.pDestDb != null) sqlite3ResetInternalSchema(p.pDestDb, 0);
                    ...
                    if (nSrcPagesize < nDestPagesize)
                    {
                        ...
                        if (nDestTruncate == (int)PENDING_BYTE_PAGE(p.pDest.pBt)) nDestTruncate--;
                    }
                    else
                    {
                    ...
                    if (nSrcPagesize < nDestPagesize)
                    {
                         ...
                         */
                        var iSize = (u32)nSrcPagesize * (u32)nSrcPage;
                        var pFile = sqlite3PagerFile(pDestPager);

                        Debug.Assert(pFile != null);
                        Debug.Assert(nDestTruncate * (i64)nDestPagesize >= iSize || (
                           ...
                           )
                        {
                            ...
                            {
                                PgHdr pSrcPg = null;
                                var iSrcPg = (u32)(iOff / nSrcPagesize + 1);
                                rc = sqlite3PagerGet(pSrcPager, iSrcPg, ref pSrcPg);
                                if (rc == SQLITE_OK)
                                {
          ...
          Debug.Assert( rc2 == SQLITE_OK );
            ...
            if (p.pDestDb != null) sqlite3_mutex_leave(p.pDestDb.mutex);
        ...
        public static int sqlite3_backup_finish(sqlite3_backup p)
        {
            ...
            if (p.pDestDb != null) p.pSrc.nBackup--;
            if (p.isAttached != 0)
            {
                pp = sqlite3PagerBackupPtr(sqlite3BtreePager(p.pSrc));
        ...
        private static void sqlite3BackupUpdate(sqlite3_backup pBackup, Pgno iPage, byte[] aData)
        {
            ...
            for (p = pBackup; p != null; p = p.pNext)
            {
                ...
                if (!isFatalError(p.rc) && iPage < p.iNext)
                {
                     ...
                     */
                    var rc = backupOnePage(p, iPage, aData);
        ...
        private static int sqlite3BtreeCopyFile(Btree pTo, Btree pFrom)
        {
            ...
            if (rc == SQLITE_OK) pTo.pBt.pageSizeFixed = false;
