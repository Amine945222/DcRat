...
namespace CS_SQLite3
{
    ...
    NULL pointers replaced by SQL NULL.  %Q */
        ...
        public class et_info
        {
            ...
            public byte flags; /* One or more of FLAG_ constants below */ /* Information about each format field */
         ...
         */
        private static readonly string aDigits = "0123456789ABCDEF0123456789abcdef";
        private static readonly string aPrefix = "-x0\000X0";
        private static readonly et_info[] fmtinfo =
        {
...
# else
        ...
        )
        {
            ...
            length = 0;
            bufpt = 0;
            var _fmt = 0; // Work around string pointer
            fmt += '\0';
            ...
            for (; _fmt <= fmt.Length && (c = fmt[_fmt]) != 0; ++_fmt)
            {
                ...
                if (width > etBUFSIZE - 10) width = etBUFSIZE - 12;
                /* Get the precision */
                if (c == '.')
                {
                    precision = 0;
                ...
                switch (xtype)
                {
                    ...
                    case etRADIX:
                        ...
                        if (xtype == etORDINAL)
                        {
                            var zOrd = "thstndrd".ToCharArray();
                            var x = (int)(longvalue % 10);
...

                    {
                        ...
                        switch (infop._base)
                        {
                            case 16:
                                ...
                                break;
                            case 8:
                                _buf = Convert.ToString(longvalue, 8).ToCharArray();
                                break;
                            default:
                            {
                                if (flag_zeropad)
                        ...
                        for (idx = precision - length; idx > 0; idx--) buf[--bufpt] = '0'; /* Zero pad */
...
for(idx = precision, rounder = 0.4999; idx>0; idx--, rounder *= 0.1);
                        ...
                        if (double.IsNaN(realvalue) ||
                            !double.TryParse(Convert.ToString(realvalue),
                                out d)) //if( sqlite3IsNaN((double)realvalue) )
                        {
                            buf = "NaN".ToCharArray();
                            length = 3;
                            break;
                        ...
                        {
                            int i;
                            var nPad = width - length;
                            for (i = width; i >= nPad; i--) buf[bufpt + i] = buf[bufpt + i - nPad];
                            i = prefix != '\0' ? 1 : 0;
                    ...
                    case etDYNSTRING:
                        bufpt = 0; //
                        var bufStr = (string)va_arg(ap, "string");
                    ...
                    case etSQLESCAPE3:
                    {
                        ...
                        bool needQuote;
                        char ch;
                        var q = xtype == etSQLESCAPE3 ? '"' : '\''; /* Quote character */
                        var escarg = (string)va_arg(ap, "char*") + '\0';
                    ...
                    case etTOKEN:
                    {
                        var pToken = (Token)va_arg(ap, "Token");
                        if (pToken != null) sqlite3StrAccumAppend(pAccum, pToken.z, pToken.n);
                        length = width = 0;
                        break;
                    }
                    case etSRCLIST:
                    {
                        var pSrc = (SrcList)va_arg(ap, "SrcList");
                        var k = (int)va_arg(ap, "int");
                        var pItem = pSrc.a[k];
                ...
                if (!flag_leftjustify)
                {
                    ...
                    if (nspace > 0) appendSpace(pAccum, nspace);
                }

                if (length > 0) sqlite3StrAccumAppend(pAccum, new string(buf, bufpt, length), length);
                if (flag_leftjustify)
                {
                    int nspace;
                    nspace = width - length;
        ...
        private static string sqlite3VMPrintf(sqlite3 db, string zFormat, params va_list[] ap)
        {
            ...
            string z;
            var zBase = new StringBuilder(SQLITE_PRINT_BUF_SIZE);
            var acc = new StrAccum();
