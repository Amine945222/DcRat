...
namespace CS_SQLite3
{
    public partial class CSSQLite
    {
         ...
         ** collating sequences.
         */
        private static Expr sqlite3ExprSetColl(Parse pParse, Expr pExpr, Token pCollName)
        {
        ...
        private static CollSeq sqlite3ExprCollSeq(Parse pParse, Expr pExpr)
        {
            ...
            while (ALWAYS(p != null))
            {
                ...
                if (op != TK_CAST && op != TK_UPLUS) break;
                p = p.pLeft;
            }

            if (sqlite3CheckCollSeq(pParse, pColl) != 0) pColl = null;
            return pColl;
        }
        ...
        private static char comparisonAffinity(Expr pExpr)
        {
            ...
            else if (aff == '\0') aff = SQLITE_AFF_NONE;
            return aff;
        }
        ...
        )
        {
            ...
            else
            {
                ...
                if (pColl == null) pColl = sqlite3ExprCollSeq(pParse, pRight);
            }
         ...
         ** affinity change.
         */
        private static void codeCompareOperands(
        ...
        {
            var rc = SQLITE_OK;
            var mxHeight = pParse.db.aLimit[SQLITE_LIMIT_EXPR_DEPTH];
            if (nHeight > mxHeight)
            {
        ...
        private static void heightOfExprList(ExprList p, ref int pnHeight)
        {
            ...
            {
                int i;
                for (i = 0; i < p.nExpr; i++) heightOfExpr(p.a[i].pExpr, ref pnHeight);
            }
        }
        ...
        )
        {
            ...
            if (pToken != null)
                if (op != TK_INTEGER || pToken.z == null || pToken.z.Length == 0
                    || sqlite3GetInt32(pToken.z, ref iValue) == false)
                    nExtra = pToken.n + 1;
...
sqlite3Dequote(ref pNew.u._zToken);
        ...
        {
            var p = sqlite3ExprAlloc(pParse.db, op, pToken, 1);
         ...
         */
        private static Expr sqlite3RegisterExpr(Parse pParse, Token pToken)
        {
            var v = pParse.pVdbe;
            Expr p;
            if (pParse.nested == 0)
        ...
        private static Expr sqlite3ExprAnd(sqlite3 db, Expr pLeft, Expr pRight)
        {
            ...
            var pNew = sqlite3ExprAlloc(db, TK_AND, null, 0);
         ...
         */
        private static void sqlite3ExprClear(sqlite3 db, Expr p)
        {
            Debug.Assert(p != null);
...
//sqlite3DbFree( db, ref p.u._zToken );
        ...
        private static int dupedExprNodeSize(Expr p, int flags)
        {
            ...
            if (!ExprHasProperty(p, EP_IntValue) && p.u.zToken != null) nByte += sqlite3Strlen30(p.u.zToken) + 1;
            return ROUND8(nByte);
        }
        ...
        private static Expr exprDup(sqlite3 db, Expr p, int flags, ref Expr pzBuffer)
        {
            ...
            if (p != null)
            {
                var isReduced = (flags & EXPRDUP_REDUCE) != 0;
                var zAlloc = new Expr();
                u32 staticFlag = 0;
                ...
                if (pNew != null)
                {
                     ...
                     */
                    var nStructSize = dupedExprStructSize(p, flags);
                    var nNewSize = nStructSize & 0xfff;
                    int nToken;
                    ...
                    if (ExprHasAnyProperty(pNew, EP_Reduced | EP_TokenOnly))
                    {
                        ...
                        if (pzBuffer != null) pzBuffer = zAlloc;
                    }
                    else
                    {
                        pNew.flags2 = 0;
...
return null;
        ...
        )
        {
            ...
            if (pList != null)
            {
                var pItem = pList.a[pList.nExpr - 1];
                Debug.Assert(pList.nExpr > 0);
        ...
        {
            var mx = pParse.db.aLimit[SQLITE_LIMIT_COLUMN];
            testcase(pEList != null && pEList.nExpr == mx);
            testcase(pEList != null && pEList.nExpr == mx + 1);
            if (pEList != null && pEList.nExpr > mx) sqlite3ErrorMsg(pParse, "too many columns in %s", zObject);
        }
        ...
        private static int sqlite3FindInIndex(Parse pParse, Expr pX, ref int prNotFound)
        {
            ...
            var eType = 0; /* Type of RHS table. IN_INDEX_* */
            var iTab = pParse.nTab++; /* Cursor of the RHS table */
            var mustBeUnique = prNotFound != 0; /* True if RHS must be unique */
            ...
            if (ALWAYS(pParse.nErr == 0) && isCandidateForInOpt(p) != 0)
            {
                ...
                var pExpr = p.pEList.a[0].pExpr; /* Expression <column> */
                int iCol = pExpr.iColumn; /* Index of column <column> */
                var v = sqlite3GetVdbe(pParse); /* Virtual machine being coded */
                var pTab = p.pSrc.a[0].pTab; /* Table <table>. */
                ...
                else
                {
                    ...
                    var pReq = sqlite3BinaryCompareCollSeq(pParse, pX.pLeft, pExpr);

                    /* Check that the affinity that will be used to perform the
                     ** comparison is the same as the affinity of the column. If
                     ** it is not, it is not possible to use any index.
                     */
                    var aff = comparisonAffinity(pX);
                    var affinity_ok = pTab.aCol[iCol].affinity == aff || aff == SQLITE_AFF_NONE;
            ...
            if (eType == 0)
            {
                ...
                else if (pX.pLeft.iColumn < 0 && !ExprHasAnyProperty(pX, EP_xIsSelect)) eType = IN_INDEX_ROWID;
         ...
         */
#if !SQLITE_OMIT_SUBQUERY
        private static void sqlite3CodeSubselect(
            Parse pParse, /* Parsing context */
        ...
        {
            var testAddr = 0; /* One-time test address */
            ...
            switch (pExpr.op)
            {
                case TK_IN:
                {
                    ...
                    if (rMayHaveNull != 0) sqlite3VdbeAddOp2(v, OP_Null, 0, rMayHaveNull);
                    ...
                    if (ExprHasProperty(pExpr, EP_xIsSelect))
                    {
                        ...
                        if (sqlite3Select(pParse, pExpr.x.pSelect, ref dest) != 0) return;
                    ...
                    else if (pExpr.x.pList != null)
                    {
                        ...
                        if (affinity == '\0') affinity = SQLITE_AFF_NONE;
                    ...
                    if (!isRowid) sqlite3VdbeChangeP4(v, addr, keyInfo, P4_KEYINFO);
                    break;
                }
              ...
              VdbeComment( v, "Init EXISTS result" );
                    ...
                    if (sqlite3Select(pParse, pSel, ref dest) != 0) return;
            ...
            if (testAddr != 0) sqlite3VdbeJumpHere(v, testAddr - 1);
         ...
         ** like the continuation of the number.
         */
        private static void codeInteger(Vdbe v, Expr pExpr, bool negFlag, int iMem)
        {
        ...
        private static void cacheEntryClear(Parse pParse, yColCache p)
        {
            if (p.tempReg != 0)
            {
                if (pParse.nTempReg < ArraySize(pParse.aTempReg)) pParse.aTempReg[pParse.nTempReg++] = p.iReg;
                p.tempReg = 0;
            }
        }
         ...
         */
        private static void sqlite3ExprCacheRemove(Parse pParse, int iReg)
        {
            int i;
            yColCache p;
            for (i = 0; i < SQLITE_N_COLCACHE; i++) //p=pParse.aColCache... p++)
        ...
        )
        {
            ...
            {
                var op = IsVirtual(pTab) ? OP_VColumn : OP_Column;
        ...
        private static void sqlite3ExprCacheAffinityChange(Parse pParse, int iStart, int iCount)
        {
            ...
            for (i = 0; i < SQLITE_N_COLCACHE; i++) // p=pParse.aColCache... p++)
            {
                p = pParse.aColCache[i];
                var r = p.iReg;
                if (r >= iStart && r <= iEnd) p.affChange = true;
            }
        }
        ...
        private static void sqlite3ExprCodeMove(Parse pParse, int iFrom, int iTo, int nReg)
        {
            ...
            for (i = 0; i < SQLITE_N_COLCACHE; i++) // p=pParse.aColCache... p++)
            {
                p = pParse.aColCache[i];
                var x = p.iReg;
                if (x >= iFrom && x < iFrom + nReg) p.iReg += iTo - iFrom;
            }
        }
        ...
        private static void sqlite3ExprCodeCopy(Parse pParse, int iFrom, int iTo, int nReg)
        {
            int i;
            if (NEVER(iFrom == iTo)) return;
            for (i = 0; i < nReg; i++) sqlite3VdbeAddOp2(pParse.pVdbe, OP_Copy, iFrom + i, iTo + i);
        }
         ...
         */
        private static void sqlite3ExprHardCopy(Parse pParse, int iReg, int nReg)
        {
            VdbeOp pOp;
            Vdbe v;
            ...
            if (pOp.opcode == OP_SCopy && pOp.p1 >= iReg && pOp.p1 < iReg + nReg) pOp.opcode = OP_Copy;
        }
...
return iReg;
        ...
        private static int sqlite3ExprCodeTarget(Parse pParse, Expr pExpr, int target)
        {
            ...
            var inReg = target; /* Results stored in register inReg */
            var regFree1 = 0; /* If non-zero free this temporary register */
            var regFree2 = 0; /* If non-zero free this temporary register */
            ...
            switch (op)
            {
                case TK_AGG_COLUMN:
                {
                    var pAggInfo = pExpr.pAggInfo;
                    var pCol = pAggInfo.aCol[pExpr.iAgg];
                ...
                case TK_VARIABLE:
                {
                    ...
                    else
                    {
                        ...
                        if (pExpr.u.zToken.Length > 1) sqlite3VdbeChangeP4(v, -1, pExpr.u.zToken, 0);
                    }
                ...
                case TK_FUNCTION:
                {
                    ...
                    var constMask = 0; /* Mask of function arguments that are constant */
                    int i; /* Loop counter */
                    var enc = ENC(db); /* The text encoding used by this database */
...
pDef = sqlite3VtabOverloadFunction( db, pDef, nFarg, pFarg.a[0].pExpr );
                    ...
                    for (i = 0; i < nFarg; i++)
                    {
                        if (i < 32 && sqlite3ExprIsConstant(pFarg.a[i].pExpr) != 0) constMask |= 1 << i;
                    ...
                    if (nFarg != 0) sqlite3ReleaseTempRange(pParse, r1, nFarg);
                ...
                case TK_IN:
                {
                    ...
                    if (rMayHaveNull != 0) rNotFound = ++pParse.nMem;
                ...
                {
                    var pLeft = pExpr.pLeft;
                    var pLItem = pExpr.x.pList.a[0];
                    var pRight = pLItem.pExpr;
        ...
        private static int sqlite3ExprCodeTemp(Parse pParse, Expr pExpr, ref int pReg)
        {
            ...
            var r2 = sqlite3ExprCodeTarget(pParse, pExpr, r1);
            if (r2 == r1)
            {
                pReg = r1;
            }
        ...
        private static int sqlite3ExprCode(Parse pParse, Expr pExpr, int target)
        {
            ...
            if (inReg != target && pParse.pVdbe != null) sqlite3VdbeAddOp2(pParse.pVdbe, OP_SCopy, inReg, target);
            return target;
        }
        ...
        private static int isAppropriateForFactoring(Expr p)
        {
            if (sqlite3ExprIsConstantNotJoin(p) ==
                0) return 0; /* Only constant expressions are appropriate for factoring */
            if ((p.flags & EP_FixedDest) == 0) return 1; /* Any constant without a fixed destination is appropriate */
            ...
            switch (p.op)
            {
                ...
                case TK_UMINUS:
                {
                    if (p.pLeft.op == TK_FLOAT || p.pLeft.op == TK_INTEGER) return 0;
                    break;
                }
                default:
                {
        ...
        )
        {
            ...
            for (i = 0; i < n; i++) // pItem++)
            {
                ...
                if (pItem.iAlias != 0)
                {
                    var iReg = codeAlias(pParse, pItem.iAlias, pItem.pExpr, target + i);
                    var v = sqlite3GetVdbe(pParse);
                    if (iReg != target + i) sqlite3VdbeAddOp2(v, OP_SCopy, iReg, target + i);
                }
                else
                {
                ...
                if (doHardCopy /* && 0 == pParse.db.mallocFailed */) sqlite3ExprHardCopy(pParse, target, n);
            }
        ...
        private static void sqlite3ExprIfTrue(Parse pParse, Expr pExpr, int dest, int jumpIfNull)
        {
            ...
            switch (op)
            {
                ...
                case TK_BETWEEN:
                {
                     ...
                     ** elementation of x.
                     */
                    var exprAnd = new Expr();
                    var compLeft = new Expr();
                    var compRight = new Expr();
                    var exprX = new Expr();
         ...
         */
        private static bool sqlite3ExprCompare(Expr pA, Expr pB)
        {
            int i;
            ...
            if (ExprHasProperty(pA, EP_xIsSelect) || ExprHasProperty(pB, EP_xIsSelect)) return false;
            ...
            if (pA.x.pList != null && pB.x.pList != null)
            {
                ...
                {
                    var pExprA = pA.x.pList.a[i].pExpr;
                    var pExprB = pB.x.pList.a[i].pExpr;
            ...
            {
                if (!ExprHasProperty(pB, EP_IntValue) || pA.u.iValue != pB.u.iValue) return false;
            }
            else if (pA.op != TK_COLUMN && pA.u.zToken != null)
            {
                if (ExprHasProperty(pB, EP_IntValue) || NEVER(pB.u.zToken == null)) return false;
                if (sqlite3StrICmp(pA.u.zToken, pB.u.zToken) != 0) return false;
            }
        ...
        private static int analyzeAggregate(Walker pWalker, ref Expr pExpr)
        {
            ...
            var pSrcList = pNC.pSrcList;
            var pAggInfo = pNC.pAggInfo;
                        ...
                        for (i = 0; i < pSrcList.nSrc; i++)
                        {
                            ...
                            if (pExpr.iTable == pItem.iCursor)
                            {
                                   ...
                                   )
                                {
                                    ...
                                    if (pCol.iSorterColumn < 0) pCol.iSorterColumn = pAggInfo.nSortingColumn++;
                                }
                ...
                case TK_AGG_FUNCTION:
                {
                    ...
                    if (pNC.nDepth == 0)
                    {
                        ...
                        for (i = 0; i < pAggInfo.nFunc; i++)
                        {
                            ...
                            if (sqlite3ExprCompare(pItem.pExpr, pExpr)) break;
                        }
                        ...
                        if (i >= pAggInfo.nFunc)
                        {
                            ...
                            */
                            var enc = pParse.db.aDbStatic[0].pSchema.enc; // ENC(pParse.db);
        ...
        {
            if (pParse.nTempReg == 0) return ++pParse.nMem;
