...
namespace CS_SQLite3
{
    public partial class CSSQLite
    {
        ...
        private const int BITVEC_SZ = 512;

        /* Round the union size down to the nearest pointer boundary, since that's how
         ** it will be aligned within the Bitvec struct. */
        //#define BITVEC_USIZE     (((BITVEC_SZ-(3*sizeof(u32)))/sizeof(Bitvec*))*sizeof(Bitvec*))
        private const int BITVEC_USIZE = (BITVEC_SZ - 3 * sizeof(u32)) / 4 * 4;
        ...
        private const int BITVEC_NELEM = BITVEC_USIZE / sizeof(BITVEC_TELEM);

        /* Number of bits in the bitmap array. */
        //#define BITVEC_NBIT      (BITVEC_NELEM*BITVEC_SZELEM)
        private const int BITVEC_NBIT = BITVEC_NELEM * BITVEC_SZELEM;

        /* Number of u32 values in hash table. */
        //#define BITVEC_NINT      (BITVEC_USIZE/sizeof(u32))
        private const int BITVEC_NINT = BITVEC_USIZE / sizeof(u32);

        /* Maximum number of entries in hash table before
         ** sub-dividing and re-hashing. */
        //#define BITVEC_MXHASH    (BITVEC_NINT/2)
        private const int BITVEC_MXHASH = BITVEC_NINT / 2;

        private const int BITVEC_NPTR = BITVEC_USIZE / 4; //sizeof(Bitvec *));
...
/* make our hash too "full".  */
            ...
            if (p.nSet >= BITVEC_MXHASH)
            {
                ...
                if (aiValues == null) return SQLITE_NOMEM;
