      ...
      private class LockingStrategy
        {
            ...
            private static extern bool LockFileEx(HANDLE hFile, uint dwFlags, uint dwReserved,
...
BOOL bExclusive;    /* Indicates an exclusive lock has been obtained */
        ...
        private static readonly LockingStrategy lockingStrategy = HelperMethods.IsRunningMediumTrust()
...
return SQLITE_OK;
         ...
         */
        private static int sqlite3_fileSize(sqlite3_file id, ref int pSize)
        {
            //DWORD upperBits;
            //DWORD lowerBits;
            //  sqlite3_file pFile = (sqlite3_file)id;
      ...
      //SimulateIOError(return SQLITE_IOERR_FSTAT);  TODO --  How to implement this?
        ...
        {
            var res = 0;
            if (isNT()) res = lockingStrategy.SharedLockFile(pFile, SHARED_FIRST, SHARED_SIZE);
...
#endif
            //}
            if (res == 0) pFile.lastErrno = (u32)Marshal.GetLastWin32Error();
            return res;
        }
        ...
        {
            var rc = SQLITE_OK; /* Return code from subroutines */
      ...
      pFile.fs.GetHashCode(), locktype, pFile.locktype, pFile.sharedLockByte );
            ...
            */
            ...
            OSTRACE2( "could not get a PENDING lock. cnt=%d\n", cnt );
                ...
                if (0 == res) error = (u32)Marshal.GetLastWin32Error();
            }
        ...
        OSTRACE2( "unreadlock = %d\n", res );
          ...
          OSTRACE2( "error-code = %d\n", error );
             ...
             ** release it now.
             */
            if (gotPendingLock && locktype == SHARED_LOCK) lockingStrategy.UnlockFile(pFile, PENDING_BYTE, 1);
      ...
      pFile.locktype, pFile.sharedLockByte );
         ...
         */
        private static int winFileControl(sqlite3_file id, int op, ref int pArg)
        {
            switch (op)
            {
         ...
         */
        private static readonly sqlite3_io_methods winIoMethod = new sqlite3_io_methods(
            1, /* iVersion */
            winClose,
            winRead,
            winWrite,
        ...
        private static int getTempname(int nBuf, StringBuilder zBuf)
        {
            ...
            for (var i = 0; i < 20; i++)
            {
                ...
                zRandom.Append(zChars[(int)(iRandom % (zChars.Length - 1))]);
            }

            //  zBuf[j] = 0;
            zBuf.Append(Path.GetTempPath() + SQLITE_TEMP_FILE_PREFIX + zRandom);
...
int isTemp = 0;
            ...
            var zUtf8Name = zName; /* Filename in UTF-8 encoding */
            ...
            {
                var rc = getTempname(MAX_PATH + 1, zTmpname);
            ...
            if (string.IsNullOrEmpty(zConverted)) return SQLITE_NOMEM;
...
pFile.zDeleteOnClose = zConverted;
      ...
      OpenCounter( +1 );
         ...
         */
        private static readonly int MX_DELETION_ATTEMPTS = 5;
        ...
        )
        {
            ...
            var zConverted = convertUtf8Filename(zFilename);
            if (zConverted == null || zConverted == "") return SQLITE_NOMEM;
#if SQLITE_TEST
      ...
      OSTRACE2( "DELETE \"%s\"\n", zFilename );
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    var zTmpname = new StringBuilder(255); /* Buffer used to create temp filename */
                    ...
                    try
                    {
                        var fs = File.Create(zTempFilename, 1, FileOptions.DeleteOnClose);
                        fs.Close();
...
return SQLITE_OK;
        ...
        {
            var bytesPerSector = SQLITE_DEFAULT_SECTOR_SIZE;
        ...
        private static int winDlError(sqlite3_vfs vfs, int nByte, ref string zErrMsg)
        {
            return 0;
        }
...
static int winDlClose(ref sqlite3_vfs vfs, object data) { return 0; }
        ...
        private static int winRandomness(sqlite3_vfs pVfs, int nBuf, ref byte[] zBuf)
        {
            var n = 0;
            UNUSED_PARAMETER(pVfs);
#if (SQLITE_TEST)
...
#else
            var sBuf = BitConverter.GetBytes(System.DateTime.Now.Ticks);
            zBuf[0] = sBuf[0];
            zBuf[1] = sBuf[1];
            zBuf[2] = sBuf[2];
            zBuf[3] = sBuf[3];
    ...
    //    static int sqlite3_current_time = 0;
        ...
        private static int winCurrentTime(sqlite3_vfs pVfs, ref double prNow)
        {
            ...
            */
            ...
            prNow = timeW + timeF / (double)ntuPerDay;
#if SQLITE_TEST
      if ( ( sqlite3_current_time.iValue ) != 0 )
      {
        ...
        private static readonly sqlite3_vfs winVfs = new sqlite3_vfs(
            1, /* iVersion */
