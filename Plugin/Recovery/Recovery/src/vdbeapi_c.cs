...
sqlite3_mutex mutex = v.db.mutex;
         ...
         */
        private static int sqlite3_clear_bindings(sqlite3_stmt pStmt)
        {
            int i;
            var rc = SQLITE_OK;
            var p = pStmt;
            ...
            if ((p.flags & (MEM_Blob | MEM_Str)) != 0)
            {
                ...
                if (p.zBLOB == null && p.z != null)
                {
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < p.zBLOB.Length; i++) p.zBLOB[i] = (u8)p.z[i];
                    }
...
return sqlite3ValueText(pVal, SQLITE_UTF16LE);
        ...
        {
            if (sqlite3VdbeMemSetStr(pCtx.s, z, n, enc, xDel) == SQLITE_TOOBIG) sqlite3_result_error_toobig(pCtx);
        }
...
//}
#endif
        private static void sqlite3_result_int(sqlite3_context pCtx, int iVal)
        {
            Debug.Assert(sqlite3_mutex_held(pCtx.s.db.mutex));
            sqlite3VdbeMemSetInt64(pCtx.s, iVal);
        }

        private static void sqlite3_result_int64(sqlite3_context pCtx, i64 iVal)
        {
            Debug.Assert(sqlite3_mutex_held(pCtx.s.db.mutex));
            sqlite3VdbeMemSetInt64(pCtx.s, iVal);
        }

        private static void sqlite3_result_null(sqlite3_context pCtx)
        {
            Debug.Assert(sqlite3_mutex_held(pCtx.s.db.mutex));
...
sqlite3VdbeMemSetStr(pCtx.s, z, n, SQLITE_UTF16LE, xDel);
...
#endif // * SQLITE_OMIT_UTF16 */
        private static void sqlite3_result_value(sqlite3_context pCtx, sqlite3_value pValue)
        {
            Debug.Assert(sqlite3_mutex_held(pCtx.s.db.mutex));
            sqlite3VdbeMemCopy(pCtx.s, pValue);
        }

        private static void sqlite3_result_zeroblob(sqlite3_context pCtx, int n)
        {
            Debug.Assert(sqlite3_mutex_held(pCtx.s.db.mutex));
            sqlite3VdbeMemSetZeroBlob(pCtx.s, n);
        }

        private static void sqlite3_result_error_code(sqlite3_context pCtx, int errCode)
        {
        ...
        private static void sqlite3_result_error_toobig(sqlite3_context pCtx)
        {
            Debug.Assert(sqlite3_mutex_held(pCtx.s.db.mutex));
            pCtx.isError = SQLITE_ERROR;
        ...
        private static void sqlite3_result_error_nomem(sqlite3_context pCtx)
        {
            Debug.Assert(sqlite3_mutex_held(pCtx.s.db.mutex));
            sqlite3VdbeMemSetNull(pCtx.s);
        ...
        private static int sqlite3Step(Vdbe p)
        {
            ...
            if (p.pc < 0)
            {
                 ...
                 */
                if (db.activeVdbeCnt == 0) db.u1.isInterrupted = false;
            ...
            if (sqlite3SafetyOff(db)) rc = SQLITE_MISUSE;
      ...
      */
            ...
            db.errCode = rc;
            if (SQLITE_NOMEM == sqlite3ApiExit(p.db, p.rc)) p.rc = SQLITE_NOMEM;
            end_of_step:
        ...
        public static int sqlite3_step(sqlite3_stmt pStmt)
        {
            ...
            if (pStmt != null)
            {
                ...
                if (rc == SQLITE_SCHEMA && ALWAYS(v.isPrepareV2) && ALWAYS(db.pErr != null))
                {
                     ...
                     */
                    var zErr = sqlite3_value_text(db.pErr);
         ...
         ** pointer to it.
         */
        private static object sqlite3_user_data(sqlite3_context p)
        {
         ...
         ** pointer to it.
         */
        private static sqlite3 sqlite3_context_db_handle(sqlite3_context p)
        {
            Debug.Assert(p != null && p.pFunc != null);
            return p.s.db;
        }
         ...
         */
        private static void sqlite3InvalidFunction(
            sqlite3_context context, /* The function calling context */
            int NotUsed, /* Number of arguments to the function */
            sqlite3_value[] NotUsed2 /* Value of each argument */
        )
        {
            var zName = context.pFunc.zName;
            string zErr;
         ...
         */
        private static string sqlite3_get_auxdata(sqlite3_context pCtx, int iArg)
        {
            VdbeFunc pVdbeFunc;

            Debug.Assert(sqlite3_mutex_held(pCtx.s.db.mutex));
            pVdbeFunc = pCtx.pVdbeFunc;
            if (null == pVdbeFunc || iArg >= pVdbeFunc.nAux || iArg < 0) return null;
         ...
         */
        private static void sqlite3_set_auxdata(
            sqlite3_context pCtx,
            int iArg,
            string pAux,
        ...
        )
        {
            ...
            {
                var nAux = pVdbeFunc != null ? pVdbeFunc.nAux : 0;
                var nMalloc = iArg;
            ...
            if (pAuxData.pAux != null && pAuxData.xDelete != null) pAuxData.xDelete(ref pAuxData.pAux);
            pAuxData.pAux = pAux;
            pAuxData.xDelete = xDelete;
            return;

            failed:
            if (xDelete != null) xDelete(ref pAux);
        }
...
*/
    static int sqlite3_aggregate_count( sqlite3_context p )
    {
      ...
      return p.pMem.n;
        ...
        {
            var pVm = pStmt;
            return pVm != null ? pVm.nResColumn : 0;
        }
...
__attribute__((aligned(8)))
                ...
                //
                var nullMem =
                    new Mem(); //    static const Mem nullMem = {{0}, (double)0, 0, "", 0, MEM_Null, SQLITE_NULL, 0, 0, 0 };
        ...
        private static int sqlite3_column_bytes(sqlite3_stmt pStmt, int i)
        {
            var val = sqlite3_value_bytes(columnMem(pStmt, i));
            columnMallocFailure(pStmt);
            return val;
        }

        private static int sqlite3_column_bytes16(sqlite3_stmt pStmt, int i)
        {
            var val = sqlite3_value_bytes16(columnMem(pStmt, i));
            columnMallocFailure(pStmt);
            return val;
        }
        ...
        {
            var val = sqlite3_value_double(columnMem(pStmt, i));
            columnMallocFailure(pStmt);
            return val;
        }
        ...
        {
            var val = sqlite3_value_int(columnMem(pStmt, i));
            columnMallocFailure(pStmt);
            return val;
        }
        ...
        {
            var val = sqlite3_value_int64(columnMem(pStmt, i));
            columnMallocFailure(pStmt);
            return val;
        }
        ...
        {
            var val = sqlite3_value_text(columnMem(pStmt, i));
            columnMallocFailure(pStmt);
            if (string.IsNullOrEmpty(val)) return null;
            return val;
        }

        private static sqlite3_value sqlite3_column_value(sqlite3_stmt pStmt, int i)
        {
            var pOut = columnMem(pStmt, i);
...
//}
        ...
        {
            var iType = sqlite3_value_type(columnMem(pStmt, i));
            columnMallocFailure(pStmt);
            return iType;
        }
...
pStmt, N, (const void*(*)(Mem*))sqlite3_value_text16, COLNAME_COLUMN);
         ...
         ** value in any case.
         */
        private static int vdbeUnbind(Vdbe p, int i)
        {
            Mem pVar;
        ...
        )
        {
            ...
            if (rc == SQLITE_OK)
            {
                if (zData != null)
                {
                    ...
                    if (rc == SQLITE_OK && encoding != 0) rc = sqlite3VdbeChangeEncoding(pVar, ENC(p.db));
        ...
        public static int sqlite3_bind_null(sqlite3_stmt pStmt, int i)
        {
            ...
            var p = pStmt;
            rc = vdbeUnbind(p, i);
            if (rc == SQLITE_OK) sqlite3_mutex_leave(p.db.mutex);
            return rc;
        }
...
return bindText(pStmt, i, zData, nData, xDel, SQLITE_UTF16NATIVE);
...
#endif // * SQLITE_OMIT_UTF16 */
        private static int sqlite3_bind_value(sqlite3_stmt pStmt, int i, sqlite3_value pValue)
        {
            int rc;
            switch (pValue.type)
            {
        ...
        private static int sqlite3_bind_zeroblob(sqlite3_stmt pStmt, int i, int n)
        {
            int rc;
            var p = pStmt;
            rc = vdbeUnbind(p, i);
         ...
         */
        private static int sqlite3_bind_parameter_count(sqlite3_stmt pStmt)
        {
            var p = pStmt;
            return p != null ? p.nVar : 0;
        }
         ...
         ** exist.
         */
        private static void createVarMap(Vdbe p)
        {
            if (0 == p.okVar)
            {
                int j;
         ...
         */
        private static string sqlite3_bind_parameter_name(sqlite3_stmt pStmt, int i)
        {
            var p = pStmt;
            if (p == null || i < 1 || i > p.nVar) return "";
            createVarMap(p);
            return p.azVar[i - 1];
        }
         ...
         */
        private static int sqlite3TransferBindings(sqlite3_stmt pFromStmt, sqlite3_stmt pToStmt)
        {
            var pFrom = pFromStmt;
            var pTo = pToStmt;
            int i;
            ...
            for (i = 0; i < pFrom.nVar; i++) sqlite3VdbeMemMove(pTo.aVar[i], pFrom.aVar[i]);
...
*/
    static int sqlite3_transfer_bindings( sqlite3_stmt pFromStmt, sqlite3_stmt pToStmt )
    {
      ...
      return sqlite3TransferBindings( pFromStmt, pToStmt );
         ...
         */
        private static sqlite3 sqlite3_db_handle(sqlite3_stmt pStmt)
        {
            return pStmt != null ? pStmt.db : null;
        }
         ...
         ** are no more.
         */
        private static sqlite3_stmt sqlite3_next_stmt(sqlite3 pDb, sqlite3_stmt pStmt)
        {
            sqlite3_stmt pNext;
         ...
         */
        private static int sqlite3_stmt_status(sqlite3_stmt pStmt, int op, int resetFlag)
        {
            var pVdbe = pStmt;
            var v = pVdbe.aCounter[op - 1];
