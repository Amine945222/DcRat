...
namespace CS_SQLite3
{
    public partial class CSSQLite
    {
        ...
        private static Table sqlite3SrcListLookup(Parse pParse, SrcList pSrc)
        {
            ...
            if (sqlite3IndexedByLookup(pParse, pItem) != 0) pTab = null;
            return pTab;
        }
...
return null;
        ...
        )
        {
            ...
            var oldIdx = -1; /* VdbeCursor for the OLD table of AFTER triggers */
            NameContext sNC; /* Name context to resolve expressions in */
            int iDb; /* Database number */
            var memCnt = -1; /* Memory cell used for change counting */
...
#endif
            var iBeginAfterTrigger = 0; /* Address of after trigger program */
            var iEndAfterTrigger = 0; /* Exit of after trigger program */
            var iBeginBeforeTrigger = 0; /* Address of before trigger program */
            var iEndBeforeTrigger = 0; /* Exit of before trigger program */
            u32 old_col_mask = 0; /* Mask of OLD.* columns in use */

            sContext = new AuthContext(); //memset(&sContext, 0, sizeof(sContext));

            db = pParse.db;
            if (pParse.nErr != 0 /*|| db.mallocFailed != 0 */) goto delete_from_cleanup;
...
isView = false;
      ...
      */
            if (sqlite3ViewGetColumnNames(pParse, pTab) != 0) goto delete_from_cleanup;

            if (sqlite3IsReadOnly(pParse, pTab, pTrigger != null ? 1 : 0)) goto delete_from_cleanup;
...
rcauth = sqlite3AuthCheck(pParse, SQLITE_DELETE, pTab->zName, 0, zDb);
            ...
            if (rcauth == SQLITE_DENY) goto delete_from_cleanup;
            Debug.Assert(!isView || pTrigger != null);

            /* Allocate a cursor used to store the old.* data for a trigger.
            */
            if (pTrigger != null) oldIdx = pParse.nTab++;

            /* Assign  cursor number to the table and all its indices.
            */
            Debug.Assert(pTabList.nSrc == 1);
            iCur = pTabList.a[0].iCursor = pParse.nTab++;
            for (pIdx = pTab.pIndex; pIdx != null; pIdx = pIdx.pNext) pParse.nTab++;
...
sqlite3AuthContextPush(pParse, sContext, pTab.zName);
      ...
      */
            ...
            if (v == null) goto delete_from_cleanup;
            ...
            if (pTrigger != null)
            {
                var orconf = pParse.trigStack != null ? pParse.trigStack.orconf : OE_Default;
                var iGoto = sqlite3VdbeAddOp0(v, OP_Goto);
            ...
            */
            ...
            if (sqlite3ResolveExprNames(sNC, ref pWhere) != 0) goto delete_from_cleanup;
#endif
            ...
            if (rcauth == SQLITE_OK && pWhere == null && null == pTrigger && !IsVirtual(pTab))
            {
                 ...
                 */
            {
                var iRowid = ++pParse.nMem; /* Used for storing rowid values. */
                var iRowSet = ++pParse.nMem; /* Register for rowset of rows to delete */
                ...
                */
                ...
                if ((db.flags & SQLITE_CountRows) != 0) sqlite3VdbeAddOp2(v, OP_AddImm, memCnt, 1);

                sqlite3WhereEnd(pWInfo);

                /* Open the pseudo-table used to store OLD if there are triggers.
                */
                if (pTrigger != null) sqlite3VdbeAddOp3(v, OP_OpenPseudo, oldIdx, 0, pTab.nCol);
                 ...
                 */
                if (pTrigger != null) sqlite3VdbeResolveLabel(v, addr);
                addr = sqlite3VdbeAddOp3(v, OP_RowSetRead, iRowSet, end, iRowid);

                if (pTrigger != null)
                {
                    var iData = ++pParse.nMem; /* For storing row data of OLD table */
