            ...
            if (v != null)
            {
                ...
                if (pParse.cookieGoto > 0)
                {
                    ...
                    for (iDb = 0, mask = 1; iDb < db.nDb; mask <<= 1, iDb++)
                    {
                        ...
                        if (db.init.busy == 0) sqlite3VdbeAddOp2(v, OP_VerifyCookie, iDb, pParse.cookieValue[iDb]);
                    }
...
pParse.nVtabLock = 0;
        ...
        )
        {
            ...
            if (SQLITE_OK != sqlite3ReadSchema(pParse)) return null;
         ...
         */
        private static void sqlite3DeleteIndex(Index p)
        {
            Index pOld;
            var zName = p.zName;
        ...
        private static void sqlite3UnlinkAndDeleteIndex(sqlite3 db, int iDb, string zIdxName)
        {
            ...
            if (pIndex != null)
            {
                ...
                else
                {
                    ...
                    if (ALWAYS(p != null && p.pNext == pIndex)) p.pNext = pIndex.pNext;
                }
        ...
        private static void sqlite3ResetInternalSchema(sqlite3 db, int iDb)
        {
            ...
            if (iDb == 0) sqlite3BtreeEnterAll(db);
         ...
         */
        private static void sqliteResetColumnNames(Table pTable)
        {
            int i;
            Column pCol;
            var db = pTable.dbMem;
         ...
         ** the table.
         */
        private static void sqlite3DeleteTable(ref Table pTable)
        {
            Index pIndex;
            Index pNext;
            FKey pFKey;
      ...
      */
        ...
        private static void sqlite3OpenMasterTable(Parse p, int iDb)
        {
            ...
            sqlite3VdbeChangeP4(v, -1, 5, P4_INT32); /* 5 column table */
            if (p.nTab == 0) p.nTab = 1;
        }
        ...
        )
        {
            ...
            if (SQLITE_OK != sqlite3CheckObjectName(pParse, zName)) goto begin_table_error;
...
goto begin_table_error;
            ...
            if (!IN_DECLARE_VTAB)
            {
                if (SQLITE_OK != sqlite3ReadSchema(pParse)) goto begin_table_error;
                pTable = sqlite3FindTable(db, zName, db.aDb[iDb].zName);
                if (pTable != null)
                {
                    if (noErr == 0) sqlite3ErrorMsg(pParse, "table %T already exists", pName);
                    goto begin_table_error;
                }
...
//        db.mallocFailed = 1;
            ...
            if (pParse.nested == 0 && zName == "sqlite_sequence") pTable.pSchema.pSeqTab = pTable;
#endif
            ...
            if (0 == db.init.busy && (v = sqlite3GetVdbe(pParse)) != null)
            {
                ...
                if (isVirtual != 0) sqlite3VdbeAddOp0(v, OP_VBegin);
                ...
                for (i = 0; i < pList.nExpr; i++)
                {
                    for (iCol = 0; iCol < pTab.nCol; iCol++)
                        if (sqlite3StrICmp(pList.a[i].zName, pTab.aCol[iCol].zName) == 0)
                            break;
                    if (iCol < pTab.nCol) pTab.aCol[iCol].isPrimKey = 0;
                }
        ...
        private static void sqlite3AddCollateType(Parse pParse, Token pToken)
        {
            ...
            if (sqlite3LocateCollSeq(pParse, zColl) != null)
            {
                ...
                for (pIdx = p.pIndex; pIdx != null; pIdx = pIdx.pNext)
                {
                    ...
                    if (pIdx.aiColumn[0] == i) pIdx.azColl[0] = p.aCol[i].zColl;
                }
            }
        ...
        private static CollSeq sqlite3LocateCollSeq(Parse pParse, string zName)
        {
            ...
            if (0 == initbusy && (pColl == null || pColl.xCmp == null))
            {
                ...
                if (pColl == null) sqlite3ErrorMsg(pParse, "no such collation sequence: %s", zName);
            }
        ...
        private static void identPut(StringBuilder z, ref int pIdx, string zSignedIdent)
        {
            ...
            if (!needQuote) needQuote = j < zIdent.Length && zIdent[j] != 0;
            if (needQuote)
            {
...
/* SQLITE_AFF_REAL    */ " REAL"
        ...
        )
        {
            ...
            if (pEnd == null && pSelect == null /*|| db.mallocFailed != 0 */) return;
            p = pParse.pNewTable;
            if (p == null) return;
      ...
      */
            if (p.pCheck != null)
            {
                ...
                sNC.pSrcList = sSrc;
                sNC.isCheck = 1;
                if (sqlite3ResolveExprNames(sNC, ref p.pCheck) != 0) return;
            }
             ...
             */
            if (db.init.busy != 0) p.tnum = db.init.newTnum;
            ...
            if (0 == db.init.busy)
            {
                int n;
                Vdbe v;
                var zType = ""; /* "view" or "table" */
                var zType2 = ""; /* "VIEW" or "TABLE" */
                var zStmt = ""; /* Text of the CREATE TABLE or CREATE VIEW statement */
            ...
            */
            if (db.init.busy != 0)
            {
                ...
                if (p.pSelect == null)
                {
                    var zName = pParse.sNameToken.z;
                    int nName;
                    Debug.Assert(pSelect == null && pCons != null && pEnd != null);
                    if (pCons.z == null) pCons = pEnd;
        ...
        )
        {
            ...
            //  return;
            //}
            if (0 == db.init.busy) sqlite3ViewGetColumnNames(pParse, p);

            /* Locate the end of the CREATE VIEW statement.  Make sEnd point to
             ** the end.
             */
            sEnd = pParse.sLastToken;
            if (ALWAYS(sEnd.z[0] != 0) && sEnd.z[0] != ';') sEnd.z = sEnd.z.Substring(sEnd.n);
            sEnd.n = 0;
        ...
        private static int sqlite3ViewGetColumnNames(Parse pParse, Table pTable)
        {
            ...
            var nErr = 0; /* Number of errors encountered */
            int n; /* Temporarily holds the number of cursors assigned */
            var db = pParse.db; /* Database connection for malloc errors */
...
db.xAuth = xAuth;
        ...
        private static void sqliteViewResetAll(sqlite3 db, int idx)
        {
            ...
            for (i = db.aDb[idx].pSchema.tblHash.first; i != null; i = i.next)
            {
                ...
                if (pTab.pSelect != null) sqliteResetColumnNames(pTab);
            }
...
//# define sqliteViewResetAll(A,B)
        ...
        private static void sqlite3RootPageMoved(Db pDb, int iFrom, int iTo)
        {
            HashElem pElem;
            Hash pHash;
            ...
            for (pElem = pHash.first;
                 ...
                 pElem = pElem.next) // ( pElem = sqliteHashFirst( pHash ) ; pElem ; pElem = sqliteHashNext( pElem ) )
            {
                var pIdx = (Index)pElem.data; // sqliteHashData( pElem );
...
destroyRootPage( pParse, pIdx.tnum, iDb );
            ...
            while (true)
            {
                ...
                for (pIdx = pTab.pIndex; pIdx != null; pIdx = pIdx.pNext)
                {
                    ...
                    if ((iDestroyed == 0 || iIdx < iDestroyed) && iIdx > iLargest) iLargest = iIdx;
                }
        ...
        private static void sqlite3DropTable(Parse pParse, SrcList pName, int isView, int noErr)
        {
            ...
            if (pTab == null)
            {
                if (noErr != 0) sqlite3ErrorClear(pParse);
                goto exit_drop_table;
            }
             ...
             ** it is initialized.
             */
            if (IsVirtual(pTab) && sqlite3ViewGetColumnNames(pParse, pTab) != 0) goto exit_drop_table;
#if !SQLITE_OMIT_AUTHORIZATION
{
int code;
...
goto exit_drop_table;
            ...
            if (v != null)
            {
                ...
                if (0 == isView && !IsVirtual(pTab)) destroyTable(pParse, pTab);

                /* Remove the table entry from SQLite's internal schema and modify
                 ** the schema cookie.
                 */
                if (IsVirtual(pTab)) sqlite3VdbeAddOp4(v, OP_VDestroy, iDb, 0, 0, pTab.zName, 0);
        ...
        )
        {
            ...
            if (pToCol != null)
                for (i = 0; i < nCol; i++)
                {
                    var n = sqlite3Strlen30(pToCol.a[i].zName);
                    if (pFKey.aCol[i] == null) pFKey.aCol[i] = new FKey.sColMap();
        ...
        {
            var pTab = pIndex.pTable; /* The table that is indexed */
            var iTab = pParse.nTab++; /* Btree cursor used for pTab */
            var iIdx = pParse.nTab++; /* Btree cursor used for pIndex */
...
return;
            ...
            if (memRootPage >= 0) sqlite3VdbeChangeP5(v, 1);
            ...
            if (pIndex.onError != OE_None)
            {
                ...
                var pRegKey = regIdxKey; // SQLITE_INT_TO_PTR( regIdxKey );
         ...
         */
        // OVERLOADS, so I don't need to rewrite parse.c
        private static void sqlite3CreateIndex(Parse pParse, int null_2, int null_3, int null_4, int null_5,
        ...
        private static void sqlite3CreateIndex(Parse pParse, int null_2, int null_3, int null_4, ExprList pList,
            int onError, int null_7, int null_8, int sortOrder, int ifNotExist)
        {
            sqlite3CreateIndex(pParse, null, null, null, pList, onError, null, null, sortOrder, ifNotExist);
        }

        private static void sqlite3CreateIndex(
        ...
        )
        {
            ...
            if ( /* db.mallocFailed != 0 || */ IN_DECLARE_VTAB) goto exit_create_index;
            ...
            if (pName != null)
            {
                ...
                if (SQLITE_OK != sqlite3CheckObjectName(pParse, zName)) goto exit_create_index;
                if (0 == db.init.busy)
                ...
                {
                    if (ifNotExist == 0) sqlite3ErrorMsg(pParse, "index %s already exists", zName);
                    goto exit_create_index;
                }
            }
            else
...
goto exit_create_index;
            ...
            for (i = 0; i < pList.nExpr; i++)
            {
                ...
                if (pExpr != null)
                {
                    ...
                    if (ALWAYS(pColl != null)) nExtra += 1 + sqlite3Strlen30(pColl.zName);
                }
            }
            ...
            if (zName.Length == nName) pIndex.zName = zName;
                ...
                pIndex.zName = zName.Substring(0, nName); // memcpy( pIndex.zName, zName, nName + 1 );
            pIndex.pTable = pTab;
            ...
            */
            ...
            for (i = 0; i < pList.nExpr; i++)
            {
                ...
                else
                {
                    ...
                    if (zColl == null) zColl = db.pDfltColl.zName;
                }

                if (0 == db.init.busy && sqlite3LocateCollSeq(pParse, zColl) == null) goto exit_create_index;
            ...
            if (db.init.busy != 0)
            {
                ...
                db.flags |= SQLITE_InternChanges;
                if (pTblName != null) pIndex.tnum = db.init.newTnum;
            }
            ...
            if (db.init.busy != 0 || pTblName == null)
            {
                ...
                {
                    var pOther = pTab.pIndex;
                    while (pOther.pNext != null && pOther.pNext.onError != OE_Replace) pOther = pOther.pNext;
        ...
        private static void sqlite3DropIndex(Parse pParse, SrcList pName, int ifExists)
        {
            ...
            //}
            Debug.Assert(pName.nSrc == 1);
            if (SQLITE_OK != sqlite3ReadSchema(pParse)) goto exit_drop_index;
            pIndex = sqlite3FindIndex(db, pName.a[0].zName, pName.a[0].zDatabase);
            if (pIndex == null)
            {
                if (ifExists == 0) sqlite3ErrorMsg(pParse, "no such index: %S", pName, 0);
                pParse.checkSchema = 1;
                goto exit_drop_index;
            }
...
goto exit_drop_index;
        ...
        )
        {
             ...
             ** out of the way */
            for (i = pSrc.nSrc - 1; i >= iStart; i--) pSrc.a[i + nExtra] = pSrc.a[i];
        ...
        )
        {
            ...
            if (pDatabase != null && string.IsNullOrEmpty(pDatabase.z)) pDatabase = null;
            if (pDatabase != null)
            {
                var pTemp = pDatabase;
                pDatabase = pTable;
        ...
        private static void sqlite3SrcListAssignCursors(Parse pParse, SrcList pList)
        {
            ...
            if (pList != null)
                for (i = 0; i < pList.nSrc; i++)
                {
                    ...
                    if (pItem.pSelect != null) sqlite3SrcListAssignCursors(pParse, pItem.pSelect.pSrc);
                }
        }
         ...
         */
        // OVERLOADS, so I don't need to rewrite parse.c
        private static SrcList sqlite3SrcListAppendFromTerm(Parse pParse, SrcList p, int null_3, int null_4,
        ...
        private static SrcList sqlite3SrcListAppendFromTerm(Parse pParse, SrcList p, Token pTable, Token pDatabase,
        ...
        )
        {
            ...
            if (pAlias.n != 0) pItem.zAlias = sqlite3NameFromToken(db, pAlias);
        ...
        private static void sqlite3CommitTransaction(Parse pParse)
        {
            ...
            Debug.Assert(db != null);
            /*  if( db.aDb[0].pBt==0 ) return; */
            if (sqlite3AuthCheck(pParse, SQLITE_TRANSACTION, "COMMIT", null, null) != 0) return;
            v = sqlite3GetVdbe(pParse);
            if (v != null) sqlite3VdbeAddOp2(v, OP_AutoCommit, 1, 0);
        }
        ...
        private static void sqlite3RollbackTransaction(Parse pParse)
        {
            ...
            Debug.Assert(db != null);
            /*  if( db.aDb[0].pBt==0 ) return; */
            if (sqlite3AuthCheck(pParse, SQLITE_TRANSACTION, "ROLLBACK", null, null) != 0) return;
            v = sqlite3GetVdbe(pParse);
            if (v != null) sqlite3VdbeAddOp2(v, OP_AutoCommit, 1, 1);
        }
...
|| sqlite3AuthCheck(pParse, SQLITE_SAVEPOINT, az[op], zName, 0)
        ...
        private static void sqlite3CodeVerifySchema(Parse pParse, int iDb)
        {
            ...
            db = pParse.db;
            if (pParse.cookieGoto == 0) pParse.cookieGoto = sqlite3VdbeAddOp2(v, OP_Goto, 0, 0) + 1;
            if (iDb >= 0)
            {
                ...
                if ((pParse.cookieMask & mask) == 0)
                {
                    ...
                    if (OMIT_TEMPDB == 0 && iDb == 1) sqlite3OpenTempDatabase(pParse);
                }
            }
        }
        ...
        private static bool collationMatch(string zColl, Index pIndex)
        {
            ...
            for (i = 0; i < pIndex.nColumn; i++)
            {
                ...
                Debug.Assert(z != null);
                if (0 == sqlite3StrICmp(z, zColl)) return true;
            }
        ...
        private static KeyInfo sqlite3IndexKeyinfo(Parse pParse, Index pIdx)
        {
            ...
            if (pParse.nErr != 0) pKey = null; //sqlite3DbFree( db, ref pKey );
            return pKey;
        }
    }
}