...
public WhereTerm[] aStatic = new WhereTerm[1];    /* Initial static space for a[] */
        ...
        private const int WHERE_ROWID_EQ = 0x00001000; /* rowid=EXPR or rowid IN (...) */
        private const int WHERE_ROWID_RANGE = 0x00002000; /* rowid<EXPR and/or rowid>EXPR */
        private const int WHERE_COLUMN_EQ = 0x00010000; /* x=EXPR or x IN (...) */
        private const int WHERE_COLUMN_RANGE = 0x00020000; /* x<EXPR and/or x>EXPR */
        private const int WHERE_COLUMN_IN = 0x00040000; /* x IN (...) */
        private const int WHERE_COLUMN_NULL = 0x00080000; /* x IS NULL */
        private const int WHERE_INDEXED = 0x000f0000; /* Anything that uses an index */
        private const int WHERE_IN_ABLE = 0x000f1000; /* Able to support an IN operator */
        private const int WHERE_TOP_LIMIT = 0x00100000; /* x<EXPR or x<=EXPR constraint */
        private const int WHERE_BTM_LIMIT = 0x00200000; /* x>EXPR or x>=EXPR constraint */
        private const int WHERE_IDX_ONLY = 0x00800000; /* Use index only - omit table */
        private const int WHERE_ORDERBY = 0x01000000; /* Output will appear in correct order */
        private const int WHERE_REVERSE = 0x02000000; /* Scan in reverse order */
        private const int WHERE_UNIQUE = 0x04000000; /* Selects no more than one row */
        private const int WHERE_VIRTUALTABLE = 0x08000000; /* Use virtual-table processing */
        private const int WHERE_MULTI_OR = 0x10000000; /* OR using multiple indices */
        ...
        private static void whereClauseClear(WhereClause pWC)
        {
            int i;
            WhereTerm a;
            var db = pWC.pParse.db;
            for (i = pWC.nTerm - 1; i >= 0; i--) //, a++)
            {
                a = pWC.a[i];
                if ((a.wtFlags & TERM_DYNAMIC) != 0) sqlite3ExprDelete(db, ref a.pExpr);
                if ((a.wtFlags & TERM_ORINFO) != 0)
                    whereOrInfoDelete(db, a.u.pOrInfo);
                else if ((a.wtFlags & TERM_ANDINFO) != 0) whereAndInfoDelete(db, a.u.pAndInfo);
            }
        ...
        {
            var expRight = (u16)(pExpr.pRight.flags & EP_ExpCollate);
            var expLeft = (u16)(pExpr.pLeft.flags & EP_ExpCollate);
        ...
        )
        {
            ...
            var c = 0; /* One character in z[] */
            int cnt; /* Number of non-wildcard prefix characters */
            var wc = new char[3]; /* Wildcard characters */
            CollSeq pColl; /* Collating sequence for LHS */
            var db = pParse.db; /* Data_base connection */

            if (!sqlite3IsLikeFunction(db, pExpr, ref pnoCase, wc)) return 0;
#if SQLITE_EBCDIC
if( pnoCase ) return 0;
#endif
            pList = pExpr.x.pList;
...
return 1;
        ...
        {
            var pParse = pWC.pParse; /* Parser context */
            var db = pParse.db; /* Data_base connection */
            var pTerm = pWC.a[idxTerm]; /* The term to be analyzed */
            var pExpr = pTerm.pExpr; /* The expression of the term */
            var pMaskSet = pWC.pMaskSet; /* Table use masks */
            int i; /* Loop counters */
...
//      if ( db.mallocFailed != 0 ) return;
            ...
            for (i = pOrWc.nTerm - 1; i >= 0 && indexable != 0; i--) //, pOrTerm++ )
            {
                    ...
                    if (pAndInfo != null)
                    {
                        ...
                        ////if ( 0 == db.mallocFailed )
                        {
                            for (j = 0; j < pAndWC.nTerm; j++) //, pAndTerm++ )
                            {
                                ...
                                if (allowedOp(pAndTerm.pExpr.op)) b |= getMask(pMaskSet, pAndTerm.leftCursor);
                            }
                        }
                        indexable &= b;
                    }
                ...
                else
                {
                    ...
                    {
                        var pOther = pOrWc.a[pOrTerm.iParent];
            ...
            if (chngToIN != 0)
            {
                var okToChngToIN = 0; /* True if the conversion to IN is valid */
                var iColumn = -1; /* Column index on lhs of IN operator */
                var iCursor = -1; /* Table cursor common to all terms */
        ...
        )
        {
            ...
            var pParse = pWC.pParse; /* Parsing context */
            var db = pParse.db; /* Data_base connection */
            ...
            {
                var x = getMask(pMaskSet, pExpr.iRightJoinTable);
                prereqAll |= x;
...
pNewTerm.prereqAll = pTerm.prereqAll;
        ...
        {
            var allowed = ~getMask(pMaskSet, iBase);
        ...
        )
        {
            ...
            var sortOrder = 0; /* XOR of index and ORDER BY sort direction */
         ...
         */
        private static bool sortableByRowid(
...
sqlite3DebugPrintf( "  estimatedCost=%g\n", p.estimatedCost );
        ...
        )
        {
            ...
            var iCur = pSrc.iCursor; /* The cursor of the table to be accessed */
            var maskSrc = getMask(pWC.pMaskSet, iCur); /* Bitmask for pSrc */
            var pWCEnd = pWC.a[pWC.nTerm]; /* End of pWC.a[] */
            WhereTerm pTerm; /* A single term of the WHERE clause */

            /* Search the WHERE clause terms for a usable WO_OR term. */
            for (var _pt = 0; _pt < pWC.nTerm; _pt++) //<pWCEnd; pTerm++)
            {
                pTerm = pWC.a[_pt];
                ...
                {
                    var pOrWC = pTerm.u.pOrInfo.wc;
                    var pOrWCEnd = pOrWC.a[pOrWC.nTerm];
                    WhereTerm pOrTerm;
                    var flags = WHERE_MULTI_OR;
                    double rTotal = 0;
                    double nRow = 0;
            ...
            _pOrWC, pOrWC.nTerm - _pOrWC//( pOrTerm - pOrWC.a ), ( pTerm - pWC.a )
                        ...
                        {
                            var pAndWC = pOrTerm.u.pAndInfo.wc;
...
bestOrClauseIndex(pParse, pWC, pSrc, notReady, pOrderBy, pCost);
        ...
        )
        {
            ...
            var rev = 0; /* True to scan in reverse order */
            ...
            if (pSrc.notIndexed != 0) pProbe = null;
            ...
            if (null == pSrc.pIndex)
            {
                ...
                if (pTerm != null)
                {
                    ...
                    if (!ExprHasProperty(pExpr = pTerm.pExpr, EP_xIsSelect)
                        && pExpr.x.pList != null
                       )
                    {
                ...
                */
                ...
                if (pOrderBy != null)
                {
                    if (sortableByRowid(iCur, pOrderBy, pWC.pMaskSet, ref rev))
                    {
                        ...
                        if (rev != 0) wsFlags |= WHERE_REVERSE;
                    }
                    else
                    {
        ...
        WHERETRACE( "... index %s:\n", pProbe.zName );
                ...
                {
                    var j = pProbe.aiColumn[i];
                    pTerm = findTerm(pWC, iCur, j, notReady, eqTermMask, pProbe);
                    if (pTerm == null) break;
                    wsFlags |= WHERE_COLUMN_EQ;
                    if ((pTerm.eOperator & WO_IN) != 0)
                    {
                        var pExpr = pTerm.pExpr;
                ...
                if (pProbe.onError != OE_None && nEq == pProbe.nColumn)
                {
                    ...
                    if ((wsFlags & (WHERE_COLUMN_IN | WHERE_COLUMN_NULL)) == 0) wsFlags |= WHERE_UNIQUE;
                }
#if (SQLITE_TEST) && (SQLITE_DEBUG)
        ...
        nEq, inMultiplier, nRow, cost );
            ...
            nRow, cost );
                ...
                */
                if (pOrderBy != null)
                {
                    ...
                    {
                        if (wsFlags == 0) wsFlags = WHERE_COLUMN_RANGE;
            ...
            WHERETRACE( "...... orderby increases cost to %.9g\n", cost );
                ...
                if (wsFlags != 0 && pSrc.colUsed < (Bitmask)1 << (BMS - 1))
                {
                    ...
                    {
                        var x = pProbe.aiColumn[j];
                        if (x < BMS - 1) m &= ~((Bitmask)1 << x);
                    }
            ...
            */
            pCost.plan.wsFlags = pCost.plan.wsFlags | eqTermMask;
            WHERETRACE("best index is %s, nrow=%.9g, cost=%.9g, wsFlags=%x, nEq=%d\n",
                (pCost.plan.wsFlags & WHERE_INDEXED) != 0 ? pCost.plan.u.pIdx.zName : "(none)", pCost.nRow,
...
//sqlite3DbFree(pParse.db, p);
        ...
        private static void disableTerm(WhereLevel pLevel, WhereTerm pTerm)
        {
                ...
                && (pLevel.iLeftJoin == 0 || ExprHasProperty(pTerm.pExpr, EP_FromJoin)))
            {
                ...
                if (pTerm.iParent >= 0)
                {
                    var pOther = pTerm.pWC.a[pTerm.iParent];
         ...
         */
        private static void codeApplyAffinity(Parse pParse, int _base, int n, Index pIdx)
        {
            if (n > 0)
            {
                var v = pParse.pVdbe;
        ...
        )
        {
            ...
            else
            {
                ...
                if (pLevel.u._in.nIn == 0) pLevel.addrNxt = sqlite3VdbeMakeLabel(v);
                pLevel.u._in.nIn++;
        ...
        {
            var nEq = (int)pLevel.plan.nEq; /* The number of == or IN constraints to code */
            var v = pParse.pVdbe; /* The vm under construction */
            Index pIdx; /* The index being used for this loop */
            var iCur = pLevel.iTabCur; /* The cursor of the table */
        ...
        )
        {
            ...
            var iRowidReg = 0; /* Rowid is stored in this register, if not zero */
            var iReleaseReg = 0; /* Temp register to free before returning */
...
/* TK_GE */ OP_SeekGe
          ...
          VdbeComment( v, "pk" );
            ...
            else if ((pLevel.plan.wsFlags & (WHERE_COLUMN_RANGE | WHERE_COLUMN_EQ)) != 0)
            {
                ...
                var nEq = (int)pLevel.plan.nEq;
                var isMinQuery = 0; /* If this is an optimized SELECT min(x).. */
                ...
                var nExtraReg = 0; /* Number of extra registers needed */
                 ...
                 */
                if (bRev == (pIdx.aSortOrder[nEq] == SQLITE_SO_ASC ? 1 : 0)) SWAP(ref pRangeEnd, ref pRangeStart);
            ...
            if ((pLevel.plan.wsFlags & WHERE_MULTI_OR) != 0)
            {
                ...
                var oneTab = new SrcList(); /* Shortened table list */

                var regReturn = ++pParse.nMem; /* Register used with OP_Gosub */
                var regRowset = 0; /* Register for RowSet object */
                var regRowid = 0; /* Register holding rowid */
                var iLoopBody = sqlite3VdbeMakeLabel(v); /* Start of loop body */
                ...
                for (ii = 0; ii < pOrWc.nTerm; ii++)
                {
                    ...
                    if (pOrTerm.leftCursor == iCur || pOrTerm.eOperator == WO_AND)
                    {
                        ...
                        if (pSubWInfo != null)
                        {
                            if ((wctrlFlags & WHERE_DUPLICATES_OK) == 0)
                            {
                                var iSet = ii == pOrWc.nTerm - 1 ? -1 : ii;
                                int r;
...

            {
                 ...
                 */
                u8[] aStep = { OP_Next, OP_Prev };
                u8[] aStart = { OP_Rewind, OP_Last };
            ...
            for (j = pWC.nTerm; j > 0; j--) //, pTerm++)
            {
                ...
                if (pLevel.iLeftJoin != 0 &&
                    !((pE.flags & EP_FromJoin) == EP_FromJoin)) // !ExprHasProperty(pE, EP_FromJoin) ){
                    continue;
...
*/
    ...
    static int nQPlan = 0;              /* Next free slow in _query_plan[] */
        ...
        )
        {
            ...
            var v = pParse.pVdbe; /* The virtual data_base engine */
...
pWC.vmask |= ( (Bitmask)1 << i );
      ...
      WHERETRACE( "*** Optimizer Start ***\n" );
            ...
            for (i = iFrom = 0; i < pTabList.nSrc; i++) //, pLevel++ )
            {
                ...
                var bestJ = 0; /* The value of j */
                Bitmask m; /* Bitmask value for j or bestJ */
                var once = 0; /* True when first table is seen */
...
bestVirtualIndex(pParse, pWC, pTabItem, notReady, pOrderBy, &sCost, pp);
        ...
        i );//pLevel - pWInfo.a );
#endif
                if ((bestPlan.plan.wsFlags & WHERE_ORDERBY) != 0) ppOrderBy = null;
      ...
      WHERETRACE( "*** Optimizer Finished ***\n" );
#endif
            if (pParse.nErr != 0 /*|| db.mallocFailed != 0 */) goto whereBeginError;

            /* If the total query only selects a single row, then the ORDER BY
             ** clause is irrelevant.
             */
            if ((andFlags & WHERE_UNIQUE) != 0 && ppOrderBy != null) ppOrderBy = null;
            ...
            for (i = 0; i < pTabList.nSrc; i++) //, pLevel++ )
            {
                ...
                {
                    string zMsg;
                    var pItem = pTabList.a[pLevel.iFrom];
                    zMsg = sqlite3MPrintf(db, "TABLE %s", pItem.zName);
                    if (pItem.zAlias != null) zMsg = sqlite3MAppendf(db, zMsg, "%s AS %s", zMsg, pItem.zAlias);
...
pVTab, P4_VTAB);
                ...
                {
                    var op = pWInfo.okOnePass != 0 ? OP_OpenWrite : OP_OpenRead;
                    sqlite3OpenTable(pParse, pTabItem.iCursor, iDb, pTab, op);
                    if (0 == pWInfo.okOnePass && pTab.nCol < BMS)
                    {
                        var b = pTabItem.colUsed;
                        var n = 0;
                        for (; b != 0; b = b >> 1, n++)
                        {
                        }
                ...
                if ((pLevel.plan.wsFlags & WHERE_INDEXED) != 0)
                {
                    var pIx = pLevel.plan.u.pIdx;
                    var pKey = sqlite3IndexKeyinfo(pParse, pIx);
                    var iIdxCur = pLevel.iIdxCur;
...
*/
      ...
      nQPlan = 0;
        ...
        {
            var pParse = pWInfo.pParse;
            var v = pParse.pVdbe;
            int i;
            WhereLevel pLevel;
            var pTabList = pWInfo.pTabList;
            var db = pParse.db;
            ...
            */
            ...
            for (i = pTabList.nSrc - 1; i >= 0; i--)
            {
                ...
                if (pLevel.iLeftJoin != 0)
                {
                    ...
                    if (pLevel.iIdxCur >= 0) sqlite3VdbeAddOp1(v, OP_NullRow, pLevel.iIdxCur);
            ...
            */
            ...
            {
                pLevel = pWInfo.a[i];
                var pTabItem = pTabList.a[pLevel.iFrom];
                var pTab = pTabItem.pTab;
                ...
                if ((pWInfo.wctrlFlags & WHERE_OMIT_CLOSE) == 0)
                {
                    ...
                    if ((pLevel.plan.wsFlags & WHERE_INDEXED) != 0) sqlite3VdbeAddOp1(v, OP_Close, pLevel.iIdxCur);
                }
                ...
                if ((pLevel.plan.wsFlags & WHERE_INDEXED) != 0) ///* && 0 == db.mallocFailed */ )
                {
                    int k, j, last;
                    VdbeOp pOp;
                    var pIdx = pLevel.plan.u.pIdx;
                    var useIndexOnly = (pLevel.plan.wsFlags & WHERE_IDX_ONLY) != 0 ? 1 : 0;
