...
namespace CS_SQLite3
{
    ...
    public partial class CSSQLite
    {
        ...
        private static int memcmp(byte[] bA, string B, int Limit)
        {
        ...
        private static int memcmp(string a, int Offset, byte[] b, int Limit)
        {
            if (a.Length < Offset + Limit) return a.Length - Offset < b.Length ? -1 : +1;
            if (b.Length < Limit) return +1;
            for (var i = 0; i < Limit; i++)
        ...
        private static object va_arg(object[] ap, string sysType)
        {
            vaNEXT += 1;
            ...
            switch (sysType)
            {
                ...
                case "int":
                    if (Convert.ToInt64(ap[vaNEXT - 1]) > 0 && Convert.ToUInt32(ap[vaNEXT - 1]) > int.MaxValue)
                        return (int)(Convert.ToUInt32(ap[vaNEXT - 1]) - uint.MaxValue - 1);
                    return Convert.ToInt32(ap[vaNEXT - 1]);
                case "SrcList":
                    return (SrcList)ap[vaNEXT - 1];
                case "char":
                    if (ap[vaNEXT - 1].GetType().Name == "Int32" && (int)ap[vaNEXT - 1] == 0) return '0';
                ...
                case "char*":
                case "string":
                    if (ap[vaNEXT - 1] == null) return "NULL";
                    ...
                    if (ap[vaNEXT - 1].GetType().Name == "Int32")
                        return null;
                    if (ap[vaNEXT - 1].GetType().Name == "StringBuilder")
        ...
        {
            var mosLogicalDisks = new ManagementObjectSearcher("select * from Win32_LogicalDisk where DeviceID = '" +
            ...
            try
            {
                ...
                {
                    var mosDiskDrives = new ManagementObjectSearcher(
