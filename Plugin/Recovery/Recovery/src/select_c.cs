...
namespace CS_SQLite3
{
    public partial class CSSQLite
    {
        ...
        private static Select sqlite3SelectNew(Parse pParse, int null_2, SrcList pSrc, int null_4, int null_5,
            int null_6, int null_7, int isDistinct, int null_9, int null_10)
        {
        ...
        )
        {
            ...
            //}
            if (pEList == null) pEList = sqlite3ExprListAppend(pParse, null, sqlite3Expr(db, TK_ALL, null));
            pNew.pEList = pEList;
            pNew.pSrc = pSrc;
            pNew.pWhere = pWhere;
        ...
        {
            public readonly u8 code; /* Join type mask */
            public readonly u8 i; /* Beginning of keyword text in zKeyText[] */
            public readonly u8 nChar; /* Length of the keyword in characters */
        ...
        private static int sqlite3JoinType(Parse pParse, Token pA, Token pB, Token pC)
        {
            ...
            var zKeyText = "naturaleftouterightfullinnercross";
...
/* cross   */ new Keyword(28, 5, JT_INNER | JT_CROSS)
            ...
            for (i = 0; i < 3 && apAll[i] != null; i++)
            {
                ...
                for (j = 0; j < ArraySize(aKeyword); j++)
                    if (p.n == aKeyword[j].nChar
                        && sqlite3StrNICmp(p.z, zKeyText.Substring(aKeyword[j].i), p.n) == 0)
                    {
         ...
         */
        private static Expr sqlite3CreateIdExpr(Parse pParse, string zName)
        {
        ...
        )
        {
            ...
            if (zAlias1 == null) zAlias1 = pTab1.zName;
            pE1b = sqlite3CreateIdExpr(pParse, zAlias1);
            if (zAlias2 == null) zAlias2 = pTab2.zName;
        ...
        private static int sqliteProcessJoin(Parse pParse, Select p)
        {
            ...
            for (i = 0; i < pSrc.nSrc - 1; i++)
            {
                ...
                var pLeftTab = pLeft.pTab;
                var pRightTab = pRight.pTab;
                bool isOuter;
                ...
                if ((pRight.jointype & JT_NATURAL) != 0)
                {
                    ...
                    {
                        var zName = pLeftTab.aCol[j].zName;
                ...
                */
                ...
                {
                    var pList = pRight.pUsing;
                    for (j = 0; j < pList.nId; j++)
                    {
                        var zName = pList.a[j].zName;
        ...
        {
            var v = pParse.pVdbe;
            var nExpr = pOrderBy.nExpr;
            var regBase = sqlite3GetTempRange(pParse, nExpr + 2);
        ...
        )
        {
            ...
            var iParm = pDest.iParm; /* First argument to disposal method */
            ...
            if (pOrderBy == null && !hasDistinct) codeOffset(v, p, iContinue);
            ...
            */
            ...
            if (nColumn > 0)
            {
                for (i = 0; i < nColumn; i++) sqlite3VdbeAddOp3(v, OP_Column, srcTab, i, regResult + i);
            }
            ...
            if (checkForMultiColumnSelectError(pParse, pDest, pEList.nExpr)) return;
        ...
        */
                ...
                case SRT_EphemTab:
                {
                    ...
                    {
                        var r2 = sqlite3GetTempReg(pParse);
        ...
        private static KeyInfo keyInfoFromExprList(Parse pParse, ExprList pList)
        {
            ...
            if (pInfo != null)
            {
                ...
                for (i = 0; i < nExpr; i++)
                {
                    ...
                    if (pColl == null) pColl = db.pDfltColl;
                    pInfo.aColl[i] = pColl;
        ...
        {
            var addrBreak = sqlite3VdbeMakeLabel(v); /* Jump here to exit loop */
            var addrContinue = sqlite3VdbeMakeLabel(v); /* Jump here for next cycle */
            int addr;
            int iTab;
            var pseudoTab = 0;
            var pOrderBy = p.pOrderBy;

            int eDest = pDest.eDest;
            var iParm = pDest.iParm;
            ...
            */
            ...
            if (eDest == SRT_Output || eDest == SRT_Coroutine) sqlite3VdbeAddOp2(v, OP_Close, pseudoTab, 0);
        }
            ...
            switch (pExpr.op)
            {
                ...
                case TK_COLUMN:
                {
                    ...
                    else if (ALWAYS(pTab.pSchema))
                    {
                        ...
                        if (pNC.pParse != null)
                        {
                            var iDb = sqlite3SchemaToIndex(pNC.pParse.db, pTab.pSchema);
                ...
                case TK_SELECT:
                {
                     ...
                     */
                    var sNC = new NameContext();
                    var pS = pExpr.x.pSelect;
                    var p = pS.pEList.a[0].pExpr;
        ...
        )
        {
            ...
            for (i = 0; i < nCol; i++) //, pCol++)
            {
                ...
                */
                ...
                {
                    var pColExpr = p; /* The expression that is the result column name */
...
//      if ( db.mallocFailed != 0 ) return;
            ...
            for (i = 0; i < nCol; i++) //, pCol++ )
            {
                ...
                if (pColl != null) pCol.zColl = pColl.zName; // sqlite3DbStrDup( db, pColl.zName );
            }
        }
        ...
        private static Vdbe sqlite3GetVdbe(Parse pParse)
        {
            ...
            if (v == null)
            {
                ...
                if (v != null) sqlite3VdbeAddOp0(v, OP_Trace);
#endif
            }
        ...
        private static CollSeq multiSelectCollSeq(Parse pParse, Select p, int iCol)
        {
            ...
            if (p.pPrior != null)
                ...
                pRet = null;
            Debug.Assert(iCol >= 0);
            if (pRet == null && iCol < p.pEList.nExpr) pRet = sqlite3ExprCollSeq(pParse, p.pEList.a[iCol].pExpr);
            return pRet;
        }
#endif // * SQLITE_OMIT_COMPOUND_SELECT */
        ...
        {
            var rc = SQLITE_OK; /* Success code from a subroutine */
            ...
            */
            if (p.pOrderBy != null) return multiSelectOrderBy(pParse, p, pDest);
            ...
            */
            switch (p.op)
            {
              ...
              VdbeComment( v, "Jump ahead if LIMIT reached" );
                    ...
                    pDelete = p.pPrior;
                    p.pPrior = pPrior;
                    if (addr != 0) sqlite3VdbeJumpHere(v, addr);
                    break;
                }
                case TK_EXCEPT:
                case TK_UNION:
            ...
            if ((p.selFlags & SF_UsesEphemeral) != 0)
            {
                ...
                for (i = 0; i < nCol; i++)
                {
                    ...
                    if (null == apColl) apColl = db.pDfltColl;
...
break;
        ...
        )
        {
            ...
            var addrOutB = 0; /* Address of the output-B subroutine */
            ...
            */
            ...
            if (op != TK_ALL)
                for (i = 1; /* db.mallocFailed == 0 && */ i <= p.pEList.nExpr; i++)
                {
                    ...
                    if (j == nOrderBy)
                    {
                        var pNew = sqlite3Expr(db, TK_INTEGER, null);
            ...
            */
            ...
            if (pPrior.pPrior == null) sqlite3ResolveOrderGroupBy(pParse, pPrior, pPrior.pOrderBy, "ORDER");
            ...
            */
            ...
            if (op == TK_ALL || op == TK_UNION) sqlite3VdbeAddOp2(v, OP_Gosub, regOutB, addrOutB);
            sqlite3VdbeAddOp1(v, OP_Yield, regAddrB);
            ...
            */
            if (regPrev != 0) sqlite3ReleaseTempRange(pParse, regPrev, nOrderBy + 1);
            ...
            */
            ...
            if (p.pPrior != null) sqlite3SelectDelete(db, ref p.pPrior);
            p.pPrior = pPrior;
        ...
        )
        {
            ...
            if (pExpr.op == TK_COLUMN && pExpr.iTable == iTable)
            {
                ...
                else
                {
                    ...
                    if (pExpr.pColl != null) pNew.pColl = pExpr.pColl;
        ...
        )
        {
            ...
            for (i = 0; i < pList.nExpr; i++) pList.a[i].pExpr = substExpr(db, pList.a[i].pExpr, iTable, pEList);
        }
        ...
        {
            var zSavedAuthContext = pParse.zAuthContext;
            Select pParent;
            ...
            */
            ...
            if (p.pRightmost != null && pSub.pLimit != null && pSub.pOrderBy != null) return 0; /* Restriction (15) */
            if (pSubSrc.nSrc == 0) return 0; /* Restriction (7)  */
            if ((pSub.selFlags & SF_Distinct) != 0 || (pSub.pLimit != null
                                                       && (pSrc.nSrc > 1 || isAgg)))
                /* Restrictions (4)(5)(8)(9) */
                return 0;
            if ((p.selFlags & SF_Distinct) != 0 && subqueryIsAgg) return 0; /* Restriction (6)  */
            if (p.pOrderBy != null && pSub.pOrderBy != null) return 0; /* Restriction (11) */
            ...
            */
            ...
            if (pSub.pPrior != null)
            {
                if (pSub.pOrderBy != null) return 0; /* Restriction 20 */
                if (isAgg || (p.selFlags & SF_Distinct) != 0 || pSrc.nSrc != 1) return 0;
...
//        if ( db.mallocFailed != 0 ) return 1;
            ...
            for (pParent = p; pParent != null; pParent = pParent.pPrior, pSub = pSub.pPrior)
            {
                 ...
                 */
                if (nSubSrc > 1) pParent.pSrc = pSrc = sqlite3SrcListEnlarge(db, pSrc, nSubSrc - 1, iFrom + 1);
                //if ( db.mallocFailed != 0 )
                //{
                //  break;
                //}
                ...
                for (i = 0; i < pList.nExpr; i++)
                    ...
                    {
                        var zSpan = pList.a[i].zSpan;
                        if (ALWAYS(zSpan)) pList.a[i].zName = zSpan; // sqlite3DbStrDup( db, zSpan );
                    }
        ...
        private static u8 minMaxQuery(Select p)
        {
            ...
            if (string.Compare(pExpr.u.zToken, "max", true) == 0) //sqlite3StrICmp(pExpr->u.zToken,"max")==0 )
        ...
        private static int selectExpander(Walker pWalker, Select p)
        {
            ...
            //  return WRC_Abort;
            //}
            if (NEVER(p.pSrc == null) || (p.selFlags & SF_Expanded) != 0) return WRC_Prune;
            ...
            for (i = 0; i < pTabList.nSrc; i++) // pFrom++ )
            {
                ...
                if (sqlite3IndexedByLookup(pParse, pFrom) != 0) return WRC_Abort;
            }

            /* Process NATURAL keywords, and ON and USING clauses of joins.
            */
            if ( /* db.mallocFailed != 0 || */ sqliteProcessJoin(pParse, p) != 0) return WRC_Abort;
            ...
            if (k < pEList.nExpr)
            {
                ...
                var a = pEList.a;
                ExprList pNew = null;
                var flags = pParse.db.flags;
                var longNames = (flags & SQLITE_FullColNames) != 0
                ...
                for (k = 0; k < pEList.nExpr; k++)
                {
                    ...
                    else
                    {
                         ...
                         ** expanded. */
                        var tableSeen = 0; /* Set to 1 when TABLE matches */
                        ...
                        for (i = 0; i < pTabList.nSrc; i++) //, pFrom++ )
                        {
                            pFrom = pTabList.a[i];
                            var pTab = pFrom.pTab;
                            var zTabName = pFrom.zAlias;
                            if (zTabName == null) zTabName = pTab.zName;
                            ///if ( db.mallocFailed != 0 ) break;
                            if (zTName != null && sqlite3StrICmp(zTName, zTabName) != 0) continue;
                            tableSeen = 1;
                            for (j = 0; j < pTab.nCol; j++)
                            {
                                Expr pExpr, pRight;
                                var zName = pTab.aCol[j].zName;
...
sqlite3ErrorMsg(pParse, "too many columns in result set");
        ...
        private static void resetAccumulator(Parse pParse, AggInfo pAggInfo)
        {
            ...
            for (i = 0; i < pAggInfo.nColumn; i++) sqlite3VdbeAddOp2(v, OP_Null, 0, pAggInfo.aCol[i].iMem);
            for (i = 0; i < pAggInfo.nFunc; i++)
            {
                ...
                if (pFunc.iDistinct >= 0)
                {
                    ...
                    {
                        var pKeyInfo = keyInfoFromExprList(pParse, pE.x.pList);
        ...
        private static void updateAccumulator(Parse pParse, AggInfo pAggInfo)
        {
            ...
            for (i = 0; i < pAggInfo.nFunc; i++)
            {
                ...
                if ((pF.pFunc.flags & SQLITE_FUNC_NEEDCOLL) != 0)
                {
                    ...
                    if (pColl == null) pColl = pParse.db.pDfltColl;
        ...
        )
        {
            ...
            var rc = 1; /* Value to return from this function */
            ...
            db = pParse.db;
            if (p == null /*|| db.mallocFailed != 0 */ || pParse.nErr != 0) return 1;
...
if (sqlite3AuthCheck(pParse, SQLITE_SELECT, 0, 0, 0)) return 1;
            ...
            pTabList = p.pSrc;
            pEList = p.pEList;
            if (pParse.nErr != 0 /*|| db.mallocFailed != 0 */) goto select_end;
            ...
            */
            ...
            {
                var pItem = pTabList.a[i];
                var dest = new SelectDest();
                var pSub = pItem.pSelect;
                bool isAggSub;
      ...
      */
            ...
            */
            if (pDest.eDest == SRT_EphemTab) sqlite3VdbeAddOp2(v, OP_OpenEphemeral, pDest.iParm, pEList.nExpr);
            ...
            */
            ...
            else
            {
                ...
                if (pHaving != null) sqlite3ExprAnalyzeAggregates(sNC, ref pHaving);
                    ...
                    else
                    {
                        ...
                        for (i = 0; i < sAggInfo.nColumn; i++)
                        {
                            ...
                            if (pCol.iSorterColumn >= j)
                            {
                                ...
                                if (r1 != r2) sqlite3VdbeAddOp2(v, OP_SCopy, r2, r1);
                                j++;
                            }
                        }
          ...
          */
                ...
                else
                {
                    ...
                    if ((pTab = isSimpleCount(p, sAggInfo)) != null)
                    {
                        ...
                        if (pKeyInfo != null) sqlite3VdbeChangeP4(v, -1, pKeyInfo, P4_KEYINFO_HANDOFF);
             ...
             */
            if (pOrderBy != null) generateSortTail(pParse, p, v, pEList.nExpr, pDest);
            ...
            */
            if (rc == SQLITE_OK && pDest.eDest == SRT_Output) generateColumnNames(pParse, pTabList, pEList);
