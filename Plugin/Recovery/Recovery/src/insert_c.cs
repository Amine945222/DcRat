...
namespace CS_SQLite3
{
    public partial class CSSQLite
    {
         ...
         */
        private static void sqlite3IndexAffinityStr(Vdbe v, Index pIdx)
        {
            if (pIdx.zColAff == null || pIdx.zColAff[0] == '\0')
            {
                 ...
                 */
                int n;
                var pTab = pIdx.pTable;
                var db = sqlite3VdbeDb(v);
                var pIdx_zColAff = new StringBuilder(pIdx.nColumn + 2); // (char *)sqlite3Malloc(pIdx->nColumn+2);
                if (pIdx_zColAff == null)
                    ////        db.mallocFailed = 1;
                    return;
                for (n = 0; n < pIdx.nColumn; n++) pIdx_zColAff.Append(pTab.aCol[pIdx.aiColumn[n]].affinity);
        ...
        private static void sqlite3TableAffinityStr(Vdbe v, Table pTab)
        {
            ...
            if (pTab.zColAff == null)
            {
                ...
                for (i = 0; i < pTab.nCol; i++) zColAff.Append(pTab.aCol[i].affinity);
                //zColAff.Append( '\0' );
        ...
        private static bool readsTable(Parse p, int iStartAddr, int iDb, Table pTab)
        {
            ...
            int i;
            var iEnd = sqlite3VdbeCurrentAddr(v);
  ...
  VTable pVTab = IsVirtual(pTab) ? sqlite3GetVTable(p,db, pTab) : null;
            ...
            {
                var pOp = sqlite3VdbeGetOp(v, i);
                Debug.Assert(pOp != null);
                if (pOp.opcode == OP_OpenRead && pOp.p3 == iDb)
                {
                    Index pIndex;
                    var tnum = pOp.p2;
                    if (tnum == pTab.tnum) return true;
...
return true;
        ...
        {
            var memId = 0; /* Register holding maximum rowid */
        ...
        private static void sqlite3AutoincrementBegin(Parse pParse)
        {
            ...
            var db = pParse.db; /* The database connection */
            Db pDb; /* Database only autoinc table */
            int memId; /* Register holding max rowid */
            int addr; /* A VDBE address */
            var v = pParse.pVdbe; /* VDBE under construction */
        ...
        {
            if (memId > 0) sqlite3VdbeAddOp2(pParse.pVdbe, OP_MemMax, memId, regRowid);
        }
...
//# define autoIncStep(A,B,C)
        ...
        private static void sqlite3Insert(Parse pParse, SrcList pTabList, int null_3, int null_4, IdList pColumn,
            int onError)
        {
            sqlite3Insert(pParse, pTabList, null, null, pColumn, onError);
        }

        private static void sqlite3Insert(Parse pParse, SrcList pTabList, int null_3, Select pSelect, IdList pColumn,
            int onError)
        {
            sqlite3Insert(pParse, pTabList, null, pSelect, pColumn, onError);
        }

        private static void sqlite3Insert(Parse pParse, SrcList pTabList, ExprList pList, int null_4, IdList pColumn,
            int onError)
        {
        ...
        )
        {
            ...
            var i = 0;
            var j = 0;
            var idx = 0; /* Loop counters */
            Vdbe v; /* Generate code into this virtual machine */
            Index pIdx; /* For looping over indices of the table */
            int nColumn; /* Number of columns in the data */
            var nHidden = 0; /* Number of hidden columns if TABLE is virtual */
            var baseCur = 0; /* VDBE VdbeCursor number for pTab */
            var keyColumn = -1; /* Column that is the INTEGER PRIMARY KEY */
            var endOfLoop = 0; /* Label for the end of the insertion loop */
            var useTempTable = false; /* Store SELECT results in intermediate table */
            var srcTab = 0; /* Data comes from this temporary cursor if >=0 */
            var addrInsTop = 0; /* Jump to label "D" */
            var addrCont = 0; /* Top of insert loop. Label "C" in templates 3 and 4 */
            var addrSelect = 0; /* Address of coroutine that implements the SELECT */
            SelectDest dest; /* Destination for SELECT on rhs of INSERT */
            var newIdx = -1; /* VdbeCursor for the NEW pseudo-table */
            int iDb; /* Index of database holding TABLE */
            Db pDb; /* The database containing table being inserted into */
            var appendFlag = false; /* True if the insert is likely to be an append */

            /* Register allocations */
            var regFromSelect = 0; /* Base register for data coming from SELECT */
            var regAutoinc = 0; /* Register holding the AUTOINCREMENT counter */
            var regRowCount = 0; /* Memory cell used for the row counter */
            int regIns; /* Block of regs holding rowid+data being inserted */
            int regRowid; /* registers holding insert rowid */
            int regData; /* register holding first column to insert */
            int regRecord; /* Holds the assemblied row record */
            var regEof = 0; /* Register recording end of SELECT data */
            int[] aRegIdx = null; /* One register allocated to each index */


#if !SQLITE_OMIT_TRIGGER
            var isView = false; /* True if attempting to insert into a view */
            Trigger pTrigger; /* List of triggers on pTab, if required */
            var tmask = 0; /* Mask of trigger times */
#endif

            db = pParse.db;
            dest = new SelectDest(); // memset( &dest, 0, sizeof( dest ) );

            if (pParse.nErr != 0 /*|| db.mallocFailed != 0 */) goto insert_cleanup;
            ...
            */
            ...
            if (pTab == null) goto insert_cleanup;
...
isView = false;
             ...
             ** module table).
             */
            if (sqlite3ViewGetColumnNames(pParse, pTab) != -0) goto insert_cleanup;
#endif
             ...
             */
            if (sqlite3IsReadOnly(pParse, pTab, tmask)) goto insert_cleanup;
            ...
            */
            ...
            if (pTrigger != null) newIdx = pParse.nTab++;
        ...
        VdbeComment( v, "Jump over SELECT coroutine" );
                ...
                if (rc != 0 || NEVER(pParse.nErr != 0) /*|| db.mallocFailed != 0 */) goto insert_cleanup;
        ...
        VdbeComment( v, "End of SELECT coroutine" );
                 ...
                 */
                if (pTrigger != null || readsTable(pParse, addrSelect, iDb, pTab)) useTempTable = true;
            ...
            if (pColumn != null)
            {
                for (i = 0; i < pColumn.nId; i++) pColumn.a[i].idx = -1;
             ...
             */
            if (pColumn == null && nColumn > 0) keyColumn = pTab.iPKey;

            /* Open the temp table for FOR EACH ROW triggers
            */
            if (pTrigger != null) sqlite3VdbeAddOp3(v, OP_OpenPseudo, newIdx, 0, pTab.nCol);
            ...
            */
            ...
            if (!isView)
            {
                ...
                if (aRegIdx == null) goto insert_cleanup;
                for (i = 0; i < nIdx; i++) aRegIdx[i] = ++pParse.nMem;
            }
            ...
            */
            ...
            if ((tmask & TRIGGER_BEFORE) != 0)
            {
                ...
                */
                 ...
                 */
                if (!isView) sqlite3TableAffinityStr(v, pTab);
    ...
    if( IsVirtual(pTab) ){
      ...
      sqlite3VdbeAddOp4(v, OP_VUpdate, 1, pTab->nCol+2, regIns, pVTab, P4_VTAB);
...
#endif
                {
                    var isReplace = 0; /* Set to true if constraints may cause a replace */
            ...
            */
            if ((db.flags & SQLITE_CountRows) != 0) sqlite3VdbeAddOp2(v, OP_AddImm, regRowCount, 1);
             ...
             */
            if (pParse.nested == 0 && pParse.trigStack == null) sqlite3AutoincrementEnd(pParse);
        ...
        )
        {
            ...
            var seenReplace = false; /* True if REPLACE is used to resolve INT PK conflict */
            var hasTwoRowids = isUpdate && rowidChng;
            ...
            */
            for (i = 0; i < nCol; i++)
            {
                ...
                if (onError == OE_Replace && pTab.aCol[i].pDflt == null) onError = OE_Abort;
            ...
            */
            ...
            {
                var allOk = sqlite3VdbeMakeLabel(v);
            ...
            if (rowidChng)
            {
                ...
                {
                    if (isUpdate) j2 = sqlite3VdbeAddOp3(v, OP_Eq, regRowid, 0, regRowid - 1);
                    ...
                    if (isUpdate) sqlite3VdbeJumpHere(v, j2);
                }
            }
            ...
            for (iCur = 0, pIdx = pTab.pIndex; pIdx != null; pIdx = pIdx.pNext, iCur++)
            {
                ...
                switch (onError)
                {
                    ...
                    case OE_Fail:
                    {
                        ...
                        {
                            var zCol = pTab.aCol[pIdx.aiColumn[j]].zName;
        ...
        )
        {
            ...
            for (i = nIdx - 1; i >= 0; i--)
            {
                ...
                if (useSeekResult) sqlite3VdbeChangeP5(v, OPFLAG_USESEEKRESULT);
            }
            ...
            if (pParse.nested == 0) sqlite3VdbeChangeP4(v, -1, pTab.zName, P4_STATIC);
        ...
        )
        {
            ...
            {
                var pKey = sqlite3IndexKeyinfo(pParse, pIdx);
        ...
        VdbeComment( v, "%s", pIdx.zName );
            ...
            if (pParse.nTab < baseCur + i) pParse.nTab = baseCur + i;
            return i - 1;
        }
...
*/
    //static int sqlite3_xferopt_count = 0;
        ...
        private static bool xferCompatibleIndex(Index pDest, Index pSrc)
        {
            ...
            if (pDest.nColumn != pSrc.nColumn) return false; /* Different number of columns */
            if (pDest.onError != pSrc.onError) return false; /* Different conflict resolution strategies */
            for (i = 0; i < pSrc.nColumn; i++)
            {
                if (pSrc.aiColumn[i] != pDest.aiColumn[i]) return false; /* Different columns indexed */
                if (pSrc.aSortOrder[i] != pDest.aSortOrder[i]) return false; /* Different sort orders */
                if (!xferCompatibleCollation(pSrc.azColl[i],
                        pDest.azColl[i])) return false; /* Different collating sequences */
            }
        ...
        )
        {
            ...
            var destHasUniqueIdx = false; /* True if pDest has a UNIQUE index */
            int regData, regRowid; /* Registers holding data and rowid */

            if (pSelect == null) return 0; /* Must be of the form  INSERT INTO ... SELECT ... */
#if !SQLITE_OMIT_TRIGGER
            if (sqlite3TriggerList(pParse, pDest) != null) return 0; /* tab1 must not have triggers */
#endif

            if ((pDest.tabFlags & TF_Virtual) != 0) return 0; /* tab1 must not be a virtual table */
            if (onError == OE_Default) onError = OE_Abort;
            if (onError != OE_Abort &&
                onError != OE_Rollback) return 0; /* Cannot do OR REPLACE or OR IGNORE or OR FAIL */
            Debug.Assert(pSelect.pSrc != null); /* allocated even if there is no FROM clause */
            if (pSelect.pSrc.nSrc != 1) return 0; /* FROM clause must have exactly one term */
            if (pSelect.pSrc.a[0].pSelect != null) return 0; /* FROM clause cannot contain a subquery */
            if (pSelect.pWhere != null) return 0; /* SELECT may not have a WHERE clause */
            if (pSelect.pOrderBy != null) return 0; /* SELECT may not have an ORDER BY clause */
            /* Do not need to test for a HAVING clause.  If HAVING is present but
             ** there is no ORDER BY, we will get an error. */
            if (pSelect.pGroupBy != null) return 0; /* SELECT may not have a GROUP BY clause */
            if (pSelect.pLimit != null) return 0; /* SELECT may not have a LIMIT clause */
            Debug.Assert(pSelect.pOffset == null); /* Must be so if pLimit==0 */
            if (pSelect.pPrior != null) return 0; /* SELECT may not be a compound query */
            if ((pSelect.selFlags & SF_Distinct) != 0) return 0; /* SELECT may not be DISTINCT */
            pEList = pSelect.pEList;
            Debug.Assert(pEList != null);
            if (pEList.nExpr != 1) return 0; /* The result set must have exactly one column */
            Debug.Assert(pEList.a[0].pExpr != null);
            if (pEList.a[0].pExpr.op != TK_ALL) return 0; /* The result set must be the special operator "*" */
            ...
            if (pSrc == null) return 0; /* FROM clause does not contain a real table */
            if (pSrc == pDest) return 0; /* tab1 and tab2 may not be the same table */
            if ((pSrc.tabFlags & TF_Virtual) != 0) return 0; /* tab2 must not be a virtual table */
            if (pSrc.pSelect != null) return 0; /* tab2 may not be a view */
            if (pDest.nCol != pSrc.nCol) return 0; /* Number of columns must be the same in tab1 and tab2 */
            if (pDest.iPKey != pSrc.iPKey) return 0; /* Both tables must have the same INTEGER PRIMARY KEY */
            for (i = 0; i < pDest.nCol; i++)
            {
                if (pDest.aCol[i].affinity !=
                    pSrc.aCol[i].affinity) return 0; /* Affinity must be the same on all columns */
                if (!xferCompatibleCollation(pDest.aCol[i].zColl,
                        pSrc.aCol[i].zColl)) return 0; /* Collating sequence must be the same on all columns */
                if (pDest.aCol[i].notNull != 0 &&
                    pSrc.aCol[i].notNull == 0) return 0; /* tab2 must be NOT NULL if tab1 is */
            }

            for (pDestIdx = pDest.pIndex; pDestIdx != null; pDestIdx = pDestIdx.pNext)
            {
                if (pDestIdx.onError != OE_None) destHasUniqueIdx = true;
                for (pSrcIdx = pSrc.pIndex; pSrcIdx != null; pSrcIdx = pSrcIdx.pNext)
                    if (xferCompatibleIndex(pDestIdx, pSrcIdx))
                        break;
                if (pSrcIdx == null) return 0; /* pDestIdx has no corresponding index in pSrc */
            }
#if !SQLITE_OMIT_CHECK
            if (pDest.pCheck != null &&
                !sqlite3ExprCompare(pSrc.pCheck,
                    pDest.pCheck)) return 0; /* Tables have different CHECK constraints.  Ticket #2252 */
#endif
