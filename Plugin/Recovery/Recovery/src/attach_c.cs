...
namespace CS_SQLite3
{
    ...
    public partial class CSSQLite
    {
        ...
        )
        {
            while (pStep != null)
            {
                if (sqlite3FixSelect(pFix, pStep.pSelect) != 0) return 1;
                if (sqlite3FixExpr(pFix, pStep.pWhere) != 0) return 1;
                if (sqlite3FixExprList(pFix, pStep.pExprList) != 0) return 1;
                pStep = pStep.pNext;
            }
...
break;
            ...
            if (rc != 0)
            {
                ...
                else if (zErrDyn == "") zErrDyn = sqlite3MPrintf(db, "unable to open database: %s", zFile);
                goto attach_error;
            }
            ...
            if (zErrDyn != "") sqlite3_result_error(context, zErrDyn, -1);
        ...
        {
            string zName =
                zName = argv[0].z != null && argv[0].z.Length > 0
...
goto attach_end;
        ...
        )
        {
            while (pSelect != null)
            {
                if (sqlite3FixExprList(pFix, pSelect.pEList) != 0) return 1;
                if (sqlite3FixSrcList(pFix, pSelect.pSrc) != 0) return 1;
                if (sqlite3FixExpr(pFix, pSelect.pWhere) != 0) return 1;
                if (sqlite3FixExpr(pFix, pSelect.pHaving) != 0) return 1;
        ...
        )
        {
            while (pExpr != null)
            {
                ...
                if (sqlite3FixExpr(pFix, pExpr.pRight) != 0) return 1;
                pExpr = pExpr.pLeft;
            }
        ...
        )
        {
            ...
            {
                pItem = pList.a[i];
                if (sqlite3FixExpr(pFix, pItem.pExpr) != 0) return 1;
            }
