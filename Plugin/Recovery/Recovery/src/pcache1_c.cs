    ...
    public partial class CSSQLite
    {
        ...
        private static PgHdr pcache1Alloc(int nByte)
        {
            ...
            else
            {
                ...
                pcache1EnterMutex();
                //  if( p !=null){
                var sz = nByte; //int sz = sqlite3MallocSize(p);
        ...
        private static void pcache1Free(ref PgHdr p)
        {
            ...
            {
                var iSize = p.pData.Length; //sqlite3MallocSize( p );
        ...
        private static PgHdr1 pcache1AllocPage(PCache1 pCache)
        {
            ...
            var pPg = pcache1Alloc(pCache.szPage);
            PgHdr1 p;
            //if ( pPg != null )
            {
                ...
                p.pCache = pCache;
                p.pPgHdr = pPg;
                if (pCache.bPurgeable) pcache1.nCurrentPage++;
            }
            //else
            //{
            //  p = null;
        ...
        private static void pcache1FreePage(ref PgHdr1 p)
        {
            if (ALWAYS(p != null))
            {
                if (p.pCache.bPurgeable) pcache1.nCurrentPage--;
        ...
        private static void pcache1PinPage(PgHdr1 pPage)
        {
            ...
            {
                if (pPage.pLruPrev != null) pPage.pLruPrev.pLruNext = pPage.pLruNext;
                if (pPage.pLruNext != null) pPage.pLruNext.pLruPrev = pPage.pLruPrev;
                if (pcache1.pLruHead == pPage) pcache1.pLruHead = pPage.pLruNext;
                if (pcache1.pLruTail == pPage) pcache1.pLruTail = pPage.pLruPrev;
                pPage.pLruNext = null;
                pPage.pLruPrev = null;
      ...
      Debug.Assert( pCache.nPage == nPage );
        ...
        private static int pcache1Init(object NotUsed)
        {
            ...
            if (sqlite3GlobalConfig.bCoreMutex) pcache1.mutex = sqlite3_mutex_alloc(SQLITE_MUTEX_STATIC_LRU);
            pcache1.isInit = 1;
            return SQLITE_OK;
        }
         ...
         */
        private static PgHdr pcache1Fetch(sqlite3_pcache p, u32 iKey, int createFlag)
        {
            u32 nPinned;
            var pCache = p;
            PgHdr1 pPage = null;
            ...
            if (pCache.nPage >= pCache.nHash && pcache1ResizeHash(pCache) != 0) goto fetch_out;
             ...
             */
            if (null == pPage) pPage = pcache1AllocPage(pCache);
            ...
            fetch_out:
            if (pPage != null && iKey > pCache.iMaxKey) pCache.iMaxKey = iKey;
        ...
        )
        {
             ...
             */
            if (NEVER(iNew > pCache.iMaxKey)) pCache.iMaxKey = iNew;
         ...
         */
        private static void pcache1Truncate(sqlite3_pcache p, u32 iLimit)
        {
            var pCache = p;
            pcache1EnterMutex();
