        ...
        */
        ...
        )
        {
            ...
            do
            {
                ...
                var nCopy = MIN(nRead, JOURNAL_CHUNKSIZE - iChunkOffset);
        ...
        )
        {
            ...
            while (nWrite > 0)
            {
                ...
                var iChunkOffset = p.endpoint.iOffset % JOURNAL_CHUNKSIZE;
                var iSpace = MIN(nWrite, JOURNAL_CHUNKSIZE - iChunkOffset);

                if (iChunkOffset == 0)
                {
                    /* New chunk is required to extend the file. */
                    var pNew = new FileChunk(); // sqlite3_malloc( sizeof( FileChunk ) );
                    if (null == pNew) return SQLITE_IOERR_NOMEM;
                    pNew.pNext = null;
                    if (pChunk != null)
                    {
