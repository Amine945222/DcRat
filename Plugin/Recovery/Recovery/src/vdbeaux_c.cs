...
namespace CS_SQLite3
{
    ...
    static bool sqlite3VdbeAddopTrace = false;
        ...
        private static Vdbe sqlite3VdbeCreate(sqlite3 db)
        {
            ...
            p.db = db;
            if (db.pVdbe != null) db.pVdbe.pPrev = p;
            p.pNext = db.pVdbe;
            p.pPrev = null;
            db.pVdbe = p;
...
if( !isPrepareV2 ) return;
         ...
         */
        private static string sqlite3_sql(sqlite3_stmt pStmt)
        {
            var p = pStmt;
...
*/
    static void sqlite3VdbeTrace( Vdbe p, FILE trace )
    {
      p.trace = trace;
        ...
        {
            //VdbeOp pNew;
            var nNew = p.nOpAlloc != 0 ? p.nOpAlloc * 2 : 1024 / 4; //(int)(1024/sizeof(Op)));
...
pOp.cnt = 0;
        ...
        {
            return sqlite3VdbeAddOp2(p, op, p1, b2 ? 1 : 0);
        }
        ...
        private static int sqlite3VdbeAddOp4(Vdbe p, int op, int p1, int p2, int p3, i32 pP4, int p4type)
        {
            ...
            _p4.i = pP4;
            var addr = sqlite3VdbeAddOp3(p, op, p1, p2, p3);
        ...
        private static void sqlite3VdbeResolveLabel(Vdbe p, int x)
        {
            ...
            if (p.aLabel != null) p.aLabel[j] = p.nOp;
        }
        ...
        private static void resolveP2Values(Vdbe p, ref int pMaxFuncArgs)
        {
            ...
            Op pOp;
            var aLabel = p.aLabel;
            var doesStatementRollback = false;
            var hasStatementBegin = false;
            p.readOnly = true;
            ...
            for (i = 0; i < p.nOp; i++)
            {
                ...
                if (opcode == OP_Halt)
                {
                    if (pOp.p1 == SQLITE_CONSTRAINT && pOp.p2 == OE_Abort) doesStatementRollback = true;
                }
...
if( n>nMaxArgs ) nMaxArgs = n;
        ...
        private static int sqlite3VdbeAddOpList(Vdbe p, int nOp, VdbeOpList[] aOp)
        {
            int addr;
            Debug.Assert(p.magic == VDBE_MAGIC_INIT);
            if (p.nOp + nOp > p.nOpAlloc && growOpArray(p) != 0) return 0;
            addr = p.nOp;
            if (ALWAYS(nOp > 0))
            {
                int i;
        ...
        private static void sqlite3VdbeChangeP1(Vdbe p, int addr, int val)
        {
            Debug.Assert(p != null);
            Debug.Assert(addr >= 0);
            if (p.nOp > addr) p.aOp[addr].p1 = val;
        }
        ...
        private static void sqlite3VdbeChangeP2(Vdbe p, int addr, int val)
        {
            Debug.Assert(p != null);
            Debug.Assert(addr >= 0);
            if (p.nOp > addr) p.aOp[addr].p2 = val;
        }
        ...
        private static void sqlite3VdbeChangeP3(Vdbe p, int addr, int val)
        {
            Debug.Assert(p != null);
            Debug.Assert(addr >= 0);
            if (p.nOp > addr) p.aOp[addr].p3 = val;
        }
        ...
        {
            if (ALWAYS(pDef) && (pDef.flags & SQLITE_FUNC_EPHEM) != 0) pDef = null;
        ...
        private static void sqlite3VdbeChangeP4(Vdbe p, int addr, union_p4 _p4, int n)
        {
            ...
            {
                if (n != P4_KEYINFO && n != P4_VTAB) freeP4(db, n, _p4);
                return;
            }
        ...
        private static string displayP4(Op pOp, string zBuffer, int nTemp)
        {
            ...
            switch (pOp.p4type)
            {
                ...
                case P4_KEYINFO:
                {
                    ...
                    {
                        var pColl = pKeyInfo.aColl[j];
                        if (pColl != null)
                        {
                            var n = sqlite3Strlen30(pColl.zName);
                            if (i + n > nTemp)
                            {
                            ...
                            if (pKeyInfo.aSortOrder != null &&
                                pKeyInfo.aSortOrder[j] != 0) zTemp.Append("-"); // zTemp[i++] = '-';
                ...
                case P4_MEM:
                {
                    ...
                    else if ((pMem.flags & MEM_Real) != 0) sqlite3_snprintf(nTemp, ref zTemp, "%.16g", pMem.r);
                    break;
                }
#if ! SQLITE_OMIT_VIRTUALTABLE
case P4_VTAB: {
      ...
      //fflush(pOut);
        ...
        private static void releaseMemArray(Mem[] p, int N)
        {
            if (p != null && p[0] != null && N != 0)
            {
                ...
                for (var i = 0; i < N; i++) //pEnd =  p[N] ; p < pEnd ; p++ )
                {
                    pEnd = p[i];
                     ...
                     */
                    if ((pEnd.flags & (MEM_Agg | MEM_Dyn)) != 0) sqlite3VdbeMemRelease(pEnd);
        ...
        )
        {
            ...
            */
            ...
            if (nVar >= 0 /* &&  ALWAYS(db->mallocFailed==0) */)
            {
                ...
                int nByte;
                var nArg = 0; /* Maximum number of args passed to a user function. */
                resolveP2Values(p, ref nArg);
                if (isExplain != 0 && nMem < 10) nMem = 10;
...
p.aOp[i].cycles = 0;
        ...
        private static void sqlite3VdbeFreeCursor(Vdbe p, VdbeCursor pCx)
        {
             ...
             ** the call above. */
            else if (pCx.pCursor != null) sqlite3BtreeCloseCursor(pCx.pCursor);
#if ! SQLITE_OMIT_VIRTUALTABLE
if( pCx.pVtabCursor ){
...
p.inVtabMethod = 0;
        ...
        private static void Cleanup(Vdbe p)
        {
            ...
            {
                pMem = p.aMem[1];
                if ((pMem.flags & MEM_RowSet) != 0) sqlite3RowSetClear(pMem.u.pRowSet);
        ...
        {
            int i;
            var nTrans = 0; /* Number of databases with an active write-transaction */
            var rc = SQLITE_OK;
            var needXcommit = false;
            ...
            if (needXcommit && db.xCommitCallback != null)
            {
                ...
                sqlite3SafetyOn(db);
                if (rc != 0) return SQLITE_CONSTRAINT;
            }
                ...
                || nTrans <= 1)
            {
                ...
                {
                    var pBt = db.aDb[i].pBt;
                    if (pBt != null) rc = sqlite3BtreeCommitPhaseOne(pBt, null);
                }
                ...
                {
                    var pBt = db.aDb[i].pBt;
                    if (pBt != null) rc = sqlite3BtreeCommitPhaseTwo(pBt);
                }
            ...
            else
            {
                var pVfs = db.pVfs;
                var needSync = false;
                var zMaster = ""; /* File-name for the master journal */
                var zMainFile = sqlite3BtreeGetFilename(db.aDb[0].pBt);
                ...
                for (i = 0; i < db.nDb; i++)
                {
                    ...
                    if (sqlite3BtreeIsInTrans(pBt))
                    {
                        var zFile = sqlite3BtreeGetJournalname(pBt);
                        if (zFile[0] == 0) continue; /* Ignore :memory: databases */
                        if (!needSync && 0 == sqlite3BtreeSyncDisabled(pBt)) needSync = true;
                ...
                {
                    var pBt = db.aDb[i].pBt;
                    if (pBt != null) rc = sqlite3BtreeCommitPhaseOne(pBt, zMaster);
                }
        ...
        disable_simulated_io_errors();
                ...
                {
                    var pBt = db.aDb[i].pBt;
                    if (pBt != null) sqlite3BtreeCommitPhaseTwo(pBt);
                }
...
//#define checkActiveVdbeCnt(x)
        ...
        private static void invalidateCursorsOnModifiedBtrees(sqlite3 db)
        {
            ...
            {
                var p = db.aDb[i].pBt;
                if (p != null && sqlite3BtreeIsInTrans(p)) sqlite3BtreeTripAllCursors(p, SQLITE_ABORT);
            }
        }
        ...
        private static int sqlite3VdbeCloseStatement(Vdbe p, int eOp)
        {
            ...
            if (db.nStatement != 0 && p.iStatement != 0)
            {
                ...
                for (i = 0; i < db.nDb; i++)
                {
                    ...
                    if (pBt != null)
                    {
                        if (eOp == SAVEPOINT_ROLLBACK) rc2 = sqlite3BtreeSavepoint(pBt, SAVEPOINT_ROLLBACK, iSavepoint);
                        if (rc2 == SQLITE_OK) rc2 = sqlite3BtreeSavepoint(pBt, SAVEPOINT_RELEASE, iSavepoint);
...
sqlite3BtreeEnterAll(p.db);
        ...
        private static int sqlite3VdbeHalt(Vdbe p)
        {
            ...
            //}
            closeAllCursors(p);
            if (p.magic != VDBE_MAGIC_RUN) return SQLITE_OK;
            ...
            if (p.pc >= 0)
            {
                ...
                var eStatementOp = 0;
                var isSpecialError = false; /* Set to true if a 'special' error */
             ...
             */
            if (db.autoCommit != 0) sqlite3ConnectionUnlocked(db);
...
fclose(out);
        ...
        private static void sqlite3VdbeDeleteAuxData(VdbeFunc pVdbeFunc, int mask)
        {
            ...
            {
                var pAux = pVdbeFunc.apAux[i];
                if (i > 31 || ((mask & ((u32)1 << i)) == 0 && pAux.pAux != null))
                {
                    if (pAux.xDelete != null) pAux.xDelete(ref pAux.pAux);
                    pAux.pAux = null;
                }
            }
        }
        ...
        sqlite3_search_count.iValue++;
            ...
            {
                var hasMoved = 0;
                var rc = sqlite3BtreeCursorHasMoved(p.pCursor, ref hasMoved);
                if (rc != 0) return rc;
                if (hasMoved != 0)
                {
                    p.cacheStatus = CACHE_STALE;
        ...
        private static u32 sqlite3VdbeSerialTypeLen(u32 serial_type)
        {
            ...
            u32[] aSize = { 0, 1, 2, 3, 4, 6, 8, 8, 0, 0, 0, 0 };
...
//# define swapMixedEndianFloat(X)  X = floatSwap(X)
        ...
        {
            var serial_type = sqlite3VdbeSerialType(pMem, file_format);
            u32 len;
...
swapMixedEndianFloat( v );
            ...
            if (serial_type >= 12)
            {
                Debug.Assert(pMem.n + ((pMem.flags & MEM_Zero) != 0 ? pMem.u.nZero : 0) ==
        ...
        )
        {
            switch (serial_type)
            {
                ...
                case 5:
                {
                    ...
                    var y = (u32)((buf[offset + 2] << 24) | (buf[offset + 3] << 16) | (buf[offset + 4] << 8) |
                                  buf[offset + 5]);
                    x = (x << 32) | y;
                    pMem.u.i = (i64)x;
                    pMem.flags = MEM_Int;
        ...
        )
        {
            switch (serial_type)
            {
                ...
                case 5:
                {
                    ...
                    var y = (u32)((buf[2] << 24) | (buf[3] << 16) | (buf[4] << 8) | buf[5]);
                    x = (x << 32) | y;
                    pMem.u.i = (i64)x;
                    pMem.flags = MEM_Int;
                    return 6;
...
swapMixedEndianFloat(x);
        ...
        private static void sqlite3VdbeDeleteUnpackedRecord(UnpackedRecord p)
        {
            ...
            //}
            if ((p.flags & UNPACKED_NEED_FREE) != 0) p = null; //sqlite3DbFree( p.pKeyInfo.db, ref p );
        }
        ...
        )
        {
            ...
            idx1 = (u32)((szHdr1 = aKey1[0]) <= 0x7f
                ? 1
                : getVarint32(aKey1, 0, ref szHdr1)); // GetVarint( aKey1, szHdr1 );
            d1 = (int)szHdr1;
            if ((pPKey2.flags & UNPACKED_IGNORE_ROWID) != 0) szHdr1--;
            ...
            while (idx1 < szHdr1 && i < pPKey2.nField)
            {
                ...
                idx1 += (u32)((serial_type1 = aKey1[idx1]) <= 0x7f
                    ? 1
                    : getVarint32(aKey1, idx1, ref serial_type1)); //GetVarint( aKey1 + idx1, serial_type1 );
        ...
        private static int sqlite3VdbeIdxRowid(sqlite3 db, BtCursor pCur, ref i64 rowid)
        {
            ...
            testcase(szHdr == 3);
            testcase(szHdr == m.n);
            if (unlikely(szHdr < 3 || (int)szHdr > m.n)) goto idx_rowid_corruption;
            ...
            if (unlikely(typeRowid < 1 || typeRowid > 9 || typeRowid == 7)) goto idx_rowid_corruption;
            lenRowid = sqlite3VdbeSerialTypeLen(typeRowid);
            testcase((u32)m.n == szHdr + lenRowid);
            if (unlikely((u32)m.n < szHdr + lenRowid)) goto idx_rowid_corruption;
      ...
      return SQLITE_CORRUPT_BKPT();
        ...
        {
            Vdbe p;
            for (p = db.pVdbe; p != null; p = p.pNext) p.expired = true;
        }
