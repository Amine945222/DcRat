...
namespace CS_SQLite3
{
    ...
    public partial class CSSQLite
    {
        ...
        {
            var bResult = sqlite3_value_text(argv[0]);
            var zSql = bResult == null ? "" : bResult;
            var zTableName = sqlite3_value_text(argv[1]);
        ...
        {
            var zSql = sqlite3_value_text(argv[0]);
            var zTableName = sqlite3_value_text(argv[1]);
         ...
         */
        private static void sqlite3AlterFunctions(sqlite3 db)
        {
        ...
        private static string whereTempTriggers(Parse pParse, Table pTab)
        {
            ...
            var zWhere = "";
            var tmp = "";
            var pTempSchema = pParse.db.aDb[1].pSchema; /* Temp db schema */
        ...
        private static void reloadTableSchema(Parse pParse, Table pTab, string zName)
        {
            ...
            for (pTrig = sqlite3TriggerList(pParse, pTab); pTrig != null; pTrig = pTrig.pNext)
            {
                var iTrigDb = sqlite3SchemaToIndex(pParse.db, pTrig.pSchema);
        ...
        )
        {
            ...
            Vdbe v;
#if !SQLITE_OMIT_TRIGGER
            var zWhere = ""; /* Where clause to locate temp triggers */
#endif
            ...
            if (SQLITE_OK != sqlite3CheckObjectName(pParse, zName)) goto exit_rename_table;
...
goto exit_rename_table;
            ...
            if (sqlite3ViewGetColumnNames(pParse, pTab) != 0) goto exit_rename_table;
...
return;
             ...
             */
            if (pDflt != null && pDflt.op == TK_NULL) pDflt = null;
