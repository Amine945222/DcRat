...
namespace CS_SQLite3
{
    ...
    using sqlite_int64 = sqlite3_int64;
    ...
    public partial class CSSQLite
    {
        ...
        )
        {
            ...
            switch (sqlite3_value_type(argv[0]))
            {
                ...
                case SQLITE_TEXT:
                {
                    var z = sqlite3_value_blob(argv[0]);
                    if (z == null) return;
                    len = 0;
                    var iz = 0;
        ...
        )
        {
            ...
            switch (sqlite3_value_type(argv[0]))
            {
                case SQLITE_INTEGER:
                {
                    var iVal = sqlite3_value_int64(argv[0]);
                    if (iVal < 0)
                    {
                        if (iVal << 1 == 0)
                        {
                ...
                default:
                {
                    var rVal = sqlite3_value_double(argv[0]);
                    if (rVal < 0) rVal = -rVal;
        ...
        )
        {
            ...
            if (p0type != SQLITE_BLOB)
            {
                ...
                //  SQLITE_SKIP_UTF8( ref z2 );
                //}
                sqlite3_result_text(context, z.Length == 0 || p1 > z.Length ? "" : z.Substring(p1, p2), p2,
                    SQLITE_TRANSIENT);
            }
            else
            {
                ...
                if (zBLOB.Length == 0 || p1 > zBLOB.Length) sb.Length = 0;
                    ...
                    for (var i = p1; i < p1 + p2; i++)
                        sb.Append((char)zBLOB[i]);

                sqlite3_result_blob(context, sb.ToString(), p2, SQLITE_TRANSIENT);
            }
        }
        ...
        )
        {
            ...
            if (z2 != null)
                ...
                //}
                sqlite3_result_text(context, z2.Length == 0 ? "" : z2.Substring(0, n).ToUpper(), -1,
                    null); //sqlite3_free );
            // }
        }
         ...
         ** argument.
         */
        private static void ifnullFunc(
            sqlite3_context context,
            int argc,
            sqlite3_value[] argv
        )
...
//# define GlogUpperToLower(A)     A = sqlite3UpperToLower[A]
        ...
        )
        {
            ...
            var noCase = pInfo.noCase;
            var prevEscape = false; /* True if the previous character was 'escape' */
            var inPattern = zPattern; //Entered Pattern
            ...
            while ((c = sqlite3Utf8Read(zPattern, ref zPattern)) != 0)
                if (!prevEscape && c == matchAll)
                {
                    ...
                    else if (c == matchSet)
                    {
                        ...
                        while (len < zString.Length &&
                               patternCompare(inPattern.Substring(inPattern.Length - zPattern.Length - 1),
                                   zString.Substring(len), pInfo, esc) == false) SQLITE_SKIP_UTF8(zString, ref len);
                    ...
                    while ((c2 = sqlite3Utf8Read(zString, ref zString)) != 0)
                    {
                        ...
                        {
                            while (c2 != 0 && c2 != c) c2 = sqlite3Utf8Read(zString, ref zString);
                        }
                ...
                {
                    if (sqlite3Utf8Read(zString, ref zString) == 0) return false;
                }
                else if (c == matchSet)
                {
                    var prior_c = 0;
                    ...
                    if (c2 == 0 || (seen ^ invert) == 0) return false;
                }
    ...
    //static int sqlite3_like_count = 0;
        ...
        )
        {
            ...
            if (argc == 3)
            {
                 ...
                 */
                var zEsc = sqlite3_value_text(argv[2]);
                if (zEsc == null) return;
        ...
        )
        {
            ...
            UNUSED_PARAMETER(NotUsed);
            if (sqlite3MemCompare(argv[0], argv[1], pColl) != 0) sqlite3_result_value(context, argv[0]);
        }
        ...
        )
        {
            ...
            switch (sqlite3_value_type(argv[0]))
            {
                ...
                {
                    StringBuilder zText;
                    var zBlob = sqlite3_value_blob(argv[0]);
                    var nBlob = sqlite3_value_bytes(argv[0]);
                ...
                case SQLITE_TEXT:
                {
                    int i, j;
                    int n;
                    var zArg = sqlite3_value_text(argv[0]);
                    StringBuilder z;
        ...
        )
        {
            ...
            {
                int[] lenOne = { 1 };
                byte[] azOne = { (u8)' ' }; //static unsigned char * const azOne[] = { (u8*)" " };
                nChar = 1;
                aLen = lenOne;
                azChar = new byte[1][];
                azChar[0] = azOne;
            ...
            else
            {
                ...
                var iz = 0;
                for (nChar = 0; iz < zBytes.Length; nChar++) SQLITE_SKIP_UTF8(zBytes, ref iz);
                if (nChar > 0)
                {
                    azChar = new byte[nChar][]; //contextMalloc(context, ((i64)nChar)*(sizeof(char*)+1));
                    if (azChar == null) return;
                    aLen = new int[nChar];

                    var iz0 = 0;
                    var iz1 = 0;
                    for (var ii = 0; ii < nChar; ii++)
                    {
...
sqlite3_result_text(context, "?000", 4, SQLITE_STATIC);
        ...
        )
        {
            ...
            Debug.Assert(argc == 1);
            UNUSED_PARAMETER(argc);
            var pMem = sqlite3_aggregate_context(context, -1); //sizeof(*p));
            ...
            if (p != null && type != SQLITE_NULL)
            {
                ...
                {
                    var v = sqlite3_value_int64(argv[0]);
                    p.rSum += v;
                    if (!(p.approx | (p.overflow != 0)))
                    {
                        var iNewSum = p.iSum + v;
                        var s1 = (int)(p.iSum >> (sizeof(i64) * 8 - 1));
                        var s2 = (int)(v >> (sizeof(i64) * 8 - 1));
                        var s3 = (int)(iNewSum >> (sizeof(i64) * 8 - 1));
        ...
        {
            SumCtx p = null;
            var pMem = sqlite3_aggregate_context(context, 0);
        ...
        private static void avgFinalize(sqlite3_context context)
        {
            ...
            var pMem = sqlite3_aggregate_context(context, 0);
            if (pMem != null) p = pMem._SumCtx;
            if (p != null && p.cnt > 0) sqlite3_result_double(context, p.rSum / p.cnt);
        }
        ...
        )
        {
            ...
            if ((argc == 0 || SQLITE_NULL != sqlite3_value_type(argv[0])) && p.Context != null) p.n++;
      ...
      || p.n == sqlite3_aggregate_count( context ) );
        ...
        )
        {
            ...
            if (pBest.flags != 0)
            {
                ...
                if ((max && cmp < 0) || (!max && cmp > 0)) sqlite3VdbeMemCopy(pBest, pArg);
            }
            else
            {
        ...
        private static void minMaxFinalize(sqlite3_context context)
        {
            ...
            if (pRes != null)
            {
                if (ALWAYS(pRes.flags != 0)) sqlite3_result_value(context, pRes);
        ...
        )
        {
            ...
            Debug.Assert(argc == 1 || argc == 2);
            if (sqlite3_value_type(argv[0]) == SQLITE_NULL) return;
            var pMem = sqlite3_aggregate_context(context, -1); //sizeof(*pAccum));
            ...
            {
                var db = sqlite3_context_db_handle(context);
                var firstTerm = pAccum.useMalloc == 0 ? 1 : 0;
                pAccum.useMalloc = 1;
        ...
        private static void setLikeOptFlag(sqlite3 db, string zName, int flagVal)
        {
            ...
            pDef = sqlite3FindFunction(db, zName, sqlite3Strlen30(zName),
                2, SQLITE_UTF8, 0);
            if (ALWAYS(pDef != null)) pDef.flags = (byte)flagVal;
        }
        ...
        private static void sqlite3RegisterLikeFunctions(sqlite3 db, int caseSensitive)
        {
            ...
            if (caseSensitive != 0)
                ...
                pInfo = likeInfoNorm;
            sqlite3CreateFunc(db, "like", 2, SQLITE_ANY, pInfo, likeFunc, null, null);
            sqlite3CreateFunc(db, "like", 3, SQLITE_ANY, pInfo, likeFunc, null, null);
        ...
        private static bool sqlite3IsLikeFunction(sqlite3 db, Expr pExpr, ref bool pIsNocase, char[] aWc)
        {
            ...
            pDef = sqlite3FindFunction(db, pExpr.u.zToken, sqlite3Strlen30(pExpr.u.zToken),
                2, SQLITE_UTF8, 0);
            if (NEVER(pDef == null) || (pDef.flags & SQLITE_FUNC_LIKE) == 0) return false;
...
FuncDef[] aFunc = (FuncDef[])GLOBAL( FuncDef, aBuiltinFunc );
            ...
            var aFunc = aBuiltinFunc;
#endif
            for (i = 0; i < ArraySize(aBuiltinFunc); i++) sqlite3FuncDefInsert(pHash, aFunc[i]);
