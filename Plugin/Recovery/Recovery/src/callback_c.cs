...
namespace CS_SQLite3
{
    public partial class CSSQLite
    {
         ...
         */
        private static void callCollNeeded(sqlite3 db, string zName)
        {
            Debug.Assert(db.xCollNeeded == null || db.xCollNeeded16 == null);
            if (db.xCollNeeded != null)
            {
                var zExternal = zName; // sqlite3DbStrDup(db, zName);
...
sqlite3ValueFree(ref pTmp);
        ...
        )
        {
            ...
            p = pColl;
            if (p == null) p = sqlite3FindCollSeq(db, ENC(db), zName, 0);
            ...
            if (p != null && p.xCmp == null && synthCollSeq(db, p) != 0) p = null;
        ...
        private static int sqlite3CheckCollSeq(Parse pParse, CollSeq pColl)
        {
            if (pColl != null)
            {
                var zName = pColl.zName;
                var p = sqlite3GetCollSeq(pParse.db, pColl, zName);
                if (null == p)
                {
...
//
        ...
        {
            FuncDef pOther;
            var nName = sqlite3Strlen30(pDef.zName);
            var c1 = (u8)pDef.zName[0];
            var h = (sqlite3UpperToLower[c1] + nName) % ArraySize(pHash.a);
        ...
        )
        {
            ...
            */
            ...
            while (p != null)
            {
                var score = matchQuality(p, nArg, enc);
                if (score > bestScore)
                {
                    pBest = p;
                    bestScore = score;
...
FuncDefHash pHash = GLOBAL( FuncDefHash, sqlite3GlobalFunctions );
            ...
            if (pBest != null && (pBest.xStep != null || pBest.xFunc != null || createFlag != 0)) return pBest;
            return null;
        }
         ...
         */
        private static void sqlite3SchemaFree(Schema p)
        {
            Hash temp1;
            Hash temp2;
            HashElem pElem;
            ...
            {
                var pTrigger = (Trigger)sqliteHashData(pElem);
            ...
            {
                var pTab = (Table)pElem.data; //sqliteHashData(pElem);
        ...
        private static Schema sqlite3SchemaGet(sqlite3 db, Btree pBt)
        {
            Schema p;
            if (pBt != null)
                p = sqlite3BtreeSchema(pBt, -1, sqlite3SchemaFree); //Schema.Length, sqlite3SchemaFree);
            else
