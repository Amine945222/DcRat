...
return (p==null || p.nRef!=0 || (p.flags&PGHDR_NEED_SYNC)==0)?1:0;
        ...
        private static void pcacheRemoveFromDirtyList(PgHdr pPage)
        {
            ...
            if (p.pSynced == pPage)
            {
                ...
                while (pSynced != null && (pSynced.flags & PGHDR_NEED_SYNC) != 0) pSynced = pSynced.pDirtyPrev;
                p.pSynced = pSynced;
            }
...
expensive_assert( pcacheCheckSynced(p) );
        ...
        private static void pcacheAddToDirtyList(PgHdr pPage)
        {
            ...
            p.pDirty = pPage;
            if (null == p.pDirtyTail) p.pDirtyTail = pPage;
            if (null == p.pSynced && 0 == (pPage.flags & PGHDR_NEED_SYNC)) p.pSynced = pPage;
#if SQLITE_ENABLE_EXPENSIVE_ASSERT
expensive_assert( pcacheCheckSynced(p) );
#endif
        }
        ...
        private static void pcacheUnpin(PgHdr p)
        {
            ...
            {
                if (p.pgno == 1) pCache.pPage1 = null;
        ...
        )
        {
            ...
            if (pCache.pCache != null) pPage = sqlite3GlobalConfig.pcache.xFetch(pCache.pCache, pgno, eCreate);
...
expensive_assert( pcacheCheckSynced(pCache) );
                ...
                if (pPg != null)
                {
                    ...
                    if (rc != SQLITE_OK && rc != SQLITE_BUSY) return rc;
                }
            ...
            if (pPage != null)
            {
                ...
                if (0 == pPage.nRef) pCache.nRef++;
                pPage.nRef++;
                if (pgno == 1) pCache.pPage1 = pPage;
            }
        ...
        private static void sqlite3PcacheDrop(PgHdr p)
        {
            ...
            if ((p.flags & PGHDR_DIRTY) != 0) pcacheRemoveFromDirtyList(p);
            pCache = p.pCache;
            pCache.nRef--;
            if (p.pgno == 1) pCache.pPage1 = null;
        ...
        {
            PgHdr p;
            while ((p = pCache.pDirty) != null) sqlite3PcacheMakeClean(p);
        }
        ...
        {
            PgHdr p;
            for (p = pCache.pDirty; p != null; p = p.pDirtyNext) p.flags &= ~PGHDR_NEED_SYNC;
        ...
        {
            if (pCache.pCache != null) sqlite3GlobalConfig.pcache.xDestroy(ref pCache.pCache);
        }
        ...
        private static PgHdr pcacheSortDirtyList(PgHdr pIn)
        {
            ...
            p = a[0];
            for (i = 1; i < N_SORT_BUCKET; i++) p = pcacheMergeDirtyList(p, a[i]);
            return p;
        }
        ...
        {
            PgHdr p;
            for (p = pCache.pDirty; p != null; p = p.pDirtyNext) p.pDirty = p.pDirtyNext;
        ...
        {
            var nPage = 0;
            if (pCache.pCache != null) nPage = sqlite3GlobalConfig.pcache.xPagecount(pCache.pCache);
            return nPage;
        }
...
*/
    static int sqlite3PcacheGetCachesize( PCache pCache )
    {
      return pCache.nMax;
        ...
        {
            pCache.nMax = mxPage;
            if (pCache.pCache != null) sqlite3GlobalConfig.pcache.xCachesize(pCache.pCache, mxPage);
        }
