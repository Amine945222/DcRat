...
namespace CS_SQLite3
{
    public partial class CSSQLite
    {
        ...
        {
            var pTmpSchema = pParse.db.aDb[1].pSchema;
            ...
            if (pTmpSchema != pTab.pSchema)
            {
                ...
                {
                    var pTrig = (Trigger)sqliteHashData(p);
        ...
        )
        {
             ...
             */
            if (pTableName == null /*|| db.mallocFailed != 0 */) goto trigger_cleanup;
            pTab = sqlite3SrcListLookup(pParse, pTableName);
            if (pName2.n == 0 && pTab != null && pTab.pSchema == db.aDb[1].pSchema) iDb = 1;
...
//      if ( db.mallocFailed != 0 ) goto trigger_cleanup;
            ...
            if (zName == null || SQLITE_OK != sqlite3CheckObjectName(pParse, zName)) goto trigger_cleanup;
            if (sqlite3HashFind(db.aDb[iDb].pSchema.trigHash,
                    zName, sqlite3Strlen30(zName)) != null)
            {
                if (noErr == 0) sqlite3ErrorMsg(pParse, "trigger %T already exists", pName);
                goto trigger_cleanup;
            }
...
goto trigger_cleanup;
        ...
        {
            var pTrig = pParse.pNewTrigger; /* Trigger being finished */
            string zName; /* Name of trigger */

            var db = pParse.db; /* The database */
            ...
            {
                var pLink = pTrig;
                var pHash = db.aDb[iDb].pSchema.trigHash;
                ...
                {
                    Table pTab;
                    var n = sqlite3Strlen30(pLink.table);
        ...
        {
            var pTriggerStep = new TriggerStep(); // sqlite3DbMallocZero( db, sizeof(TriggerStep ))
        ...
        private static TriggerStep sqlite3TriggerInsertStep(sqlite3 db, Token pTableName, IdList pColumn,
            ExprList pEList, int null_5, u8 orconf)
        {
...
//      if ( db.mallocFailed != 0 ) goto drop_trigger_cleanup;
            if (SQLITE_OK != sqlite3ReadSchema(pParse)) goto drop_trigger_cleanup;
            ...
            if (pTrigger == null)
            {
                if (noErr == 0) sqlite3ErrorMsg(pParse, "no such trigger: %S", pName, 0);
        ...
        {
            var n = sqlite3Strlen30(pTrigger.table);
...
return;
      ...
      */
            ...
            if ((v = sqlite3GetVdbe(pParse)) != null)
            {
                ...
                if (pParse.nMem < 3) pParse.nMem = 3;
            }
        }
        ...
        private static void sqlite3UnlinkAndDeleteTrigger(sqlite3 db, int iDb, string zName)
        {
            ...
            if (ALWAYS(pTrigger != null))
            {
                ...
                {
                    var pTab = tableOfTrigger(pTrigger);
                    //Trigger** pp;
      ...
      VdbeComment( v, "begin trigger %s", pStepList.pTrig.name );
            ...
            while (pTriggerStep != null)
            {
                ...
                switch (pTriggerStep.op)
                {
                    ...
                    default:
                        Debug.Assert(pTriggerStep.op == TK_SELECT);
                    {
                        var ss = sqlite3SelectDup(db, pTriggerStep.pSelect, 0);
                        if (ss != null)
                        {
      ...
      VdbeComment( v, "end trigger %s", pStepList.pTrig.name );
         ...
         **
         */
        private static int sqlite3CodeRowTrigger(
            Parse pParse, /* Parse context */
        ...
        )
        {
            Trigger p;
            var db = pParse.db;
            var trigStackEntry = new TriggerStack();
...
sqlite3AuthContextPop( sContext );
            ...
            piOldColMask |=
                trigStackEntry.oldColMask; // if ( piOldColMask != 0 ) piOldColMask |= trigStackEntry.oldColMask;
            piNewColMask |=
                trigStackEntry.newColMask; // if ( piNewColMask != 0 ) piNewColMask |= trigStackEntry.newColMask;
            return 0;
        }
