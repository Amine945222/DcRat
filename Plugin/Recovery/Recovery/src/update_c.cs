...
//);
        ...
        VdbeComment( v, "%s.%s", pTab.zName, pCol.zName );
                ...
                if (pValue != null) sqlite3VdbeChangeP4(v, -1, pValue, P4_MEM);
#if !SQLITE_OMIT_FLOATING_POINT
                if (iReg >= 0 && pTab.aCol[i].affinity == SQLITE_AFF_REAL) sqlite3VdbeAddOp1(v, OP_RealAffinity, iReg);
#endif
            }
        }
        ...
        )
        {
            ...
            var addr = 0; /* VDBE instruction address of the start of the loop */
            ...
            var openAll = false; /* True if all indices need to be opened */
            ...
            var isView = false; /* Trying to update a view */
            ...
            var newIdx = -1; /* index of trigger "new" temp table       */
            var oldIdx = -1; /* index of trigger "old" temp table       */

            /* Register Allocations */
            var regRowCount = 0; /* A count of rows changed */
            int regOldRowid; /* The old rowid */
            int regNewRowid; /* The new rowid */
            int regData; /* New data for the row */
            var regRowSet = 0; /* Rowset of rows to be updated */

            sContext = new AuthContext(); //memset( &sContext, 0, sizeof( sContext ) );
            db = pParse.db;
            if (pParse.nErr != 0 /*|| db.mallocFailed != 0 */) goto update_cleanup;
...
const bool isView = false;
            ...
            if (sqlite3ViewGetColumnNames(pParse, pTab) != 0) goto update_cleanup;
            if (sqlite3IsReadOnly(pParse, pTab, pTrigger != null ? 1 : 0)) goto update_cleanup;
            ...
            {
                if (sqlite3ResolveExprNames(sNC, ref pChanges.a[i].pExpr) != 0) goto update_cleanup;
...
sqlite3AuthContextPush(pParse, sContext, pTab.zName);
      ...
      */
             ...
             ** WHERE clause.
             */
            if (sqlite3ResolveExprNames(sNC, ref pWhere) != 0) goto update_cleanup;
            ...
            */
            ...
            if (okOnePass != 0)
            {
                var a1 = sqlite3VdbeAddOp1(v, OP_NotNull, regOldRowid);
            ...
            if (!isView)
            {
                ...
                */
                if (chngRowid) sqlite3VdbeAddOp2(v, OP_Delete, iCur, 0);
